-- =============================================================================
-- vw_rp_otel_detay_proje_maliyet — Otel Detaylı Proje Maliyet Raporu (GÜNCELLENDİ)
-- Rapor Merkezi / backend: vw_rp_otel_detay_proje_maliyet
--
-- Bu görünüm, proje satış kalemlerini (project_sales_items) temel alır.
-- Künye (projects) üzerindeki tarihler yerine, hotels_data içindeki 
-- otele özel tarihleri (C/IN, C/OUT) kullanır.
-- =============================================================================
...
WITH project_dates AS (
    -- Her proje-otel çifti için hotels_data içindeki tarihleri ayıkla
    SELECT 
        p.id as project_id,
...
SELECT
  ...
  LEFT(COALESCE(NULLIF(TRIM(h.name), ''), '-'), 255)::varchar AS otel,
...
'Otel bazlı proje maliyetleri: Tarihleri hotels_data JSON alanından çeker, her kalemi kendi oteliyle eşleştirir.';
