...
for (const file of files) {
  ...
  // If it's React.memo, find the actual function name
  if (mainFuncName === 'AccommodationTabOptimized' || !mainFuncName) {
      // Just fallback to looking for the main component name from filename
  ...
  traverse(ast, {
    JSXOpeningElement(path) {
      ...
      if (name === 'input' || name === 'select' || name === 'textarea') {
        ...
        if (!hasDisabled) {
          const closestFuncDecl = path.findParent(p => t.isFunctionDeclaration(p) || (t.isVariableDeclarator(p) && t.isArrowFunctionExpression(p.node.init)));
      ...
      if (path.node.id && path.node.id.name === mainFuncName) {
         ...
         if (propsParam && t.isIdentifier(propsParam)) {
            propsVarName = propsParam.name;
             ...
             // we can extract it from the first param using arguments[0]
         ...
         if (!hasPermEdit) {
            const hookInjection = parser.parse(`
              ...
              const compIsLocked = ${propsVarName}?.isLocked || ${propsVarName}?.project?.locked || false;
            `).program.body;
