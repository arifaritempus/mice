...
new_states = """const [showAddRow, setShowAddRow] = useState(false);
  const [quoteModalOpen, setQuoteModalOpen] = useState(false);
  const [selectedQuoteHotels, setSelectedQuoteHotels] = useState<string[]>([]);
  const [isConverting, setIsConverting] = useState(false);"""
text = text.replace(states_marker, new_states, 1)
...
text = text.replace(top_btn_marker, new_top_btn, 1)
...
# 5. Conversion Function (Place right before the FIRST "return (" that starts the main JSX)
# The main return is around line 430. We can look for:
...
func_logic = """
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0F172A] pb-20">
"""
text = text.replace(main_return_marker, func_logic, 1)
...
"""
    text = text[:idx] + modal_jsx

with open("frontend/src/app/requests/edit/[id]/page.tsx", "w") as f:
    f.write(text)

print("Multi-quote safe script executed!")
