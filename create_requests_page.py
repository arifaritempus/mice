...
const SearchableSelect = ({ options, value, onChange, placeholder }: any) => {
  ...
  return (
    <div className="relative" ref={ref}>
      <div
        className="w-full bg-v3-bg border border-v3-border rounded-xl px-4 py-2.5 text-sm cursor-pointer flex justify-between items-center"
...
const MultiHotelSelect = ({ options, selectedValues, onChange }: any) => {
  ...
  return (
    <div className="relative" ref={ref}>
      <div
        className="w-full bg-v3-bg border border-v3-border rounded-xl px-4 py-2 text-sm cursor-pointer min-h-[44px] flex items-center flex-wrap gap-1"
        onClick={() => setIsOpen(!isOpen)}
      >
...
export default function CreateRequestPage() {
  ...
  // Event State
  const [meeting, setMeeting] = useState({ requested: false, date: null as Date | null, notes: "" });
  const [cocktail, setCocktail] = useState({ requested: false, date: null as Date | null, notes: "" });
  const [gala, setGala] = useState({ requested: false, date: null as Date | null, notes: "" });
    ...
    try {
      ...
      const { data: reqData, error: reqErr } = await supabase
        ...
        .insert({
          ...
          agency_id: agencyId,
          date_type: dateType,
          date_details: dateType === "EXACT" ? { check_in: checkIn?.toISOString(), check_out: checkOut?.toISOString() } : { text: flexibleDateText },
          ...
          meeting: { requested: meeting.requested, date: meeting.date?.toISOString() || null, notes: meeting.notes },
          cocktail: { requested: cocktail.requested, date: cocktail.date?.toISOString() || null, notes: cocktail.notes },
          gala: { requested: gala.requested, date: gala.date?.toISOString() || null, notes: gala.notes },
  ...
  return (
    <div className="flex flex-col h-full bg-v3-bg">
                    ...
                    <div>
                      ...
                      <DatePicker
                        ...
                        onChange={(date) => {
                           ...
                           if (date && checkOut) {
                              const diffTime = Math.abs(checkOut.getTime() - date.getTime());
                    ...
                    <div className="mt-4 space-y-3 pl-7 animate-in fade-in slide-in-from-top-2 duration-200">
                      <div>
                        ...
                        <DatePicker
                          ...
                          onChange={(date) => setMeeting({...meeting, date})}
                          locale={tr}
                          dateFormat="dd MMM yyyy"
                          className="w-full bg-white dark:bg-gray-800 border border-v3-border rounded-lg px-3 py-1.5 text-xs"
                          placeholderText="Tarih seçin"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-v3-muted uppercase mb-1">Not / Düzen</label>
                        <textarea rows={2} value={meeting.notes} onChange={(e) => setMeeting({...meeting, notes: e.target.value})} className="w-full bg-white dark:bg-gray-800 border border-v3-border rounded-lg px-3 py-2 text-xs resize-none" placeholder="U-Düzen, Tiyatro vb..." />
                      </div>
                    </div>
                  )}
                </div>
                  ...
                  {cocktail.requested && (
                    <div className="mt-4 space-y-3 pl-7 animate-in fade-in slide-in-from-top-2 duration-200">
                      <div>
                        <label className="block text-[10px] font-bold text-v3-muted uppercase mb-1">Tarih (Opsiyonel)</label>
                        <DatePicker
                          selected={cocktail.date}
                          onChange={(date) => setCocktail({...cocktail, date})}
                          locale={tr}
                          dateFormat="dd MMM yyyy"
                          className="w-full bg-white dark:bg-gray-800 border border-v3-border rounded-lg px-3 py-1.5 text-xs"
                          placeholderText="Tarih seçin"
                  ...
                  {gala.requested && (
                    <div className="mt-4 space-y-3 pl-7 animate-in fade-in slide-in-from-top-2 duration-200">
                      <div>
                        ...
                        <DatePicker
                          selected={gala.date}
                          onChange={(date) => setGala({...gala, date})}
                          locale={tr}
                          dateFormat="dd MMM yyyy"
                          className="w-full bg-white dark:bg-gray-800 border border-v3-border rounded-lg px-3 py-1.5 text-xs"
                          placeholderText="Tarih seçin"
                      ...
                      <div>
                        ...
                        <textarea rows={2} value={gala.notes} onChange={(e) => setGala({...gala, notes: e.target.value})} className="w-full bg-white dark:bg-gray-800 border border-v3-border rounded-lg px-3 py-2 text-xs resize-none" placeholder="Canlı müzik, limitsiz alkol vb..." />
                      </div>
                    </div>
                  )}
                </div>
      ...
      {/* FOOTER ACTIONS */}
      <div className="fixed bottom-0 left-0 right-0 lg:left-64 xl:left-72 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-t border-v3-border p-4 z-40 flex items-center justify-between">
        <button
          onClick={() => router.back()}
          disabled={isSubmitting}
          className="px-6 py-2.5 rounded-xl border border-v3-border bg-v3-surface text-v3-text text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors disabled:opacity-50"
        >
          İptal Et
        </button>
        <div className="flex gap-3">
          <button
            ...
            disabled={isSubmitting}
            className="px-6 py-2.5 rounded-xl border border-blue-200 dark:border-blue-800/50 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 text-sm font-bold hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors disabled:opacity-50"
          >
            Sadece Kaydet
          </button>
          <button
            ...
            disabled={isSubmitting}
            className="px-6 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-bold hover:bg-blue-700 shadow-lg shadow-blue-500/25 transition-all hover:scale-105 active:scale-95 disabled:opacity-50 flex items-center gap-2"
          >
            {isSubmitting ? (
...
os.makedirs("frontend/src/app/requests/create", exist_ok=True)
with open("frontend/src/app/requests/create/page.tsx", "w") as f:
    f.write(content)

print("Created requests/create/page.tsx successfully.")
