...
def apply_foolproof_mockup_v14(filepath):
...
"""
    ...
    modern_info_and_form = """{/* Mockup Sejour Bilgileri */}
                <div className={`bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 shadow-sm mb-8 ${isEditingInfo ? 'hidden' : 'block'}`}>
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-sm font-black text-[#1e293b] dark:text-white">Sejour Bilgileri</h3>
                    <button type="button" onClick={() => setIsEditingInfo(true)} className="flex items-center gap-2 px-3 py-1.5 text-[10px] font-bold text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-all shadow-sm">
                      ✏️ Detayları Düzenle
                    </button>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
                    ...
                    <div>
                      <p className="text-[10px] text-gray-500 mb-2">Müşteri Tipi</p>
                      <p className="text-xs font-black text-[#1e293b] dark:text-gray-200">{(salesData as any).customerType === 'agency' ? 'ACENTE' : 'ŞAHIS'}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-2">{(salesData as any).customerType === 'agency' ? 'Acente' : 'Müşteri Adı'}</p>
                    ...
                    <div>
                      <p className="text-[10px] text-gray-500 mb-2">Giriş - Çıkış Tarihi</p>
                    ...
                    <div>
                      <p className="text-[10px] text-gray-500 mb-2">Rezervasyon Durumu</p>
                ...
                {/* Mockup MODERN Düzenle Formu */}
                <div className={`bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 shadow-sm mb-8 animate-in fade-in slide-in-from-top-4 duration-300 ${isEditingInfo ? 'block' : 'hidden'}`}>
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-sm font-black text-[#1e293b] dark:text-white">Sejour Bilgilerini Düzenle</h3>
                    <button type="button" onClick={() => setIsEditingInfo(false)} className="flex items-center gap-2 px-4 py-2 text-[10px] font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-all shadow-sm">
                      Kaydet ve Kapat
                    </button>
                  </div>
                  
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    {/* Left & Middle: Core Fields */}
                    <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                      {/* Voucher */}
                      <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Voucher Numarası *</label>
                        <input type="text" name="voucherNumber" value={salesData.voucherNumber} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50/50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-bold text-[#1e293b] dark:text-gray-200 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none" placeholder="VOU-YENİ" required />
                      </div>
                      
                      {/* Müşteri Tipi */}
                      <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Müşteri Tipi *</label>
                        <div className="flex p-1 bg-gray-50/50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700">
                          <button type="button" onClick={() => handleInputChange({target: {name: 'customerType', value: 'agency'}} as any)} className={`flex-1 py-2.5 text-[10px] font-black tracking-widest rounded-lg transition-all ${salesData.customerType === 'agency' ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}>ACENTE</button>
                          <button type="button" onClick={() => handleInputChange({target: {name: 'customerType', value: 'individual'}} as any)} className={`flex-1 py-2.5 text-[10px] font-black tracking-widest rounded-lg transition-all ${salesData.customerType === 'individual' ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}>ŞAHIS</button>
                        </div>
                      </div>
                      ...
                      <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">
                          {salesData.customerType === 'agency' ? 'Acente Seçimi *' : 'Müşteri Adı Soyadı *'}
                        </label>
                        {salesData.customerType === 'agency' ? (
                          <div className="relative z-[60]"><SearchableSelect options={agencies} value={salesData.agencyId} onChange={(id) => handleInputChange({target: {name: 'agencyId', value: id}} as any)} placeholder="Acente ara..." className="w-full text-xs" /></div>
                        ) : (
                          <input type="text" name="customerName" value={salesData.customerName} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50/50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-bold text-[#1e293b] dark:text-gray-200 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none" placeholder="Örn: Ahmet Yılmaz" />
                        )}
                      </div>

                      {/* Durum */}
                      <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Rezervasyon Durumu</label>
                        <div className="flex p-1 bg-gray-50/50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 h-[44px]">
                          {['BEKLEMEDE', 'KONFIRME', 'İPTAL'].map(status => (
                              <button key={status} type="button" onClick={() => handleInputChange({target: {name: 'status', value: status}} as any)} className={`flex-1 text-[9px] font-black tracking-tighter rounded-lg transition-all ${salesData.status === status ? (status === 'BEKLEMEDE' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600' : status === 'KONFIRME' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600' : 'bg-red-100 dark:bg-red-900/30 text-red-600') : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}>
                      ...
                      {/* Giriş Tarihi */}
                      <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Giriş Tarihi *</label>
                        <input type="date" name="checkInDate" value={salesData.checkInDate} onChange={handleInputChange} className="w-full px-4 py-2.5 bg-gray-50/50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-bold text-[#1e293b] dark:text-gray-200 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none" />
                      </div>

                      {/* Çıkış Tarihi */}
                      <div>
                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Çıkış Tarihi *</label>
                        <input type="date" name="checkOutDate" value={salesData.checkOutDate} onChange={handleInputChange} className="w-full px-4 py-2.5 bg-gray-50/50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-bold text-[#1e293b] dark:text-gray-200 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none" />
                      </div>
                    </div>
                    
                    {/* Right Column: Hizmet Seçimi (Toggles) */}
                    <div className="bg-gray-50/50 dark:bg-gray-800/30 border border-gray-100 dark:border-gray-800 rounded-2xl p-5">
                      <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3">Hizmet Seçimi</h3>
                      <div className="space-y-2">
                        <button type="button" onClick={() => setShowAccommodation(!showAccommodation)} className={`w-full flex items-center justify-between p-3 rounded-xl border-2 transition-all duration-300 ${showAccommodation ? "bg-blue-50 dark:bg-blue-900/20 border-blue-500 text-blue-600 shadow-sm" : "bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 text-gray-500"}`}>
                          <div className="flex items-center gap-3">
                            <span className="text-base">🏨</span>
                            <span className="text-[10px] font-black tracking-widest">KONAKLAMA</span>
                          </div>
                          {showAccommodation && <span className="text-blue-500">✓</span>}
                        </button>

                        <button type="button" onClick={() => setShowFlight(!showFlight)} className={`w-full flex items-center justify-between p-3 rounded-xl border-2 transition-all duration-300 ${showFlight ? "bg-emerald-50 dark:bg-emerald-900/20 border-emerald-500 text-emerald-600 shadow-sm" : "bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 text-gray-500"}`}>
                          <div className="flex items-center gap-3">
                            <span className="text-base">✈️</span>
                            <span className="text-[10px] font-black tracking-widest">UÇUŞ BİLGİSİ</span>
                          </div>
                          {showFlight && <span className="text-emerald-500">✓</span>}
                        </button>

                        <button type="button" onClick={() => setShowTransfer(!showTransfer)} className={`w-full flex items-center justify-between p-3 rounded-xl border-2 transition-all duration-300 ${showTransfer ? "bg-purple-50 dark:bg-purple-900/20 border-purple-500 text-purple-600 shadow-sm" : "bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 text-gray-500"}`}>
                          <div className="flex items-center gap-3">
                            <span className="text-base">🚗</span>
                            <span className="text-[10px] font-black tracking-widest">TRANSFER</span>
                          </div>
                          {showTransfer && <span className="text-purple-500">✓</span>}
                        </button>

                        <button type="button" onClick={() => setShowExtraServices(!showExtraServices)} className={`w-full flex items-center justify-between p-3 rounded-xl border-2 transition-all duration-300 ${showExtraServices ? "bg-orange-50 dark:bg-orange-900/20 border-orange-500 text-orange-600 shadow-sm" : "bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 text-gray-500"}`}>
                          <div className="flex items-center gap-3">
                            <span className="text-base">✨</span>
                            <span className="text-[10px] font-black tracking-widest">EKSTRA HİZMET</span>
                          </div>
    ...
    if "Mockup MODERN Düzenle Formu" not in content:
        ...
        if count == 0:
            ...
            print(f"SUCCESSFULLY INJECTED MODERN FORM IN {filepath}")
