...
def apply_foolproof_mockup_v7(filepath):
...
"""
    ...
    if "Mockup Sejour Bilgileri" not in content:
        ...
        rooms_header = """\n                  {/* KONAKLAMA ACCORDION ROW */}
                  ...
                  {showAccommodation && ("""
        content = content.replace('\n                {showAccommodation && (', rooms_header, 1)
        ...
        transfers_header = """\n                  </div>
                  ...
                  {showTransfer && ("""
        content = content.replace('\n                {showTransfer && (', transfers_header, 1)
        ...
        extras_header = """\n                  </div>
                  ...
                  {showExtraServices && ("""
        content = content.replace('\n                {showExtraServices && (', extras_header, 1)

        # 9. Close the last div and insert Totals BEFORE the `)}` that precedes Purchase
        totals_row = """\n                  </div>
        ...
        # Replace the precise sequence that closes the Sales Tab
        target_seq1 = '            )}\n\n            {/* Purchase Tab */}'
        target_seq2 = '            )}\n            {/* Purchase Tab */}'
        
        if target_seq1 in content:
            content = content.replace(target_seq1, totals_row + '\n            )}\n\n            {/* Purchase Tab */}', 1)
        elif target_seq2 in content:
            content = content.replace(target_seq2, totals_row + '\n            )}\n            {/* Purchase Tab */}', 1)

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Applied 100% BULLETPROOF V7 mockup design to {filepath}")

for fp in files:
    apply_foolproof_mockup_v7(fp)
