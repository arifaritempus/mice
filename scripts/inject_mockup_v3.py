...
def inject_mockup(filepath):
    ...
    if "V3 MOCKUP UI" not in content:
        ...
        # Using rsplit to ONLY replace the VERY LAST occurrence of the closing tags!
        last_divs = '    </div>\n  );\n}'
        parts = content.rsplit(last_divs, 1)
        if len(parts) == 2:
            content = parts[0] + '    </div>\n      </div>\n    </>\n  );\n}'

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Applied SUPER SAFE mockup wrapper to {filepath}")
