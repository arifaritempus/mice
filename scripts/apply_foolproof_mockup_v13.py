...
def apply_foolproof_mockup_v13(filepath):
...
"""
    ...
    if "Mockup MODERN Düzenle Formu" not in content:
        # We find from `<div className="grid grid-cols-1 lg:grid-cols-3 gap-2 responsive-filter-grid">`
        # down to `{/* Accommodation Section */}`
        # and replace EVERYTHING with `modern_info_and_form`
        # Note: We need to match spanning multiple lines.
        ...
        # Hide Satış Bilgileri if it's outside our match.
        content = content.replace(
