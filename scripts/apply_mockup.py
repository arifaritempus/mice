...
"""
    ...
    if "Mockup Top Header" not in content:
        ...
        content = content.replace('<div className="max-w-[1800px] mx-auto">', '') # Removed because we added it inside the new header correctly

    # 3. Main Tabs replacement
    ...
    info_summary_ui = """                {/* Mockup Sejour Bilgileri */}
                <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 shadow-sm mb-12">
                  ...
                  {!isEditingInfo ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 responsive-filter-grid mt-4 pt-4 border-t border-gray-100">
"""
    if "Mockup Sejour Bilgileri" not in content:
        content = content.replace(info_start, info_summary_ui)
        # We must close the wrapper at the end of the general info block.
        # General info block ends right before Rooms.
        # Rooms start at `<div className="bg-v3-surface border-2 border-blue-100`
        rooms_start = '<div className="bg-v3-surface border-2 border-blue-100'
        
        hizmetler_header = """                    </div>
                  )}
                </div>
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden mb-6">
                  ...
                  {/* EXPANDED CONTENT FOR ROOMS */}
                  <div className={`border-t border-gray-100 bg-gray-50/50 p-6 ${expandedSection === 'rooms' ? 'block' : 'hidden'}`}>
                    <div className="bg-v3-surface border-2 border-blue-100"""
        content = content.replace(rooms_start, hizmetler_header)
        
        # Now do Flights
        flights_start = '<div className="bg-v3-surface border-2 border-emerald-100'
        flights_header = """                  </div>
                  
                  {/* UÇUŞ ACCORDION ROW */}
                  <div className="flex items-center p-5 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 border-t border-b border-gray-100 dark:border-gray-800 transition-colors" onClick={() => setExpandedSection(expandedSection === 'flights' ? null : 'flights')}>
                    <div className="w-10 h-10 flex items-center justify-center bg-[#f0f5ff] dark:bg-blue-900/20 text-blue-600 rounded-xl mr-5">
                  ...
                  {/* EXPANDED CONTENT FOR FLIGHTS */}
                  <div className={`border-t border-gray-100 bg-gray-50/50 p-6 ${expandedSection === 'flights' ? 'block' : 'hidden'}`}>
                    <div className="bg-v3-surface border-2 border-emerald-100"""
        content = content.replace(flights_start, flights_header)
        
        # Now do Transfers
        transfers_start = '<div className="bg-v3-surface border-2 border-purple-100'
        transfers_header = """                  </div>
                  
                  {/* TRANSFER ACCORDION ROW */}
                  <div className="flex items-center p-5 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 border-t border-b border-gray-100 dark:border-gray-800 transition-colors" onClick={() => setExpandedSection(expandedSection === 'transfers' ? null : 'transfers')}>
                    <div className="w-10 h-10 flex items-center justify-center bg-[#f0f5ff] dark:bg-blue-900/20 text-blue-600 rounded-xl mr-5">
                  ...
                  {/* EXPANDED CONTENT FOR TRANSFERS */}
                  <div className={`border-t border-gray-100 bg-gray-50/50 p-6 ${expandedSection === 'transfers' ? 'block' : 'hidden'}`}>
                    <div className="bg-v3-surface border-2 border-purple-100"""
        content = content.replace(transfers_start, transfers_header)

        # Now do Extras
        extras_start = '<div className="bg-v3-surface border-2 border-orange-100'
        extras_header = """                  </div>
                  
                  {/* EXTRA ACCORDION ROW */}
                  <div className="flex items-center p-5 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 border-t border-gray-100 dark:border-gray-800 transition-colors" onClick={() => setExpandedSection(expandedSection === 'extras' ? null : 'extras')}>
                    <div className="w-10 h-10 flex items-center justify-center bg-[#f0f5ff] dark:bg-blue-900/20 text-blue-600 rounded-xl mr-5">
                  ...
                  {/* EXPANDED CONTENT FOR EXTRAS */}
                  <div className={`border-t border-gray-100 bg-gray-50/50 p-6 ${expandedSection === 'extras' ? 'block' : 'hidden'}`}>
                    <div className="bg-v3-surface border-2 border-orange-100"""
        content = content.replace(extras_start, extras_header)

        # Close the Hizmetler container at the end of extras
        # The extras container ends before the Finance block.
        finance_start = '<div className="bg-v3-surface border border-v3-border rounded shadow-lg transition-all duration-300">'
        finance_header = """                  </div>
                </div>
                
                {/* MOCKUP TOTALS ROW */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                  <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 p-6 shadow-sm flex items-center">
                    <div className="w-10 h-10 flex items-center justify-center bg-[#f0f5ff] text-blue-600 rounded-lg mr-4">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-1">Toplam Satış</p>
                      <p className="text-sm font-black text-[#1e293b]">459.920,00 EUR</p>
                    </div>
                  </div>
                  <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 p-6 shadow-sm flex items-center">
                    <div className="w-10 h-10 flex items-center justify-center bg-[#f0f5ff] text-blue-600 rounded-lg mr-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-black text-[#1e293b]">385.120,00 EUR</p>
                    </div>
                  </div>
                  <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 p-6 shadow-sm flex items-center">
                    <div className="w-10 h-10 flex items-center justify-center bg-[#e6fff2] text-[#00b368] rounded-lg mr-4">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500 mb-1">Tahmini Kâr</p>
                      <p className="text-sm font-black text-[#1e293b]">74.800,00 EUR</p>
                    </div>
                  </div>
                </div>
        ...
        content = content.replace(finance_start, finance_header)
        
        # Make sure the "Satış Bilgileri" header is hidden from the original code since we replaced it with Sejour Bilgileri card
        content = content.replace(
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Applied mockup design to {filepath}")
