...
def apply_foolproof_mockup_v2(filepath):
...
"""
    ...
    if "Mockup Sejour Bilgileri" not in content:
        ...
        # Fix: ONLY inject the Hizmetler Header and the opening div for the accordions wrapper!
        # Do NOT inject an extra </div> here! The original grid was already closed perfectly in the original file.
        content = content.replace(
        ...
        extras_header = """                  </div>
                  ...
                  {showExtraServices && ("""
        content = re.sub(r'\{\/\* (?:Extra Services|Extra Services Information) \*\/\}\n\s*\{showExtraServices && \(', extras_header, content, count=1)
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Applied robust wrapper-based mockup design to {filepath}")

for fp in files:
    apply_foolproof_mockup_v2(fp)
