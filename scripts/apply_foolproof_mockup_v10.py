...
def apply_foolproof_mockup_v10(filepath):
...
"""
    ...
    if "Mockup Sejour Bilgileri" not in content:
        ...
        content, n_replaced = re.subn(
            r'(\s*)<div className="grid grid-cols-1 md:grid-cols-4 gap-2 responsive-filter-grid">',
            r'\1' + totals_row + r'\1<div className="hidden grid-cols-1 md:grid-cols-4 gap-2 responsive-filter-grid">',
            content,
            count=1
        )
        if n_replaced == 0:
