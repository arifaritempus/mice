...
def apply_foolproof_mockup_v11(filepath):
...
"""
    ...
    if "Mockup Sejour Bilgileri" not in content:
        ...
        # 9. Close the last div and insert Totals BEFORE the Summary Block (which is inside the grid!)
        totals_row = """                  </div>
        ...
        # Replace the original Summary Block start with the totals row and a hidden version of the block
        content, n_replaced = re.subn(
            r'(\s*)<div className="grid grid-cols-1 md:grid-cols-4 gap-2 (flex-1 )?responsive-filter-grid">',
            lambda m: m.group(1) + totals_row + m.group(1) + f'<div className="hidden grid-cols-1 md:grid-cols-4 gap-2 {m.group(2) if m.group(2) else ""}responsive-filter-grid">',
            content,
            count=1
        )
        if n_replaced == 0:
