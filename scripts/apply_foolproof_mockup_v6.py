...
def apply_foolproof_mockup_v6(filepath):
...
"""
    ...
    if "Mockup Sejour Bilgileri" not in content:
        ...
        # 5. Accommodation (WITH NEWLINE TO FORCE EXACT 16 SPACES MATCH)
        ...
        totals_row = """\n                  </div>
            ...
            {/* Purchase Tab */}"""
        content = content.replace('\n            {/* Purchase Tab */}', totals_row, 1)

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Applied SUPER ROBUST V6 mockup design to {filepath}")

for fp in files:
    apply_foolproof_mockup_v6(fp)
