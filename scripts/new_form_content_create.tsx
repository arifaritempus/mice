        <form onSubmit={handleSubmit} className="relative pb-32">
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl shadow-sm mb-6 overflow-hidden">
            ...
            {!isEditingInfo ? (
              <div className="grid grid-cols-2 md:grid-cols-6 gap-4 p-5">
                ...
                <div>
                  <p className="text-[10px] text-gray-400 mb-1 uppercase tracking-wider font-semibold">Rezervasyon T.</p>
                  <p className="text-xs font-bold text-gray-800 dark:text-gray-200">
                    {salesData.creationDate ? new Date(salesData.creationDate).toLocaleDateString("tr-TR") : "-"}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-gray-400 mb-1 uppercase tracking-wider font-semibold">Pax</p>
                  <p className="text-xs font-bold text-gray-800 dark:text-gray-200">{rooms.reduce((acc, r) => acc + (r.adultCount||0) + (r.childCount||0) + (r.infantCount||0), 0)} Kişi</p>
                </div>
                <div>
              ...
              <div className="p-5 bg-gray-50/50 dark:bg-gray-900/50">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div>
                    <label className="block text-[10px] font-semibold text-gray-600 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Voucher Numarası *</label>
                    <input type="text" name="voucherNumber" value={salesData.voucherNumber} onChange={handleInputChange} className="w-full h-[36px] px-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md text-xs font-medium text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm" required />
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block text-[10px] font-semibold text-gray-600 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Rezervasyon Tarihi *</label>
                    <input type="date" name="creationDate" value={salesData.creationDate} onChange={handleInputChange} className="w-full h-[36px] px-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md text-xs font-medium text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm" required />
                  </div>
                  <div>
                    <label className="block text-[10px] font-semibold text-gray-600 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Operatör Referans</label>
                    <input type="text" name="operatorReference" value={salesData.operatorReference} onChange={handleInputChange} className="w-full h-[36px] px-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md text-xs font-medium text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-semibold text-gray-600 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Sisteme Giren</label>
                    <input type="text" name="enteredBy" value={salesData.enteredBy} onChange={handleInputChange} className="w-full h-[36px] px-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md text-xs font-medium text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm" />
                  </div>
                  <div>
                  ...
                  <div className="md:col-span-4 mt-2">
                    ...
                    <textarea name="notes" value={salesData.notes} onChange={handleInputChange} rows={2} className="w-full px-3 py-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md text-xs font-medium text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm"></textarea>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Hizmetler</h2>
                ...
                <div className="border-t border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-800/20 p-4">
                  ...
                  <div className="space-y-4">
                    {rooms.map((room, index) => {
                      const tab = activeInnerTabs[room.id] || "sales";
                      return (
                        <div key={room.id} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
                          {/* Inner Tabs Header */}
                          <div className="flex border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 px-4">
                            <div className="flex-1 flex gap-6">
                              <button type="button" onClick={() => setActiveInnerTabs(p => ({...p, [room.id]: "sales"}))} className={`py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors ${tab === "sales" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>Satış Detayları</button>
                              <button type="button" onClick={() => setActiveInnerTabs(p => ({...p, [room.id]: "purchase"}))} className={`py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors ${tab === "purchase" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>Alış (Maliyet)</button>
                              <button type="button" onClick={() => setActiveInnerTabs(p => ({...p, [room.id]: "collection"}))} className={`py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors ${tab === "collection" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>Tahsilat</button>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] font-bold text-gray-400">ODA {index + 1}</span>
                              <button type="button" onClick={() => removeRoom(room.id)} className="text-gray-400 hover:text-red-500 transition-colors" title="Odayı Sil">
                          ...
                          <div className="p-4">
                            {tab === "sales" && (
                              <div className="flex flex-col lg:flex-row gap-3 items-end w-full lg:[&>*:nth-child(1)]:flex-[2] lg:[&>*:nth-child(2)]:flex-[1] lg:[&>*:nth-child(3)]:flex-[1] lg:[&>*:nth-child(4)]:flex-[1.5] lg:[&>*:nth-child(5)]:flex-[1.5] lg:[&>*:nth-child(6)]:flex-[1.2]">
                                <div>
                                ...
                                <div>
                                  ...
                                  <select value={room.roomType || ""} onChange={(e) => updateRoom(room.id, "roomType", e.target.value)} className="w-full h-[36px] px-3 bg-white border border-gray-200 rounded-md text-xs font-medium focus:border-blue-500 outline-none">
                                ...
                                <div>
                                  ...
                                  <div className="flex gap-0 items-center border border-gray-200 rounded-md overflow-hidden h-[36px]">
                                    <input type="number" min="1" value={room.adultCount || 1} onChange={(e) => updateRoom(room.id, "adultCount", parseInt(e.target.value) || 1)} className="w-1/3 h-full text-center text-xs font-semibold outline-none focus:bg-blue-50" title="Yetişkin" />
                                    <div className="w-px h-4 bg-gray-200"></div>
                                    <input type="number" min="0" value={room.childCount || 0} onChange={(e) => updateRoom(room.id, "childCount", parseInt(e.target.value) || 0)} className="w-1/3 h-full text-center text-xs font-semibold outline-none focus:bg-blue-50" title="Çocuk" />
                                    <div className="w-px h-4 bg-gray-200"></div>
                                    <input type="number" min="0" value={room.infantCount || 0} onChange={(e) => updateRoom(room.id, "infantCount", parseInt(e.target.value) || 0)} className="w-1/3 h-full text-center text-xs font-semibold outline-none focus:bg-blue-50" title="Bebek" />
                                  </div>
                                </div>
                                <div>
                                ...
                                <div>
                                  ...
                                  <input type="text" placeholder="İsim Soyisim" value={room.guestInfo || ""} onChange={(e) => updateRoom(room.id, "guestInfo", e.target.value)} className="w-full h-[36px] px-3 border border-gray-200 rounded-md text-xs font-medium outline-none focus:border-blue-500" />
                                </div>
                                <div>
                                  <label className="block text-[10px] font-semibold text-blue-600 uppercase tracking-wider mb-1.5">SATIŞ TUTARI</label>
                                  <div className="flex items-center gap-1">
                                    <input type="text" placeholder="0,00" value={roomPriceInput[room.id] !== undefined ? roomPriceInput[room.id] : room.price ? room.price.toString().replace(".", ",") : ""} onChange={(e) => setRoomPriceInput((prev) => ({ ...prev, [room.id]: e.target.value }))} onBlur={(e) => { const parsed = parseAmount(e.target.value); if (parsed !== null) { updateRoom(room.id, "price", parsed); setRoomPriceInput((prev) => ({ ...prev, [room.id]: formatAmount(parsed) })); } }} className="flex-1 h-[36px] px-2 text-right border border-gray-200 rounded-md text-xs font-bold text-blue-600 outline-none focus:border-blue-500 bg-blue-50/30" />
                                    <select value={room.currency || "USD"} onChange={(e) => updateRoom(room.id, "currency", e.target.value)} className="w-[60px] h-[36px] px-1 bg-gray-50 border border-gray-200 rounded-md text-xs font-bold outline-none">
                            ...
                            {tab === "purchase" && (
                              <div className="flex flex-col lg:flex-row gap-3 items-end w-full lg:[&>*:nth-child(1)]:flex-[2] lg:[&>*:nth-child(2)]:flex-[1.5]">
                                <div>
                                ...
                                <div>
                                  ...
                                  <div className="flex items-center gap-1">
                                    <input type="text" placeholder="0,00" value={roomCostInput?.[room.id] !== undefined ? roomCostInput[room.id] : room.costPrice ? room.costPrice.toString().replace(".", ",") : ""} onChange={(e) => setRoomCostInput((prev) => ({ ...prev, [room.id]: e.target.value }))} onBlur={(e) => { const parsed = parseAmount(e.target.value); if (parsed !== null) { updateRoom(room.id, "costPrice", parsed); setRoomCostInput((prev) => ({ ...prev, [room.id]: formatAmount(parsed) })); } }} className="flex-1 h-[36px] px-2 text-right border border-gray-200 rounded-md text-xs font-bold text-gray-700 outline-none focus:border-gray-500" />
                                    <select value={room.costCurrency || "USD"} onChange={(e) => updateRoom(room.id, "costCurrency", e.target.value)} className="w-[60px] h-[36px] px-1 bg-gray-50 border border-gray-200 rounded-md text-xs font-bold outline-none">
                                      <option value="USD">USD</option><option value="EUR">EUR</option><option value="TRY">TRY</option><option value="GBP">GBP</option>
                                    </select>
                                  </div>
                                </div>
                            ...
                            {tab === "collection" && (
                              <div className="py-4 text-center border-2 border-dashed border-gray-200 rounded-lg">
                                ...
                                <h3 className="text-xs font-bold text-gray-700">Tahsilat & Ödeme</h3>
                                <p className="text-[10px] text-gray-500 mt-1">Bu modül yapım aşamasındadır veya harici yönetilmektedir.</p>
                              </div>
                            )}
                          </div>
                        </div>
                ...
                <div className="border-t border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-800/20 p-4">
                  ...
                  <div className="space-y-4">
                    {flights.map((flight, index) => {
                      const tab = activeInnerTabs[flight.id] || "sales";
                      return (
                        <div key={flight.id} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
                          {/* Inner Tabs Header */}
                          <div className="flex border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 px-4">
                            <div className="flex-1 flex gap-6">
                              <button type="button" onClick={() => setActiveInnerTabs(p => ({...p, [flight.id]: "sales"}))} className={`py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors ${tab === "sales" ? "border-emerald-600 text-emerald-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>Satış Detayları</button>
                              <button type="button" onClick={() => setActiveInnerTabs(p => ({...p, [flight.id]: "purchase"}))} className={`py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors ${tab === "purchase" ? "border-emerald-600 text-emerald-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>Alış (Maliyet)</button>
                              <button type="button" onClick={() => setActiveInnerTabs(p => ({...p, [flight.id]: "collection"}))} className={`py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors ${tab === "collection" ? "border-emerald-600 text-emerald-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>Tahsilat</button>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] font-bold text-gray-400">{flight.type === 'departure' ? 'GİDİŞ' : 'DÖNÜŞ'} UÇUŞU {index + 1}</span>
                              <button type="button" onClick={() => removeFlight(flight.id)} className="text-gray-400 hover:text-red-500 transition-colors" title="Uçuşu Sil">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="p-4">
                            {tab === "sales" && (
                              <div className="flex flex-col lg:flex-row gap-3 items-end w-full lg:[&>*:nth-child(1)]:flex-[1] lg:[&>*:nth-child(2)]:flex-[1] lg:[&>*:nth-child(3)]:flex-[1.5] lg:[&>*:nth-child(4)]:flex-[1] lg:[&>*:nth-child(5)]:flex-[1.5] lg:[&>*:nth-child(6)]:flex-[1.2]">
                                <div>
                                ...
                                <div>
                                  ...
                                  <input type="text" placeholder="THY" value={flight.airline || ""} onChange={(e) => updateFlight(flight.id, "airline", e.target.value)} className="w-full h-[36px] px-3 border border-gray-200 rounded-md text-xs font-medium outline-none focus:border-emerald-500" />
                                </div>
                                <div>
                                ...
                                <div>
                                  ...
                                  <input type="text" placeholder="TK100" value={flight.flightNo || ""} onChange={(e) => updateFlight(flight.id, "flightNo", e.target.value)} className="w-full h-[36px] px-3 border border-gray-200 rounded-md text-xs font-medium outline-none focus:border-emerald-500" />
                                </div>
                                <div>
                                ...
                                <div>
                                  ...
                                  <div className="flex items-center gap-1">
                                    <input type="text" placeholder="0,00" value={servicePriceInput[`flight_${flight.id}`] !== undefined ? servicePriceInput[`flight_${flight.id}`] : flight.price ? flight.price.toString().replace(".", ",") : ""} onChange={(e) => setServicePriceInput((prev) => ({ ...prev, [`flight_${flight.id}`]: e.target.value }))} onBlur={(e) => { const parsed = parseAmount(e.target.value); if (parsed !== null) { updateFlight(flight.id, "price", parsed); setServicePriceInput((prev) => ({ ...prev, [`flight_${flight.id}`]: formatAmount(parsed) })); } }} className="flex-1 h-[36px] px-2 text-right border border-gray-200 rounded-md text-xs font-bold text-emerald-600 outline-none focus:border-emerald-500 bg-emerald-50/30" />
                                    <select value={flight.currency || "USD"} onChange={(e) => updateFlight(flight.id, "currency", e.target.value)} className="w-[60px] h-[36px] px-1 bg-gray-50 border border-gray-200 rounded-md text-xs font-bold outline-none">
                                      <option value="USD">USD</option><option value="EUR">EUR</option><option value="TRY">TRY</option><option value="GBP">GBP</option>
                                    </select>
                                  </div>
                                </div>
                            ...
                            {tab === "purchase" && (
                              <div className="flex flex-col lg:flex-row gap-3 items-end w-full lg:[&>*:nth-child(1)]:flex-[2] lg:[&>*:nth-child(2)]:flex-[1] lg:[&>*:nth-child(3)]:flex-[1.5] lg:[&>*:nth-child(4)]:flex-[1.5]">
                                <div>
                                  <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">TEDARİKÇİ (TICKETING)</label>
                                  <SearchableSelect options={suppliers.map(s => ({id: s.id, name: s.name}))} value={flight.ticketingProvider || ""} onChange={(val) => updateFlight(flight.id, "ticketingProvider", val)} placeholder="Tedarikçi Seçiniz..." />
                                </div>
                                <div>
                                  <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">PNR</label>
                                  <input type="text" placeholder="PNR" value={flight.pnr || ""} onChange={(e) => updateFlight(flight.id, "pnr", e.target.value)} className="w-full h-[36px] px-3 border border-gray-200 rounded-md text-xs font-medium outline-none focus:border-emerald-500" />
                                </div>
                                <div>
            ...
            <div className={`bg-white dark:bg-gray-900 rounded-xl shadow-sm mb-4 transition-all duration-300 ${expandedSection === "transfers" ? "border border-purple-200 dark:border-purple-800" : "border border-gray-200 dark:border-gray-800"}`}>
              <div className={`flex items-center p-4 cursor-pointer transition-colors ${expandedSection === 'transfers' ? 'bg-purple-50/30 dark:bg-purple-900/10 border-b border-purple-100 dark:border-purple-800/50' : 'hover:bg-gray-50 dark:hover:bg-gray-800'}`} onClick={() => setExpandedSection(expandedSection === 'transfers' ? null : 'transfers')}>
                  ...
                  {expandedSection !== 'transfers' && (
                    transfers.length > 0 ? transfers.map((t, i) => (
                      <div key={t.id} className="grid grid-cols-5 gap-4 items-center px-2 py-1">
                        <div className="col-span-2">
                          <p className="text-[9px] text-gray-400 mb-0.5 uppercase font-semibold">Tedarikçi</p>
                          <p className="text-[11px] font-semibold text-gray-700 dark:text-gray-300 truncate">
                            {t.provider ? suppliers.find((s) => s.id === t.provider)?.name : "Tedarikçi Seçilmedi"}
                          </p>
                        </div>
                        <div>
                          <p className="text-[9px] text-gray-400 mb-0.5 uppercase font-semibold">Tip & Yön</p>
                          <p className="text-[11px] font-semibold text-gray-700 dark:text-gray-300">{t.type === 'private' ? 'Özel' : 'Ekonomik'} - {t.direction === 'arrival' ? 'Geliş' : t.direction === 'return' ? 'Dönüş' : 'Ara'}</p>
                        </div>
                        <div>
                          <p className="text-[9px] text-gray-400 mb-0.5 uppercase font-semibold">Tarih</p>
                          <p className="text-[11px] font-semibold text-gray-700 dark:text-gray-300">
                ...
                <div className="flex items-center gap-2 text-gray-400 ml-4 shrink-0">
                  ...
                  <div className="relative group/menu">
                    <button type="button" onClick={(e) => e.stopPropagation()} className="p-1.5 hover:bg-gray-100 rounded-md transition-colors">
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 16a2 2 0 012 2 2 2 0 01-2 2 2 2 0 01-2-2 2 2 0 012-2m0-6a2 2 0 012 2 2 2 0 01-2 2 2 2 0 01-2-2 2 2 0 01-2-2 2 2 0 012-2z" /></svg>
                    </button>
                    <div className="absolute right-0 mt-1 w-32 bg-white border border-gray-100 rounded-lg shadow-lg opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all z-10">
                ...
                <div className="border-t border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-800/20 p-4">
                  ...
                  <div className="space-y-4">
                    {transfers.map((transfer, index) => {
                      const tab = activeInnerTabs[transfer.id] || "sales";
                      return (
                        <div key={transfer.id} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
                          {/* Inner Tabs Header */}
                          <div className="flex border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 px-4">
                            <div className="flex-1 flex gap-6">
                              <button type="button" onClick={() => setActiveInnerTabs(p => ({...p, [transfer.id]: "sales"}))} className={`py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors ${tab === "sales" ? "border-purple-600 text-purple-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>Satış Detayları</button>
                              <button type="button" onClick={() => setActiveInnerTabs(p => ({...p, [transfer.id]: "purchase"}))} className={`py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors ${tab === "purchase" ? "border-purple-600 text-purple-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>Alış (Maliyet)</button>
                              <button type="button" onClick={() => setActiveInnerTabs(p => ({...p, [transfer.id]: "collection"}))} className={`py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors ${tab === "collection" ? "border-purple-600 text-purple-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>Tahsilat</button>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] font-bold text-gray-400">{transfer.direction === 'arrival' ? 'GELİŞ' : transfer.direction === 'return' ? 'DÖNÜŞ' : 'ARA'} TRANSFER {index + 1}</span>
                              <button type="button" onClick={() => removeTransfer(transfer.id)} className="text-gray-400 hover:text-red-500 transition-colors" title="Transferi Sil">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="p-4">
                            {tab === "sales" && (
                              <div className="flex flex-col lg:flex-row gap-3 items-end w-full lg:[&>*:nth-child(1)]:flex-[1] lg:[&>*:nth-child(2)]:flex-[1] lg:[&>*:nth-child(3)]:flex-[1.5] lg:[&>*:nth-child(4)]:flex-[1] lg:[&>*:nth-child(5)]:flex-[1.2]">
                                <div>
                                ...
                                <div>
                                  ...
                                  <select value={transfer.type || ""} onChange={(e) => updateTransfer(transfer.id, "type", e.target.value)} className="w-full h-[36px] px-3 bg-white border border-gray-200 rounded-md text-xs font-medium focus:border-purple-500 outline-none">
                                ...
                                <div>
                                  ...
                                  <select value={transfer.vehicle || ""} onChange={(e) => updateTransfer(transfer.id, "vehicle", e.target.value)} className="w-full h-[36px] px-3 bg-white border border-gray-200 rounded-md text-xs font-medium focus:border-purple-500 outline-none">
                                    <option value="">Seçin</option>
                                    {vehicleTypes.map((t) => (
                                ...
                                <div>
                                  ...
                                  <input type="text" placeholder="Örn: Otel - Havalimanı" className="w-full h-[36px] px-3 border border-gray-200 rounded-md text-xs font-medium outline-none focus:border-purple-500" />
                                </div>
                                <div>
                                  <label className="block text-[10px] font-semibold text-purple-600 uppercase tracking-wider mb-1.5">SATIŞ TUTARI</label>
                                  <div className="flex items-center gap-1">
                                    <input type="text" placeholder="0,00" value={servicePriceInput[`transfer_${transfer.id}`] !== undefined ? servicePriceInput[`transfer_${transfer.id}`] : transfer.price ? transfer.price.toString().replace(".", ",") : ""} onChange={(e) => setServicePriceInput((prev) => ({ ...prev, [`transfer_${transfer.id}`]: e.target.value }))} onBlur={(e) => { const parsed = parseAmount(e.target.value); if (parsed !== null) { updateTransfer(transfer.id, "price", parsed); setServicePriceInput((prev) => ({ ...prev, [`transfer_${transfer.id}`]: formatAmount(parsed) })); } }} className="flex-1 h-[36px] px-2 text-right border border-gray-200 rounded-md text-xs font-bold text-purple-600 outline-none focus:border-purple-500 bg-purple-50/30" />
                                    <select value={transfer.currency || "USD"} onChange={(e) => updateTransfer(transfer.id, "currency", e.target.value)} className="w-[60px] h-[36px] px-1 bg-gray-50 border border-gray-200 rounded-md text-xs font-bold outline-none">
                                      <option value="USD">USD</option><option value="EUR">EUR</option><option value="TRY">TRY</option><option value="GBP">GBP</option>
                                    </select>
                                  </div>
                                </div>
                            ...
                            {tab === "purchase" && (
                              <div className="flex flex-col lg:flex-row gap-3 items-end w-full lg:[&>*:nth-child(1)]:flex-[2] lg:[&>*:nth-child(2)]:flex-[1.5]">
                                <div>
                                  <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">TEDARİKÇİ</label>
                                  <SearchableSelect options={suppliers.map(s => ({id: s.id, name: s.name}))} value={transfer.provider || ""} onChange={(val) => updateTransfer(transfer.id, "provider", val)} placeholder="Tedarikçi Seçiniz..." />
                                </div>
                                <div>
                                  <label className="block text-[10px] font-semibold text-gray-600 uppercase tracking-wider mb-1.5">ALIŞ (MALİYET) TUTARI</label>
                                  <div className="flex items-center gap-1">
                                    <input type="text" placeholder="0,00" value={serviceCostInput?.[`transfer_${transfer.id}`] !== undefined ? serviceCostInput[`transfer_${transfer.id}`] : transfer.costPrice ? transfer.costPrice.toString().replace(".", ",") : ""} onChange={(e) => setServiceCostInput((prev) => ({ ...prev, [`transfer_${transfer.id}`]: e.target.value }))} onBlur={(e) => { const parsed = parseAmount(e.target.value); if (parsed !== null) { updateTransfer(transfer.id, "costPrice", parsed); setServiceCostInput((prev) => ({ ...prev, [`transfer_${transfer.id}`]: formatAmount(parsed) })); } }} className="flex-1 h-[36px] px-2 text-right border border-gray-200 rounded-md text-xs font-bold text-gray-700 outline-none focus:border-gray-500" />
                                    <select value={transfer.costCurrency || "USD"} onChange={(e) => updateTransfer(transfer.id, "costCurrency", e.target.value)} className="w-[60px] h-[36px] px-1 bg-gray-50 border border-gray-200 rounded-md text-xs font-bold outline-none">
                                      <option value="USD">USD</option><option value="EUR">EUR</option><option value="TRY">TRY</option><option value="GBP">GBP</option>
                                    </select>
                                  </div>
                                </div>
                    ...
                    extraServices.length > 0 ? extraServices.map((e, i) => (
                      <div key={e.id} className="grid grid-cols-5 gap-4 items-center px-2 py-1">
                        <div className="col-span-2">
                          <p className="text-[9px] text-gray-400 mb-0.5 uppercase font-semibold">Tedarikçi</p>
                          <p className="text-[11px] font-semibold text-gray-700 dark:text-gray-300 truncate">
                            {e.provider ? suppliers.find((s) => s.id === e.provider)?.name : "Tedarikçi Seçilmedi"}
                          </p>
                        </div>
                        <div>
                ...
                <div className="border-t border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-800/20 p-4">
                  ...
                  <div className="space-y-4">
                    {extraServices.map((service, index) => {
                      const tab = activeInnerTabs[service.id] || "sales";
                      return (
                        <div key={service.id} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
                          {/* Inner Tabs Header */}
                          <div className="flex border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 px-4">
                            <div className="flex-1 flex gap-6">
                              <button type="button" onClick={() => setActiveInnerTabs(p => ({...p, [service.id]: "sales"}))} className={`py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors ${tab === "sales" ? "border-orange-600 text-orange-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>Satış Detayları</button>
                              <button type="button" onClick={() => setActiveInnerTabs(p => ({...p, [service.id]: "purchase"}))} className={`py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors ${tab === "purchase" ? "border-orange-600 text-orange-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>Alış (Maliyet)</button>
                              <button type="button" onClick={() => setActiveInnerTabs(p => ({...p, [service.id]: "collection"}))} className={`py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors ${tab === "collection" ? "border-orange-600 text-orange-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>Tahsilat</button>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] font-bold text-gray-400">EKSTRA HİZMET {index + 1}</span>
                              <button type="button" onClick={() => removeExtraService(service.id)} className="text-gray-400 hover:text-red-500 transition-colors" title="Hizmeti Sil">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="p-4">
                            {tab === "sales" && (
                              <div className="flex flex-col lg:flex-row gap-3 items-end w-full lg:[&>*:nth-child(1)]:flex-[1] lg:[&>*:nth-child(2)]:flex-[1.5] lg:[&>*:nth-child(3)]:flex-[2] lg:[&>*:nth-child(4)]:flex-[1.2]">
                                <div>
                                  <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">TARİH</label>
                                ...
                                <div>
                                  ...
                                  <select value={service.serviceType || ""} onChange={(e) => updateExtraService(service.id, "serviceType", e.target.value)} className="w-full h-[36px] px-3 bg-white border border-gray-200 rounded-md text-xs font-medium focus:border-orange-500 outline-none">
                                    <option value="">Seçin</option>
                                ...
                                <div>
                                  ...
                                  <input type="text" placeholder="Örn: Rehberlik" value={service.description || ""} onChange={(e) => updateExtraService(service.id, "description", e.target.value)} className="w-full h-[36px] px-3 border border-gray-200 rounded-md text-xs font-medium outline-none focus:border-orange-500" />
                                </div>
                                <div>
                                  <label className="block text-[10px] font-semibold text-orange-600 uppercase tracking-wider mb-1.5">SATIŞ TUTARI</label>
                                  <div className="flex items-center gap-1">
                                    <input type="text" placeholder="0,00" value={servicePriceInput[`extra_${service.id}`] !== undefined ? servicePriceInput[`extra_${service.id}`] : service.price ? service.price.toString().replace(".", ",") : ""} onChange={(e) => setServicePriceInput((prev) => ({ ...prev, [`extra_${service.id}`]: e.target.value }))} onBlur={(e) => { const parsed = parseAmount(e.target.value); if (parsed !== null) { updateExtraService(service.id, "price", parsed); setServicePriceInput((prev) => ({ ...prev, [`extra_${service.id}`]: formatAmount(parsed) })); } }} className="flex-1 h-[36px] px-2 text-right border border-gray-200 rounded-md text-xs font-bold text-orange-600 outline-none focus:border-orange-500 bg-orange-50/30" />
                                    <select value={service.currency || "USD"} onChange={(e) => updateExtraService(service.id, "currency", e.target.value)} className="w-[60px] h-[36px] px-1 bg-gray-50 border border-gray-200 rounded-md text-xs font-bold outline-none">
                                      <option value="USD">USD</option><option value="EUR">EUR</option><option value="TRY">TRY</option><option value="GBP">GBP</option>
                                    </select>
                                  </div>
                                </div>
                            ...
                            {tab === "purchase" && (
                              <div className="flex flex-col lg:flex-row gap-3 items-end w-full lg:[&>*:nth-child(1)]:flex-[2] lg:[&>*:nth-child(2)]:flex-[1.5]">
                                <div>
                                  <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">TEDARİKÇİ</label>
                                  <SearchableSelect options={suppliers.map(s => ({id: s.id, name: s.name}))} value={service.provider || ""} onChange={(val) => updateExtraService(service.id, "provider", val)} placeholder="Tedarikçi Seçiniz..." />
                                </div>
                                <div>
                                  <label className="block text-[10px] font-semibold text-gray-600 uppercase tracking-wider mb-1.5">ALIŞ (MALİYET) TUTARI</label>
                                  <div className="flex items-center gap-1">
                                    <input type="text" placeholder="0,00" value={serviceCostInput?.[`extra_${service.id}`] !== undefined ? serviceCostInput[`extra_${service.id}`] : service.costPrice ? service.costPrice.toString().replace(".", ",") : ""} onChange={(e) => setServiceCostInput((prev) => ({ ...prev, [`extra_${service.id}`]: e.target.value }))} onBlur={(e) => { const parsed = parseAmount(e.target.value); if (parsed !== null) { updateExtraService(service.id, "costPrice", parsed); setServiceCostInput((prev) => ({ ...prev, [`extra_${service.id}`]: formatAmount(parsed) })); } }} className="flex-1 h-[36px] px-2 text-right border border-gray-200 rounded-md text-xs font-bold text-gray-700 outline-none focus:border-gray-500" />
                                    <select value={service.costCurrency || "USD"} onChange={(e) => updateExtraService(service.id, "costCurrency", e.target.value)} className="w-[60px] h-[36px] px-1 bg-gray-50 border border-gray-200 rounded-md text-xs font-bold outline-none">
                                      <option value="USD">USD</option><option value="EUR">EUR</option><option value="TRY">TRY</option><option value="GBP">GBP</option>
                                    </select>
                                  </div>
                                </div>
