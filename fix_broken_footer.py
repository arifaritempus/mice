...
def fix_file(filename):
    ...
    # The broken footer starts at {/* TOTALS FOOTER */} and ends somewhere before the buttons
    # We want to replace from {/* TOTALS FOOTER */} up to the `İptal` button container
    
    # We will search for {/* TOTALS FOOTER */} until `<button type="button" onClick={() => router.push("/sejour/list")}`
    pattern = r'\{\/\* TOTALS FOOTER \*\/\}.*?(<button type="button" onClick=\{\(\) => router\.push\("/sejour/list"\)\})'
    ...
    text = re.sub(pattern, new_footer, text, flags=re.DOTALL)
