...
export default function ExpenseCompletedPage() {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, \`Tamamlanan_Gider_Faturalari_\${new Date().toISOString().split('T')[0]}.xlsx\`);
  };
    ...
    <div className="h-[calc(100vh-2rem)] w-full p-6 sm:p-8 flex flex-col gap-6 overflow-hidden font-sans text-white">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-4 shrink-0">
        ...
        <div className="shrink-0 mr-2">
          <h1 className="text-2xl font-light tracking-wide text-white glow-text">Tamamlanan Gider Faturaları</h1>
          <p className="text-xs text-slate-400 mt-1">Sisteme işlenmiş alış/gider faturalarının listesi</p>
        </div>
...
fs.writeFileSync('src/app/accounting/invoices/expense/completed/page.tsx', code, 'utf8');
