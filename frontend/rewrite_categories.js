...
// 3. Update isCategoryVisibleByStatsFilter to isCategoryVisible with search logic
const oldVisibleRegex = /const isCategoryVisibleByStatsFilter = \(category: Category\) => {[\s\S]*?return true;\n  };/;
const newVisibleFunc = `const isCategoryVisible = (category: Category) => {
    ...
    const matches = (s: string) => {
      ...
      return (
        ...
        (category.description && category.description.toLowerCase().includes(lowerS))
      );
    };
...
code = code.replace(oldVisibleRegex, newVisibleFunc);

// 4. Update isCategoryVisibleByStatsFilter references
code = code.replace(/isCategoryVisibleByStatsFilter/g, 'isCategoryVisible');
...
const layoutRegex = /<div className="h-full w-full p-6 sm:p-8 flex flex-col gap-6 overflow-y-auto custom-scrollbar font-sans text-white">[\s\S]*?(?=\{\/\* Categories List - Grouped by Main Categories \*\/)/;
      ...
      <div className="w-full min-w-0 flex-1 flex flex-col min-h-0">
        ...
        <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-4 mb-4 shrink-0">
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <div className="space-y-0.5">
              <h1 className="text-2xl font-light tracking-wide text-white glow-text">Kategori Yönetimi</h1>
              <p className="text-xs text-slate-400 mt-1">Ana kategorileri ve alt kategorileri yönetin</p>
            </div>
          </div>
          ...
          <div className="flex flex-row items-end justify-start xl:justify-end gap-3 flex-1 flex-wrap">
            ...
            <div className="flex flex-col gap-1.5 flex-[2] min-w-[250px] max-w-lg shrink-0">
              <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">GENEL ARAMA (KATEGORİ, KOD, AÇIKLAMA...)</label>
              <div className="h-10">
            ...
            <button onClick={handleExportExcel} disabled={exporting} className="h-10 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 py-2 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4 bg-[#0f172a]/40 backdrop-blur-md border border-white/10 rounded-xl p-2 shadow-sm shrink-0">
          ...
          <button onClick={() => setStatsFilter('all')} className={\`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all duration-200 flex items-center gap-2 \${statsFilter === 'all' ? 'bg-blue-500/20 border border-blue-500/30 text-blue-300' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent'}\`}>
            TÜMÜ
            <span className={\`px-1.5 py-0.5 rounded-md text-[9px] \${statsFilter === 'all' ? 'bg-blue-500/20 text-blue-300' : 'bg-white/10'}\`}>{categories.length}</span>
          </button>
          <button onClick={() => setStatsFilter('main')} className={\`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all duration-200 flex items-center gap-2 \${statsFilter === 'main' ? 'bg-emerald-500/20 border border-emerald-500/30 text-emerald-300' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent'}\`}>
            ANA KATEGORİ
            <span className={\`px-1.5 py-0.5 rounded-md text-[9px] \${statsFilter === 'main' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-white/10'}\`}>{categories.filter(c => !c.parent_id).length}</span>
          </button>
          <button onClick={() => setStatsFilter('sub')} className={\`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all duration-200 flex items-center gap-2 \${statsFilter === 'sub' ? 'bg-purple-500/20 border border-purple-500/30 text-purple-300' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent'}\`}>
            ALT KATEGORİ
            <span className={\`px-1.5 py-0.5 rounded-md text-[9px] \${statsFilter === 'sub' ? 'bg-purple-500/20 text-purple-300' : 'bg-white/10'}\`}>{categories.filter(c => c.parent_id).length}</span>
          </button>
          <button onClick={() => setStatsFilter('active')} className={\`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all duration-200 flex items-center gap-2 \${statsFilter === 'active' ? 'bg-orange-500/20 border border-orange-500/30 text-orange-300' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent'}\`}>
            AKTİF
            <span className={\`px-1.5 py-0.5 rounded-md text-[9px] \${statsFilter === 'active' ? 'bg-orange-500/20 text-orange-300' : 'bg-white/10'}\`}>{categories.filter(c => c.is_active).length}</span>
          </button>
        </div>
...
// Replace Table Row Hover effect inside the Categories list group
code = code.replace(
  /className="bg-white\/5 border-l-4 border-blue-500\/50 px-4 py-3 group hover:bg-white\/10 transition-colors"/g,
  'className="bg-white/5 border-l-4 border-blue-500/50 px-4 py-3 group hover:bg-blue-500/10 cursor-pointer transition-colors" onDoubleClick={() => { setSelectedCategory(mainCategory); setShowEditModal(true); }}'
);

code = code.replace(
  /className="px-4 py-3 border-b border-white\/5 group hover:bg-white\/5 transition-colors"/g,
  'className="px-4 py-3 border-b border-white/5 group hover:bg-blue-500/10 cursor-pointer transition-colors" onDoubleClick={() => { setSelectedCategory(subCategory); setShowEditModal(true); }}'
);
