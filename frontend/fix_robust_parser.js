...
const parseReplacement = `
        ...
        if (dataRaw[0][0] === "KATILIMCI BİLGİLERİ") {
          ...
          for (let i = 0; i < Math.min(10, dataRaw.length); i++) {
             ...
             if (strRow.some((c: string) => c.includes("ad") || c.includes("isim") || c.includes("soyad") || c.includes("katılımcı"))) {
                headerRowIndex = i;
                break;
             }
          }
          ...
          formattedData = dataRows.map((rawRow: any) => {
            ...
            let fName = row["ad"] || row["adı"] || row["first_name"] || row["isim"] || row["i̇sim"] || row["name"] || "";
          ...
          }).filter((r: any) => r && r.first_name); // allow just first name if last name is missing
        }
`;
...
if (pStartIdx !== -1 && pEndIdx !== -1) {
    ...
    console.log("Successfully replaced firmly!");
} else {
