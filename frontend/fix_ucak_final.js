...
const target1 = `            if (row.ucus_parkuru) {
...
const idx2 = code.indexOf(`            if (row.donus_ucus_parkuru) {`);
const endIdx = code.indexOf(`        if (row.transfer_ucreti || row.transfer_guzergah || row.donus_transfer_ucreti || row.donus_transfer_guzergah) {`);
...
if (idx1 !== -1 && endIdx !== -1) {
    ...
    console.log("Fixed UCAK logic");
} else {
    console.log("NOT FOUND", idx1, endIdx);
}
