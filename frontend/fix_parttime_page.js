...
const oldHeaderBlockRegex = /{\/\* Header \*\/}[\s\S]*?(?=<div className={`bg-white dark:bg-gray-800 rounded-lg shadow-sm transition-colors duration-200 w-full min-w-0 flex-1 flex flex-col min-h-0 relative \${tableBusy \? 'opacity-80' : ''}`})/g;

const newHeaderBlock = `{/* Unified Header */}
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-2">
          {/* Left: Title */}
          <div className="shrink-0 mr-4">
            <h1 className="text-2xl font-light tracking-wide text-white glow-text">Yarı Zamanlı Çalışan Yönetimi</h1>
            <p className="text-xs text-slate-400 mt-1">MICE ve Sejour part-time operasyonlarını tek ekrandan yönetin</p>
          </div>
...
// 3. Table Wrappers
content = content.replace(
  /className={`bg-white dark:bg-gray-800 rounded-lg shadow-sm transition-colors duration-200 w-full min-w-0 flex-1 flex flex-col min-h-0 relative \${tableBusy \? 'opacity-80' : ''}`}/g,
...
content = content.replace(/px-3 py-2 text-xs text-gray-500 dark:text-gray-400/g, 'px-2.5 py-2.5 text-[11px] text-slate-300');
...
fs.writeFileSync('src/app/operations/part-time/page.tsx', content, 'utf8');

console.log("Part-time page modernized successfully.");
