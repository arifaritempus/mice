...
export function useTransferState() {
  ...
  const [showVehicleAssignmentModal, setShowVehicleAssignmentModal] =
    useState(false);
  ...
  const [currentTransferIndex, setCurrentTransferIndex] = useState<
    number | null
  >(null);
  const [transferSearch, setTransferSearch] = useState<string>("");
  const [transferTotals, setTransferTotals] = useState<{
