...
export default function AccommodationTabSimple({
  ...
  const simpleStats = useMemo(() => {
    ...
    const roomTypes = [...new Set(accommodationItems.map(item => item.oda_tipi).filter(Boolean))];
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{simpleStats.totalRooms}</div>
                <div className="text-sm text-gray-600 dark:text-gray-300">Toplam Oda</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-green-600 dark:text-green-400">{simpleStats.totalPax}</div>
                <div className="text-sm text-gray-600 dark:text-gray-300">Toplam Pax</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">{simpleStats.roomTypes.length}</div>
                <div className="text-sm text-gray-600 dark:text-gray-300">Oda Tipi</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Konaklama Listesi</h3>
              <input
                type="text"
                  ...
                  <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    <div className="flex-1">
                      <div className="flex items-center gap-4 text-sm">
                        <span className="font-medium">{item.isim} {item.soyisim}</span>
                        <span className="text-gray-600 dark:text-gray-300">{item.oda_tipi}</span>
                        <span className="text-gray-600 dark:text-gray-300">{item.gelis_tarihi} - {item.cikis_tarihi}</span>
                        <span className="text-gray-600 dark:text-gray-300">{item.otel}</span>
                        <span className="text-gray-600 dark:text-gray-300">{item.toplam} {item.doviz}</span>
                      </div>
                    </div>
