...
interface TransferTurTabProps {
  ...
  setSelectedTransfers: (
    transfers: string[] | ((prev: string[]) => string[]),
  ) => void;
  ...
  transferTotals: {
    [key: string]: { kisiSayisi: number; toplamMaliyet: number };
  };
...
export default function TransferTurTab(props: TransferTurTabProps) {
  ...
  const supplierTotalsCalc = useMemo(() => {
    const totals: { [supplierName: string]: { [currency: string]: number } } = {};
                    ...
                    <input
                      ...
                      onChange={() => {
                        ...
                        if (allFilteredSelected) {
                          ...
                          const newSelections = [
                            ...new Set([...selectedTransfers, ...filteredIds]),
                          ];
                  ...
                  <div className="flex items-center justify-between relative">
                    ...
                    <div className="relative">
                      ...
                      {showAddTransferMenu &&
                        ...
                        createPortal(
                          <div
                            id="manual-transfer-menu"
                            className="fixed bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-[9999] py-1 w-44"
                            style={{
              ...
              {filteredTransfers.flatMap((transfer) => {
                const elements: any[] = [
                  <tr
                    ...
                    tabIndex={transfer.isEditing ? 0 : -1}
                    onKeyDown={(e) => handleTransferRowKeyDown(e, transfer.id)}
                    className={`hover:bg-blue-500/10 transition-colors group cursor-pointer border-b border-white/5 last:border-0 ${transfer.isEditing ? "bg-blue-500/10 dark:bg-blue-900/20" : ""}`}
                    onDoubleClick={() => {
                  ...
                  >
                    ...
                    <td className="px-2.5 py-2.5">
                      {transfer.typeLabel === "Ara Transfer" ? (
                        <span
                          className={`px-2 py-1 rounded-md text-xs font-medium inline-block bg-amber-100 dark:bg-amber-900 text-amber-800 dark:text-amber-200 border border-amber-200 dark:border-amber-700`}
                        >
                          Ara Transfer
                        </span>
                      ) : transfer.isGroup ? (
                        ...
                        <span
                          className={`px-2 py-1 rounded-md text-xs font-medium inline-block ${
                            transfer.direction === "arrival"
                              ? "bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 border border-green-200 dark:border-green-700"
                              : "bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 border border-blue-200 dark:border-gray-700"
                          }`}
                        >
                    ...
                    <td className="px-2.5 py-2.5 text-gray-900 dark:text-white">
                      {transfer.isEditing ? (
                        <input
                          ...
                          value={transfer.date || ""}
                          onChange={(e) =>
                            updateTransfer(transfer.id, "date", e.target.value)
                          }
                          onKeyDown={(e) => {
                            if (e.key === "Enter") {
                              e.preventDefault();
                    ...
                    <td className="px-2.5 py-2.5 text-gray-900 dark:text-white">
                      {transfer.isEditing ? (
                        <input
                          ...
                          value={transfer.time || ""}
                          onChange={(e) =>
                            updateTransfer(transfer.id, "time", e.target.value)
                          }
                          onKeyDown={(e) => {
                            if (e.key === "Enter") {
                              e.preventDefault();
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateTransfer(
                              transfer.id,
                              "passengerCount",
                              parseInt(formatIntegerForInput(e.target.value)) ||
                                1,
                            )
                          }
                          onKeyDown={(e) => {
                        ...
                        <input
                          ...
                          value={
                            supplierDropdowns[transfer.id]?.isOpen
                              ? (supplierDropdowns[transfer.id]?.searchTerm ??
                                "")
                              ...
                              {allSuppliers
                                ...
                                .map((item: any, itemIndex: number) => (
                                  <div
                                    ...
                                    className={`px-2.5 py-2.5 text-xs cursor-pointer transition-colors duration-150 flex items-center justify-between ${
                                      itemIndex ===
                                      supplierDropdowns[transfer.id]
                                        ?.selectedIndex
                    ...
                    <td className="px-2.5 py-2.5">
                      <input
                        ...
                        inputMode="decimal"
                        value={
                          transferCostInput[transfer.id] ??
                          (transfer.costAmount
                            ? formatNumberForDisplay(transfer.costAmount)
                            : "")
                        }
                        onChange={(e) => {
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            ...
                            setTransferCostInput((prev) => ({
                              ...prev,
                              [transfer.id]: transfer.costAmount
                                ? formatNumberForDisplay(transfer.costAmount)
                                : "",
                            }));
                          }
                    ...
                    <td className="px-2.5 py-2.5 w-48 max-w-[12rem] overflow-hidden text-gray-900 dark:text-white">
                      {transfer.isEditing ? (
                        <input
                          type="text"
                          value={
                            (transfer as any).passengersInput ??
                        ...
                        <div
                          ...
                          title={transfer.passengers.join(", ")}
                        >
                          {transfer.passengers.join(", ")}
                        </div>
                      )}
                        ...
                        {transfer.isEditing ? (
                          ...
                          <>
                            {!transfer.vehicleAssigned && (
                              <button
                                onClick={() =>
                                  openVehicleAssignmentModal(transfer.id)
                                }
                      ...
                      <td colSpan={12} className="px-2.5 py-2.5">
                        <div className="bg-white dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
                          ...
                          <div className="space-y-2">
                            {transfer.originalTransfers?.map(
                              (originalTransfer: any, detailIndex: number) => {
                                const hotelName = (() => {
                                  if (!originalTransfer.hotel_id)
                                    return "Genel";
                                  const tabHotel = project?.hotels_data?.find(
                                    (h: any) =>
                                      h.id === originalTransfer.hotel_id,
                                  );
                                  if (tabHotel) {
                                    if (tabHotel.hotel_name)
                                      return tabHotel.hotel_name;
                                    const masterHotel = hotels?.find(
                                      (h: any) => h.id === tabHotel.hotel_id,
                                    );
                                    if (masterHotel) return masterHotel.name;
                                ...
                                return (
                                  <div
                                    key={detailIndex}
                                    className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-600 rounded border"
                                  >
                                    <div className="flex items-center gap-4 text-xs">
                                      <span
                                        className={`px-2 py-1 rounded text-xs font-medium ${
                                          originalTransfer.direction ===
                                          "arrival"
              ...
              {transfers.length > 0 &&
                Object.keys(transferTotals)
                  ...
                  .map((doviz, index) => (
                    ...
                    >
                      <td
                        className="px-2.5 py-2.5 text-gray-900 dark:text-white"
                        colSpan={8}
                      >
                        TOPLAM ({doviz})
                      </td>
        ...
        <div className="group relative overflow-hidden rounded-xl bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 border border-purple-200 dark:border-purple-700/50 p-4 shadow-sm hover:shadow-md transition-all duration-200">
          ...
          <div className="relative">
            ...
            <div className="space-y-2 mt-4 max-h-[160px] overflow-y-auto pr-1">
              {Object.keys(supplierTotalsCalc).length > 0 ? (
                Object.entries(supplierTotalsCalc)
                  .sort(([a], [b]) => a.localeCompare(b))
                  .map(([supplierName, currencies]) => (
                    <div
                      key={supplierName}
                      className="flex flex-col text-xs bg-white/50 dark:bg-purple-900/20 rounded-lg px-2 py-1.5"
                    >
                      <span className="text-purple-800 dark:text-purple-200 font-semibold mb-1">
                        {supplierName}
                      </span>
                      {Object.entries(currencies).map(([curr, amount]) => (
                        <div key={curr} className="flex justify-between items-center ml-2 border-t border-purple-200/30 dark:border-purple-700/30 pt-0.5 mt-0.5 first:border-0 first:pt-0 first:mt-0">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 responsive-filter-grid">
            {Object.entries(stats?.totals?.byVehicle || {})
              ...
              .map(([veh, v]: any) => (
                <div
                  key={veh}
                  className="bg-white/60 dark:bg-gray-800/60 rounded-lg px-2.5 py-2.5 text-center"
                >
        ...
        <div className="bg-blue-500/10 dark:bg-blue-900/20 rounded-lg p-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-sm font-medium border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
            >
              Seçimi Temizle
            </button>
          </div>
