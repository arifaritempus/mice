...
export default function FinancialTab({
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  <>
                      ...
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            ...
                            if (showHotelSupplierDropdown && selectedSupplierIndex >= 0 && selectedSupplierIndex < filteredHotelSuppliers.length) {
                              ...
                              const updatedTempFinancialServiceItem = {
                                ...tempFinancialServiceItem,
                                supplier: currentInputValue || tempFinancialServiceItem?.supplier || '',
                                hotel: currentInputValue || tempFinancialServiceItem?.hotel || ''
                              };
                              setTempFinancialServiceItem(updatedTempFinancialServiceItem);
                              handleFinancialSave();
                            }
                    ...
                    <td className="px-2 py-2">
                      <div className="flex gap-1">
                        <button onClick={handleFinancialSave} className="p-1 rounded text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/30" title="Kaydet">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                        </button>
                        <button onClick={handleFinancialCancel} className="p-1 rounded text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-900/30" title="İptal">
              ...
              {filteredFinancialServices.length === 0 && editingFinancialServiceIndex === null ? (
                <tr>
                  <td colSpan={10} className="px-2 py-8 text-center text-gray-500 dark:text-gray-400">Henüz finansal servis eklenmemiş</td>
                </tr>
              ) : (
                filteredFinancialServices
                  .map((item, index) => {
                    // Orijinal index'i bul (filteredFinancialServices'deki index değil, financials'deki index)
                    const originalIndex = financialServices.findIndex(os => os.id === item.id);
                    return (
                    <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      {editingFinancialServiceIndex === originalIndex ? (
                        <>
                          ...
                          <td className="px-2 py-2">
                            <input type="text" value={financialAmountInput} onChange={(e) => {
                              ...
                              const rate = parseTurkishNumber(tempFinancialServiceItem?.exchangeRate || '1');
                              // Tutar x Kur = Toplam TL
                              const total = amount * rate;
                              if (!isEditingFinancialTotal) {
                                setFinancialTotalTRYInput(formatNumberForDisplay(total));
                              }
                              setTempFinancialServiceItem(prev => ({ ...prev, amount, totalTRY: total, exchangeRate: rate, fx: rate }));
                          ...
                          <td className="px-2 py-2">
                            <select value={tempFinancialServiceItem?.currency || 'TRY'} onChange={(e) => {
                              const newCur = e.target.value;
                              const amount = parseTurkishNumber(financialAmountInput || '0');
                              const rate = parseTurkishNumber(tempFinancialServiceItem?.exchangeRate || '1');
                          ...
                          <td className="px-2 py-2">
                            <input type="text" value={tempFinancialServiceItem?.exchangeRate || ''} onChange={(e) => {
                              const input = e.target.value;
                              // exchangeRate is managed in tempFinancialServiceItem(input);
                              const rate = parseTurkishNumber(input || '1');
                              const amountInput = parseTurkishNumber(financialAmountInput || '0');
                              ...
                              setTempFinancialServiceItem(prev => ({ ...prev, exchangeRate: rate, fx: rate, amount: amountInput, totalTRY: total }));
                            }} onBlur={(e) => {
                          ...
                          <td className="px-2 py-2">
                          <input type="text" value={financialTotalTRYInput} onFocus={() => setIsEditingFinancialTotal(true)} onChange={(e) => {
                              const input = e.target.value;
                              setFinancialTotalTRYInput(input);
                            }} onBlur={(e) => {
                              setIsEditingFinancialTotal(false);
                              ...
                              const newAmount = rate > 0 ? tl / rate : 0;
                              setFinancialAmountInput(formatNumberForDisplay(newAmount));
                              setTempFinancialServiceItem(prev => ({ ...prev, totalTRY: tl, amount: newAmount, exchangeRate: rate, fx: rate }));
                              setFinancialTotalTRYInput(formatNumberForDisplay(tl));
                            }} className="w-full px-1 py-0.5 border border-gray-300 dark:border-gray-600 rounded text-xs dark:bg-gray-700 dark:text-white text-right" placeholder="0,00" inputMode="decimal" />
                          </td>
                          <td className="px-2 py-2">
                            <div className="flex gap-1">
                        ...
                        <>
                          ...
                          <td className="px-2 py-2">
                            <div className="flex gap-1">
                              <button onClick={() => handleFinancialEdit(originalIndex)} className="p-1 rounded text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30" title="Düzenle"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                              <button onClick={() => handleFinancialDelete(originalIndex)} className="p-1 rounded text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30" title="Sil"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                            </div>
                          </td>
                        </>
                      )}
        ...
        <div className="bg-blue-600 dark:bg-blue-700 rounded-md p-3">
          <div className="grid grid-cols-12 gap-2 text-white text-sm">
            ...
            <div className="col-span-3 text-right font-bold">
              {formatNumber(financialServices.reduce((sum: number, item: any) => sum + (parseFloat(item.totalTRY || item.total_try || 0) || 0), 0))} TL
            </div>
      ...
      {showHotelSupplierDropdown && createPortal(
        <div
