...
export default function EtkinlikAktiviteTab(props: EtkinlikAktiviteTabProps) {
  ...
  const [eventSupplierDropdownPosition, setEventSupplierDropdownPosition] =
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-3 py-1 bg-blue-500 text-white text-xs rounded hover:bg-blue-500/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Yeni Ekle
          </button>
          {handleEventActivityExport && (
          ...
          <table className="w-full text-xs">
            <thead className="bg-gray-100 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-2 py-2 text-left font-semibold text-gray-900 dark:text-white cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-600 select-none">
                  OTEL/TEDARİKÇİ
                </th>
            ...
            <tbody>
                ...
                >
                              ...
                              {allSuppliers
                                .filter((s) =>
                                  (s.displayName || s.name || "")
                                    .toLowerCase()
                                    .includes(
                                      eventSupplierSearch?.toLowerCase() || "",
                                    ),
                                )
                                .map((s, idx) => (
                                  <div
                                    ...
                                    onMouseEnter={() =>
                                      setSelectedEventSupplierIndex(idx)
                                    }
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setTempEventItem({
                          ...
                          amount,
                          total_tl:
                            amount * (tempEventItem.exchange_rate || 1.0),
                        });
                      }}
                  ...
                  <td className="px-2 py-2 text-gray-900 dark:text-white">
                    <input
                      type="number"
                      step="0.0001"
                      value={tempEventItem?.exchange_rate || 1.0}
                      onChange={(e) => {
                  ...
                  <td className="px-2 py-2 text-gray-900 dark:text-white">
                    <input
                      ...
                      step="0.01"
                      value={
                        tempEventItem?.total_tl ||
                        tempEventItem.amount * tempEventItem.exchange_rate ||
                        0
                      }
                      onChange={(e) => {
                        const totalTL = parseFloat(e.target.value) || 0;
                        const rate = tempEventItem.exchange_rate || 1.0;
                        setTempEventItem({
                          ...tempEventItem,
                          total_tl: totalTL,
                          amount: totalTL / rate,
              ...
              {sortedEvents.map((event, index) => {
                const originalIndex = eventsActivities.findIndex(
                  (x: any) => x.id === event.id,
                );
                  ...
                  >
                      ...
                      {isEditing ? (
                        <div className="relative">
                          <input
                            ...
                            onKeyDown={(e) =>
                              handleEventSupplierKeyDown(e, displayEvent.id)
                            }
                            className="w-full px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                          />
                          {showEventSupplierDropdown &&
                        ...
                        <span>
                          {displayEvent.supplier?.name ||
                            ...
                            (displayEvent.hotel_id &&
                              hotels.find(
                                (h: any) => h.id === displayEvent.hotel_id,
                              )?.name) ||
                            (displayEvent.supplier_id &&
                              suppliers.find(
                                (s: any) => s.id === displayEvent.supplier_id,
                              )?.name) ||
                            "-"}
                        </span>
                      )}
                      ...
                      {isEditing ? (
                        ...
                        <span>
                          {displayEvent.sub_category?.name ||
                            (displayEvent.sub_category_id &&
                              eventSubCategories.find(
                                (cat: any) =>
                                  cat.id === displayEvent.sub_category_id,
                              )?.name) ||
                            "-"}
                        </span>
                      )}
                    ...
                    <td className="px-2 py-2 text-gray-900 dark:text-white">
                      {isEditing ? (
                        ...
                        <span>
                          {formatNumberForDisplay(displayEvent.amount || 0)}
                        </span>
                      )}
                    </td>
                    <td className="px-2 py-2 text-gray-900 dark:text-white">
                    ...
                    <td className="px-2 py-2 text-gray-900 dark:text-white text-right">
                      {isEditing ? (
                        <input
                          type="number"
                          step="0.0001"
                          value={displayEvent.exchange_rate || 1.0}
                          onChange={(e) => {
                            const rate = parseFloat(e.target.value) || 1.0;
                            setTempEventItem({
                              ...tempEventItem,
                        ...
                        <span>
                          {formatNumberForDisplay(
                            displayEvent.exchange_rate || 1.0,
                          )}
                        </span>
                      )}
                    </td>
                        ...
                        {isEditing ? (
                          ...
                          <>
                            <button
                              ...
                              className="p-1 text-blue-600 hover:bg-blue-500/10 dark:hover:bg-blue-900/20 rounded disabled:opacity-50 disabled:cursor-not-allowed"
                              title="Düzenle"
                            >
                              <svg
                                className="w-4 h-4"
              ...
              {eventsActivities.length === 0 && (
                <tr>
                  ...
                  >
                    Henüz etkinlik eklenmemiş. "Yeni Ekle" butonunu kullanarak
        ...
        <div className="bg-blue-500 dark:bg-blue-700 rounded-md p-3">
          <div className="grid grid-cols-12 gap-2 text-white text-sm responsive-filter-grid">
            ...
            <div className="col-span-3 text-right font-bold">
              {formatNumber(
                eventsActivities.reduce(
                  (sum: number, item: any) =>
                    sum + (parseFloat(item.total_tl || 0) || 0),
                  0,
                ),
              )}{" "}
              TL
