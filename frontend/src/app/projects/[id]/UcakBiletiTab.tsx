...
export default function UcakBiletiTab({
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-3 py-1 bg-blue-500 text-white text-xs rounded hover:bg-blue-500/90 transition-colors cursor-pointer"
            type="button"
          >
            Yeni Bilet Ekle
          </button>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-xs">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {sortedFlightTickets.length === 0 &&
              !(
                editingFlightIndex === flightTickets.length &&
                tempFlightItem &&
                isNewFlightItem
              ) ? (
                <tr>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              const kisiSayisi =
                                tempFlightItem?.kisiSayisi || 0;
                              const ppMaliyet =
                                kisiSayisi > 0 ? toplamMaliyet / kisiSayisi : 0;
                              const kur = tempFlightItem?.kur || 1;
                              const doviz = tempFlightItem?.doviz || "TL";
                              const toplamTl =
                                doviz === "TL"
                        ...
                        <td className="px-2 py-2">
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              const kur = tempFlightItem?.kur || 1;
                              const toplamMaliyet =
                                tempFlightItem?.toplamMaliyet || 0;
                              const toplamTl =
                                doviz === "TL"
                                  ? toplamMaliyet
                                  : toplamMaliyet * kur;
      ...
      {tempFlightItem?.id &&
        supplierDropdowns[tempFlightItem.id]?.isOpen &&
        dropdownPosition &&
        createPortal(
          <div
            ...
            {allSuppliers
              .filter((item: any) =>
                (item.displayName || item.name || item.title || "")
                  ...
                  .includes(
                    (
                      supplierDropdowns[tempFlightItem.id]?.searchTerm || ""
                    ).toLowerCase(),
                  ),
              )
              .map((item: any, itemIndex: number) => (
                <div
                  ...
                  className={`px-3 py-2 text-xs cursor-pointer transition-colors duration-150 flex items-center justify-between ${
                    itemIndex ===
                    supplierDropdowns[tempFlightItem.id]?.selectedIndex
