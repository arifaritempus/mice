  ...
  return (
    <div className="relative" ref={wrapperRef}>
      <input
        ...
        autoFocus={autoFocus}
        className="w-full px-1.5 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 truncate"
      />
      {isOpen && filteredOptions.length > 0 && (
        <div className="absolute z-50 mt-1 w-full min-w-[200px] max-h-48 overflow-y-auto bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-lg">
          {filteredOptions.map((opt, idx) => (
            <div
              key={opt.id || opt.label}
              className={`px-2 py-1.5 text-xs cursor-pointer flex justify-between items-center ${
                highlightedIndex === idx ? 'bg-indigo-50 dark:bg-indigo-900/30' : 'hover:bg-gray-100 dark:hover:bg-gray-700'
              }`}
...
export default function DigerTab(props: DigerTabProps) {
  ...
  const contactOptions = useMemo(() => {
    const s = (suppliers || []).map(sup => ({ id: sup.id, label: sup.name, subLabel: 'Ted' }));
    const h = (hotels || []).map(hot => ({ id: hot.id, label: hot.name, subLabel: 'Otel' }));
  ...
  const mainCategoryOptions = useMemo(() => {
    return (categories || [])
      ...
      .map(c => ({ id: c.id, label: c.name, order: c.sort_order || 9999 }))
    ...
    try {
      ...
      try {
        ...
        if (wordmarkLogoBase64) {
          ...
          sheet.addImage(markId, { tl: { col: 8.5, row: 0.23 }, ext: { width: inchToPx(2.4), height: inchToPx(0.55) } as any } as any);
        }
      } catch(e) {
      ...
      sheet.columns = [
        ...
        { header: 'Otel/Tedarikçi', key: 'contact', width: 25 },
        { header: 'Ana Kategori', key: 'main_cat', width: 20 },
        { header: 'Alt Kategori', key: 'sub_cat', width: 20 },
        { header: 'Açıklama', key: 'desc', width: 35 },
        { header: 'Maliyet', key: 'cost_amount', width: 15 },
        { header: 'M.Döviz', key: 'cost_cur', width: 10 },
        { header: 'M.Top (TL)', key: 'cost_try', width: 15 },
        { header: 'Satış', key: 'sale_amount', width: 15 },
        { header: 'S.Döviz', key: 'sale_cur', width: 10 },
        { header: 'S.Top (TL)', key: 'sale_try', width: 15 }
      ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-center justify-between bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700">
          ...
          <div className="relative flex flex-wrap items-center gap-2 px-2 py-1.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-600 rounded-lg focus-within:ring-2 focus-within:ring-indigo-500/50 focus-within:border-indigo-500 transition-all">
            <div className="flex flex-wrap gap-1.5 pl-2 flex-1 items-center">
              {searchTerms.map(term => (
                <span key={term} className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-xs font-medium">
                  {term}
              ...
              <input
                ...
                placeholder={searchTerms.length === 0 ? "Arama yap... (Enter'a basarak ekle)" : "Arama ekle..."}
                className="flex-1 bg-transparent border-none outline-none text-sm text-gray-800 dark:text-gray-200 min-w-[120px] py-0.5 focus:ring-0 placeholder:text-gray-400"
              />
            </div>
            {searchTerms.length > 0 && (
              <button onClick={() => setSearchTerms([])} className="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 pr-1">
                Temizle
              </button>
            )}
          </div>
        ...
        <div className="flex gap-2 whitespace-nowrap">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 text-white text-xs font-medium rounded hover:bg-emerald-700 transition-colors shadow-sm"
          >
              ...
              {filteredOthers.length === 0 ? (
                ...
                filteredOthers.map((item) => {
                  ...
                  return (
                    <tr 
                      key={item.id} 
                      onDoubleClick={() => !isLocked && !isEditing && startEditing(item)}
                      className={`group transition-colors ${
                        isEditing 
                          ? "bg-indigo-50/40 dark:bg-indigo-900/10 ring-1 ring-inset ring-indigo-500/20" 
                    ...
                    >
                      ...
                      <td className="px-2 py-1.5 align-middle">
                        {isEditing ? (
                          <input
                            ...
                            onChange={(e) => setDraftItem({...draftItem, date: e.target.value})}
                            onKeyDown={handleGlobalKeyDown}
                            autoFocus
                            className="w-full px-1.5 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500"
                          />
                        ) : (
                      ...
                      <td className="px-2 py-1.5 align-middle">
                        {isEditing ? (
                          <AutocompleteInput
                            ...
                            onKeyDownOuter={handleGlobalKeyDown}
                            onSelect={(id, label) => setDraftItem({...draftItem, contact_id: id, contact_name: label})}
                          />
                        ) : (
                          <span className="font-medium text-indigo-700 dark:text-indigo-300 truncate block max-w-[180px]" title={currentItem.contact_name || '-'}>
                      ...
                      <td className="px-2 py-1.5 align-middle">
                        {isEditing ? (
                          <AutocompleteInput
                            ...
                            onKeyDownOuter={handleGlobalKeyDown}
                            onSelect={(id, label) => setDraftItem({...draftItem, category_name: label, sub_category_name: ""})}
                          />
                        ) : (
                          <span className="text-gray-700 dark:text-gray-300 text-xs block truncate max-w-[120px]" title={currentItem.category_name || '-'}>
                      ...
                      <td className="px-2 py-1.5 align-middle">
                        {isEditing ? (
                          <AutocompleteInput
                            value={currentItem.sub_category_name || ""}
                            options={getSubCategoryOptions(currentItem.category_name)}
                            placeholder="Alt Kategori"
                            onKeyDownOuter={handleGlobalKeyDown}
                            onSelect={(id, label) => setDraftItem({...draftItem, sub_category_name: label})}
                          />
                        ) : (
                          <span className="text-gray-600 dark:text-gray-400 text-xs block truncate max-w-[120px]" title={currentItem.sub_category_name || '-'}>
                      ...
                      <td className="px-2 py-1.5 align-middle">
                        {isEditing ? (
                          <input
                            ...
                            onKeyDown={handleGlobalKeyDown}
                            className="w-full px-1.5 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500 truncate"
                            title={currentItem.description || ""}
                            placeholder="Açıklama"
                          />
                        ) : (
                          <span className="text-gray-600 dark:text-gray-400 block max-w-[200px] truncate" title={currentItem.description}>
                      ...
                      <td className="px-2 py-1.5 align-middle text-right">
                        {isEditing ? (
                          <input
                            type="text"
                            value={currentItem.cost_amount ?? ""}
                            onChange={(e) => setDraftItem({...draftItem, cost_amount: cleanInputValue(e.target.value)})}
                            onKeyDown={handleGlobalKeyDown}
                            className="w-16 text-right px-1.5 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500"
                          />
                        ) : (
                      ...
                      <td className="px-2 py-1.5 align-middle">
                        {isEditing ? (
                          <select
                            value={currentItem.cost_currency || "EUR"}
                            onChange={(e) => setDraftItem({...draftItem, cost_currency: e.target.value})}
                            onKeyDown={handleGlobalKeyDown}
                            className="w-full px-1 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500"
                          >
                            {CURRENCIES.map(c => <option key={c} value={c}>{c}</option>)}
                          </select>
                        ) : (
                          <span className="text-gray-700 dark:text-gray-300 font-medium">{currentItem.cost_currency || "EUR"}</span>
                        )}
                      </td>
                      ...
                      <td className="px-2 py-1.5 align-middle text-right">
                        {isEditing ? (
                          <input
                            type="text"
                            value={currentItem.cost_exchange_rate ?? "1"}
                            onChange={(e) => setDraftItem({...draftItem, cost_exchange_rate: cleanInputValue(e.target.value)})}
                            onKeyDown={handleGlobalKeyDown}
                            className="w-12 text-right px-1 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500"
                          />
                        ) : (
                      ...
                      <td className="px-2 py-1.5 align-middle text-right">
                        <span className="font-bold text-gray-900 dark:text-gray-100">
                          {formatNumberForDisplay(isEditing 
                            ? (Number(currentItem.cost_amount) || 0) * (Number(currentItem.cost_exchange_rate) || 1)
                            : currentItem.cost_amount_try)}
                        </span>
                      </td>
                      ...
                      <td className="px-2 py-1.5 align-middle text-right border-l border-gray-200 dark:border-gray-700">
                        {isEditing ? (
                          <input
                            type="text"
                            value={currentItem.sale_amount ?? ""}
                            onChange={(e) => setDraftItem({...draftItem, sale_amount: cleanInputValue(e.target.value)})}
                            onKeyDown={handleGlobalKeyDown}
                            className="w-16 text-right px-1.5 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500"
                          />
                        ) : (
                      ...
                      <td className="px-2 py-1.5 align-middle">
                        {isEditing ? (
                          <select
                            value={currentItem.sale_currency || "EUR"}
                            onChange={(e) => setDraftItem({...draftItem, sale_currency: e.target.value})}
                            onKeyDown={handleGlobalKeyDown}
                            className="w-full px-1 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500"
                          >
                            {CURRENCIES.map(c => <option key={c} value={c}>{c}</option>)}
                          ...
                          <span className="text-gray-700 dark:text-gray-300 font-medium">{currentItem.sale_currency || "EUR"}</span>
                        )}
                      </td>
                      ...
                      <td className="px-2 py-1.5 align-middle text-right">
                        {isEditing ? (
                          <input
                            type="text"
                            value={currentItem.sale_exchange_rate ?? "1"}
                            onChange={(e) => setDraftItem({...draftItem, sale_exchange_rate: cleanInputValue(e.target.value)})}
                            onKeyDown={handleGlobalKeyDown}
                            className="w-12 text-right px-1 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 focus:ring-1 focus:ring-indigo-500"
                          />
                        ) : (
                      ...
                      <td className="px-2 py-1.5 align-middle text-right">
                        <span className="font-bold text-gray-900 dark:text-gray-100">
                          {formatNumberForDisplay(isEditing 
                            ? (Number(currentItem.sale_amount) || 0) * (Number(currentItem.sale_exchange_rate) || 1)
                            : currentItem.sale_amount_try)}
                        </span>
                      </td>
      ...
      {filteredOthers.length > 0 && Object.keys(totalsBySupplierAndCurrency).length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {Object.entries(totalsBySupplierAndCurrency).map(([supplier, data]) => (
              <div key={supplier} className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                ...
                <div className="mb-2">
                  ...
                  {Object.entries(data.costs).map(([cur, amount]) => amount ? (
                    <div key={cur} className="flex justify-between text-xs mb-0.5">
                ...
                <div>
                  ...
                  {Object.entries(data.sales).map(([cur, amount]) => amount ? (
                    <div key={cur} className="flex justify-between text-xs mb-0.5">
                      <span className="text-gray-600 dark:text-gray-400">{cur}</span>
