...
interface PurchaseTabProps {
  ...
  setExpandedCategoriesPurchase: (
    expanded: Set<string> | ((prev: Set<string>) => Set<string>),
  ) => void;
  newItem: any;
  ...
  CheckboxWithIndeterminate: React.ComponentType<{
    checked: boolean;
      ...
      <div className="overflow-x-auto custom-scrollbar pb-2 -mx-4 px-4 md:mx-0 md:px-0">
        <div className="min-w-max md:min-w-full">
          ...
          <div className="bg-gray-100 dark:bg-gray-700 rounded-md p-2 flex items-center gap-2 text-xs font-semibold text-gray-800 dark:text-gray-200">
            ...
            <div className="w-36">{t('projects.hotelVendor') || "OTEL / TEDARİKÇİ"}</div>
              ...
              {permCreate(Module.PROJECTS) && !isLocked && (
                <button
                  onClick={() => {
                    if (isLocked) {
                      alert(
                        t('projects.lockedProjectAlert') || "Bu proje kilitli olduğu için alış kaydı eklenemez. Önce Projeler listesinden kilidi kaldırmanız gerekir.",
                      );
                      return;
                    }
                    console.log(
                      "Başlık ekleme ikonuna tıklandı - kategori seçim modalı açılmalı (Alış)",
                    );
          ...
          {showAddRowPurchase && (
            <div className="bg-blue-500/10 dark:bg-blue-900/20 rounded-md p-2 border border-blue-200 dark:border-blue-800 mt-1">
              ...
              >
                  ...
                  <div className="space-y-4">
                    {mainCategories.length === 0 ? (
                      ...
                      mainCategories.map((mainCat: any) => {
                        ...
                        const allSelected = isAllSubCategoriesSelected(
                          mainCat.id,
                          "purchase",
                        );
                        const someSelected = isSomeSubCategoriesSelected(
                          mainCat.id,
                          "purchase",
                        );
                            ...
                            <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                              <div className="flex items-center gap-2 flex-1">
                                <button
                                  ...
                                  title={isExpanded ? (t('common.collapse') || "Daralt") : (t('common.expand') || "Genişlet")}
                                >
                                  <svg
                              ...
                              <div className="p-3 bg-white dark:bg-gray-800 space-y-1 border-t border-gray-200 dark:border-v3-border">
                                {subCats.map((subCat: any) => (
                                  ...
                                  >
                                    <input
                                      type="checkbox"
                                      checked={selectedCategoriesPurchase.has(
                                        subCat.id,
                                      )}
                                      onChange={(e) => {
                                        setSelectedCategoriesPurchase(
                ...
                <div className="flex items-center justify-between p-4 border-t border-gray-200 dark:border-v3-border">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {t('projects.addSelectedCategories') || "Seçili Kategorileri Ekle"} ({selectedCategoriesPurchase.size})
                    </button>
                  </div>
                </div>
              </div>
                ...
                return (
                  ...
                  >
                    <div className="flex flex-nowrap items-center gap-2 text-xs font-semibold text-gray-800 dark:text-gray-200">
                      ...
                      <div className="w-28 text-right pr-1 whitespace-nowrap">
                        {Object.entries(curMap)
                          .map(
                            ([cur, val]: any) =>
                              `${formatNumber(Number(val || 0))} ${cur}`,
                          )
                          .join(" + ")}
                      </div>
                      <div className="w-16"></div>
                      <div className="w-16"></div>
                      <div className="w-16"></div>
                      <div className="w-32 text-right pr-1 whitespace-nowrap">
                        {formatNumber(row.totalTRY || 0)} TL
                      </div>
                    ...
                    <>
                      <div className="w-44">
                        ...
                        >
                          ...
                          {categories
                            .filter(
                              (c: any) => c.parent_id === it.main_category,
                            )
                            .map((c: any) => (
                              <option key={c.id} value={c.id}>
                                {c.name}
                              ...
                              (supplier: any, index: number) => (
                                <div
                                  ...
                                  className={`px-3 py-2 text-xs cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 ${
                                    index === selectedPurchaseSupplierIndex
