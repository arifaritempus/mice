...
export default function ProjectDetailPage() {
  ...
  const formatTRY = (n: number) => new Intl.NumberFormat('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(n || 0));
    ...
    try {
      ...
      for (let rowNumber = 3; rowNumber <= worksheet.rowCount; rowNumber++) {
        ...
        const rowData: any = {
          ...
          gelis_ucak_inis: formatTimeAccommodation(getCellValue(row.getCell(11))),
          cikis_tarihi: formatDateAccommodation(getCellValue(row.getCell(12))),
          donus_kodu: getCellValue(row.getCell(13)),
          donus_ucak_kalkis: formatTimeAccommodation(getCellValue(row.getCell(14))),
          donus_ucak_inis: formatTimeAccommodation(getCellValue(row.getCell(15))),
  ...
  const handleAccommodationClear = () => {
    ...
    return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-3 transition-colors duration-200">
      {/* Header */}
              ...
              {activeTab === 'konaklama' && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h2 className="text-sm font-semibold text-gray-900 dark:text-white">Konaklama</h2>
                    <div className="flex items-center gap-2">
                      <input
                        type="file"
                        accept=".xlsx,.xls"
                  ...
                  {accommodationItems.length > 0 ? (
                    <div className="space-y-4">
                            ...
                            <tbody>
                              {accommodationItems.map((item, index) => (
                                <tr key={index} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                  <td className="px-2 py-1">{item.oda_no || '-'}</td>
                                  <td className="px-2 py-1">{item.oda_tipi || '-'}</td>
                                  <td className="px-2 py-1">{item.yatak_tipi || '-'}</td>
                                  <td className="px-2 py-1">{item.isim || '-'}</td>
                                  <td className="px-2 py-1">{item.soyisim || '-'}</td>
                                  <td className="px-2 py-1">{item.oda_kategori || '-'}</td>
                                  <td className="px-2 py-1">{item.oda_notu || '-'}</td>
                                  <td className="px-2 py-1">{formatDateAccommodation(item.gelis_tarihi) || '-'}</td>
                                  <td className="px-2 py-1">{item.gelis_kodu || '-'}</td>
                                  <td className="px-2 py-1">{formatTimeAccommodation(item.gelis_saat) || '-'}</td>
                                  <td className="px-2 py-1">{formatTimeAccommodation(item.gelis_ucak_inis) || '-'}</td>
                                  <td className="px-2 py-1">{formatDateAccommodation(item.cikis_tarihi) || '-'}</td>
                                  <td className="px-2 py-1">{item.donus_kodu || '-'}</td>
                                  <td className="px-2 py-1">{formatTimeAccommodation(item.donus_ucak_kalkis) || '-'}</td>
                                  <td className="px-2 py-1">{formatTimeAccommodation(item.donus_ucak_inis) || '-'}</td>
                                  <td className="px-2 py-1">{formatDateAccommodation(item.tarih_1) || '-'}</td>
                                  <td className="px-2 py-1">{formatDateAccommodation(item.tarih_2) || '-'}</td>
                                  <td className="px-2 py-1">{formatDateAccommodation(item.tarih_3) || '-'}</td>
                                  <td className="px-2 py-1">{formatDateAccommodation(item.tarih_4) || '-'}</td>
                                  <td className="px-2 py-1">{formatDateAccommodation(item.tarih_5) || '-'}</td>
                                  <td className="px-2 py-1">{formatDateAccommodation(item.tarih_6) || '-'}</td>
                                  <td className="px-2 py-1">{formatDateAccommodation(item.tarih_7) || '-'}</td>
                                  <td className="px-2 py-1">{item.geceli || '-'}</td>
                                  <td className="px-2 py-1">{item.paket || '-'}</td>
                                  <td className="px-2 py-1">{item.otel || '-'}</td>
                                  <td className="px-2 py-1">{item.ucak || '-'}</td>
                                  <td className="px-2 py-1">{item.toplam || '-'}</td>
                                  <td className="px-2 py-1">{item.doviz || '-'}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                      ...
                      <div className="mt-6">
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Günlük Inhouse Kontrolü</h3>
                        <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
                          <div className="overflow-x-auto">
                            <table className="w-full text-xs">
                              <thead className="bg-gray-100 dark:bg-gray-700">
                                    ...
                                    return (
                                      <>
                                        ...
                                        {allRoomTypes.map(roomType => (
                                          <th key={roomType} className="px-3 py-2 text-center font-semibold">{roomType}</th>
                                        ))}
                                        <th className="px-3 py-2 text-center font-semibold">Toplam Oda</th>
                                        <th className="px-3 py-2 text-center font-semibold">Toplam Pax</th>
                                      </>
                                    );
                                  })()}
                                </tr>
                              ...
                              <tbody>
                                {(() => {
                                  ...
                                  accommodationItems.forEach(item => {
                                    ...
                                    if (checkIn && checkOut && checkIn !== '-' && checkOut !== '-') {
                                      try {
                                        ...
                                        if (checkInParts.length === 3 && checkOutParts.length === 3) {
                                          ...
                                          if (!isNaN(startDate.getTime()) && !isNaN(endDate.getTime())) {
                                            ...
                                            for (let d = new Date(startDate); d < endDate; d.setDate(d.getDate() + 1)) {
                                              ...
                                              if (!dateStats[dateKey].rooms.has(roomKey)) {
                                                dateStats[dateKey].rooms.add(roomKey);
                                        ...
                                        console.log('Tarih parse hatası:', checkIn, checkOut, error);
                                      }
                                    }
                                  });
                                  ...
                                  return sortedDates.map((dateKey, index) => {
                                    ...
                                    const totalRooms = Object.values(stats.roomTypes).reduce((sum: number, count: any) => sum + (count || 0), 0);
                                    
                                    // Pax hesaplama - oda tipine göre kişi sayısı
                                    const paxCount = Object.entries(stats.roomTypes).reduce((sum: number, [roomType, count]: [string, any]) => {
                                    ...
                                    return (
                                      <tr key={uniqueKey} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <td className="px-3 py-2 font-medium">{stats.date}</td>
                                        {allRoomTypes.map(roomType => {
                                          const count = stats.roomTypes[roomType] || 0;
                                          const colors = [
                                          ...
                                          const colorClass = colors[allRoomTypes.indexOf(roomType) % colors.length];
                                          
                                          return (
                                            <td key={roomType} className="px-3 py-2 text-center">
                                              {count > 0 && (
                                        ...
                                        try {
                                          ...
                                          if (checkInParts.length === 3 && checkOutParts.length === 3) {
                                            ...
                                            if (!isNaN(startDate.getTime()) && !isNaN(endDate.getTime())) {
                                              ...
                                              grandTotalPax += nights * paxPerRoom;
                                            }
                                          }
                                        } catch (error) {
                                          console.log('Genel toplam tarih parse hatası:', checkIn, checkOut, error);
                                        }
                                      }
                                    });
                        ...
                        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                          {(() => {
                            const totalStats = accommodationItems.reduce((acc: any, item) => {
                              const roomType = item.oda_tipi || 'Belirsiz';
                            ...
                            const uniqueRooms = new Set(accommodationItems.map(item => item.oda_no)).size;
                            ...
                            return (
                              <>
                                <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl p-4 text-white shadow-lg">
                                  <div className="flex items-center justify-between">
                                    <div>
                                      <p className="text-blue-100 text-sm">Toplam Misafir</p>
                                ...
                                <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-xl p-4 text-white shadow-lg">
                                  <div className="flex items-center justify-between">
                                    <div>
                                      <p className="text-green-100 text-sm">Benzersiz Oda</p>
                                      <p className="text-2xl font-bold">{uniqueRooms}</p>
                                    </div>
                                    <div className="bg-white bg-opacity-20 rounded-full p-3">
                                      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                ...
                                <div className="bg-gradient-to-r from-purple-500 to-purple-600 rounded-xl p-4 text-white shadow-lg">
                                  <div className="flex items-center justify-between">
                                    <div>
                                      <p className="text-purple-100 text-sm">Oda Tipi Dağılımı</p>
                                      <p className="text-sm">
                                        SNG: {totalStats.SNG || 0} | DBL: {totalStats.DBL || 0} | TRP: {totalStats.TRP || 0}
                                      </p>
                                    </div>
                                    <div className="bg-white bg-opacity-20 rounded-full p-3">
                                      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="bg-gray-50 dark:bg-gray-700 rounded-md p-4">
                      <p className="text-xs text-gray-600 dark:text-gray-300">Henüz konaklama verisi yok. Excel dosyasından içe aktarabilirsiniz.</p>
                    </div>
                  )}
                </div>
              )}
              {activeTab === 'otel-ekstra' && <EmptySection title="Otel Ekstra" />}
              {activeTab === 'ucak-bileti' && <EmptySection title="Uçak Bileti" />}
              {activeTab === 'transfer-tur' && <EmptySection title="Transfer & Tur" />}
              {activeTab === 'etkinlik-aktivite' && <EmptySection title="Etkinlik & Aktivite" />}
              {activeTab === 'insan-kaynaklari' && <EmptySection title="İnsan Kaynakları" />}
              {activeTab === 'diger-servisler' && <EmptySection title="Diğer Servisler" />}
...
function SectionHeader({ title }: { title: string }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        <span className="text-[10px] text-gray-500 dark:text-gray-400">Satırlar tek satır, h-10</span>
      </div>
                </div>
  );
}
...
function ServiceEditor({
  ...
  return (
    <div className="space-y-2">
      ...
      {/* Üst ekleme satırı */}
      {showAddRow && (
        <div className="rounded-md p-2 flex flex-wrap md:flex-nowrap items-center gap-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700">
          <select value={newItem.main_category} onChange={(e)=> setNewItem({ ...newItem, main_category: e.target.value })} className="w-36 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
            <option value="">Ana Kategori</option>
            {categories.filter((c:any)=>!c.parent_id).map((c:any)=> (
              <option key={c.id} value={c.id}>{c.name}</option>
            ))}
          </select>
          <select value={newItem.sub_category} onChange={(e)=> setNewItem({ ...newItem, sub_category: e.target.value })} className="w-40 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
            <option value="">Alt Kategori</option>
            {categories.filter((c:any)=>c.parent_id===newItem.main_category).map((c:any)=> (
              <option key={c.id} value={c.id}>{c.name}</option>
            ))}
          </select>
          <div className="flex-1 min-w-0"></div>
          <div className="w-24 flex items-center gap-1 justify-end pr-1">
            <button onClick={()=>{ onAdd(); setShowAddRow(false); }} className="p-1 rounded text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/30" title="Ekle">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v12m6-6H6"/></svg>
            </button>
            <button onClick={()=> setShowAddRow(false)} className="p-1 rounded text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-900/30" title="İptal">
            ...
            if (row.type === 'subtotal') {
              return (
                <div key={`subtotal-${row.category}`} className="bg-gray-100 dark:bg-gray-700 rounded-md p-2 flex flex-wrap md:flex-nowrap items-center gap-2">
                  ...
                  <div className="w-32 text-right pr-1 text-xs font-semibold text-gray-800 dark:text-gray-200">{formatTRY(row.total)}</div>
                  <div className="w-16"></div>
                  <div className="w-16"></div>
                  <div className="w-16"></div>
            ...
            return (
              ...
              {it.isEditing ? (
                ...
                <>
                  ...
                  <select value={it.sub_category} onChange={(e)=>{ const updated = {...it, sub_category: e.target.value}; const list = [...items]; const index = list.findIndex((item: any) => item.id === it.id); list[index] = updated; saveItems(side, list); }} className="w-40 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
                    <option value="">Alt Kategori</option>
                    {categories.filter((c:any)=>c.parent_id===it.main_category).map((c:any)=> (
                      <option key={c.id} value={c.id}>{c.name}</option>
                    ))}
                  </select>
                  <input value={it.qty} onChange={(e)=>{ const updated = {...it, qty: Number(e.target.value), total: Number(e.target.value) * it.repeat * it.unit_price, total_try: Number(e.target.value) * it.repeat * it.unit_price * it.fx}; const list = [...items]; const index = list.findIndex((item: any) => item.id === it.id); list[index] = updated; saveItems(side, list); }} type="number" step="1" className="w-16 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white" autoFocus />
                  <input value={it.repeat} onChange={(e)=>{ const updated = {...it, repeat: Number(e.target.value), total: it.qty * Number(e.target.value) * it.unit_price, total_try: it.qty * Number(e.target.value) * it.unit_price * it.fx}; const list = [...items]; const index = list.findIndex((item: any) => item.id === it.id); list[index] = updated; saveItems(side, list); }} type="number" step="1" className="w-20 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
                  <input value={it.unit_price} onChange={(e)=>{ const updated = {...it, unit_price: Number(e.target.value), total: it.qty * it.repeat * Number(e.target.value), total_try: it.qty * it.repeat * Number(e.target.value) * it.fx}; const list = [...items]; const index = list.findIndex((item: any) => item.id === it.id); list[index] = updated; saveItems(side, list); }} type="number" step="0.01" className="w-24 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
                  <div className="w-28 px-2 py-1 text-xs text-right text-gray-700 dark:text-gray-200">{formatTRY(it.total)}</div>
                  <select value={it.currency} onChange={(e)=>{ const updated = {...it, currency: e.target.value}; const list = [...items]; const index = list.findIndex((item: any) => item.id === it.id); list[index] = updated; saveItems(side, list); }} className="w-16 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
                    <option>EUR</option>
                    <option>USD</option>
                    <option>TL</option>
                  </select>
                  <input value={it.vat} onChange={(e)=>{ const updated = {...it, vat: Number(e.target.value)}; const list = [...items]; const index = list.findIndex((item: any) => item.id === it.id); list[index] = updated; saveItems(side, list); }} type="number" step="1" min="0" max="100" placeholder="5" className="w-16 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
                  <input value={it.fx} onChange={(e)=>{ const updated = {...it, fx: Number(e.target.value), total_try: it.qty * it.repeat * it.unit_price * Number(e.target.value)}; const list = [...items]; const index = list.findIndex((item: any) => item.id === it.id); list[index] = updated; saveItems(side, list); }} type="number" step="0.0001" className="w-16 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
                  <input
                    value={it.total_try}
                    onChange={(e)=>{
                  ...
                  />
                  <input value={it.description} onChange={(e)=>{ const updated = {...it, description: e.target.value}; const list = [...items]; const index = list.findIndex((item: any) => item.id === it.id); list[index] = updated; saveItems(side, list); }} className="flex-1 min-w-0 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white" placeholder="Açıklama" />
                  {showSupplier && (
                  ...
                  <div className="w-24 flex items-center gap-1 justify-end pr-1">
                    <button onClick={()=>{ const updated = {...it, isEditing: false}; const list = [...items]; const index = list.findIndex((item: any) => item.id === it.id); list[index] = updated; saveItems(side, list); }} className="p-1 rounded text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/30" title="Kaydet">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7"/></svg>
                    </button>
                    <button onClick={()=>onRemove(side,it.id)} className="p-1 rounded text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30" title="Sil">
                ...
                <>
                  ...
                  <div className="w-24 text-right pr-1 text-xs text-gray-900 dark:text-white">{formatTRY(it.unit_price)}</div>
                  <div className="w-32 text-right pr-1 text-xs font-semibold text-gray-900 dark:text-white">{formatTRY(it.total)}</div>
                  ...
                  {showSupplier && <div className="w-40 text-xs text-gray-900 dark:text-white">{getVendorName(it.supplier) || '-'}</div>}
                  <div className="w-24 flex items-center gap-1 justify-end pr-1">
                    <button onClick={()=>onAddBelow(side, it.id)} className="p-1 rounded text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/30" title="Satır Ekle">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                    </button>
                    <button onClick={()=>onEditRow(side, it.id)} className="p-1 rounded text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30" title="Düzenle">
          ...
          <div className="mt-4 bg-blue-600 dark:bg-blue-700 rounded-md p-3">
            <div className="flex flex-wrap md:flex-nowrap items-center gap-2">
              ...
              <div className="w-32 text-right pr-1 text-sm font-bold text-white">
                {Object.entries(totals).map(([cur, val]: any) => `${formatTRY(Number(val || 0))} ${cur}`).join(' + ')}
              </div>
              <div className="w-16"></div>
              <div className="w-16"></div>
              <div className="w-16"></div>
              <div className="w-32 text-right pr-1 text-sm font-bold text-white">
                {formatTRY(items.reduce((sum: number, it: any) => sum + Number(it.total_try || 0), 0))} TL
              </div>
              <div className="flex-1 min-w-0"></div>
              {showSupplier && <div className="w-40"></div>}
              <div className="w-24"></div>
...
function EmptySection({ title }: { title: string }) {
  return (
    <div className="bg-gray-50 dark:bg-gray-700 rounded-md p-4">
      <p className="text-xs text-gray-600 dark:text-gray-300">{title} sekmesi için tasarım ve veri entegrasyonu burada oluşturulacaktır.</p>
            </div>
  );
}
...
function ProfitLossList({ 
  ...
  const groupedServices = aggregatedList.reduce((acc: any, service: Aggregated) => {
  ...
  const calculateProfitLoss = (service: Aggregated) => {
    return Number(service.salesTotalTRY || 0) - Number(service.purchaseTotalTRY || 0);
  };
  ...
  const calculateProfitMargin = (service: Aggregated) => {
    ...
    return ((salesTotalTRY - Number(service.purchaseTotalTRY || 0)) / salesTotalTRY) * 100;
  };
  ...
  const tooltipForFx = (fxSet: Set<number>) => {
    ...
    return Array.from(fxSet).map((v) => formatTRY(v)).join(', ');
  };
      ...
      {Object.entries(groupedServices).map(([categoryName, services]: [string, any]) => (
        <div key={categoryName} className="space-y-1">
          ...
          <div className="bg-gray-200 dark:bg-gray-600 rounded-md p-2">
            <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200">{categoryName}</h3>
          </div>
          ...
          {services.map((service: any, index: number) => {
            ...
            return (
              <div key={index} className="bg-white dark:bg-gray-800 rounded-md p-2 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                <div className="flex items-center gap-2 text-xs">
                  {/* Alt kategori */}
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-gray-900 dark:text-white">{getCategoryName(service.sub_category) || '-'}</div>
                  </div>
                  
                  {/* Satış Döviz */}
                  <div className="w-32 text-right pr-1">
                    <div className="font-semibold text-green-600 dark:text-green-400 cursor-help" title={tooltipForCurrencies(service.salesCurrencies, service.salesTotal)}>{displayCurrencyTotal(service.salesCurrencies, service.salesTotal)}</div>
                  </div>
                  
                  {/* Satış Kur */}
                  <div className="w-24 text-right pr-1"><div className="text-gray-600 dark:text-gray-400 cursor-help" title={tooltipForFx(service.salesFxValues)}>{displayFx(service.salesFxValues)}</div></div>
                  
                  {/* Satış TL */}
                  <div className="w-32 text-right pr-1"><div className="font-semibold text-green-600 dark:text-green-400">{formatTRY(service.salesTotalTRY)}</div></div>
                  
                  {/* Alış Döviz */}
                  <div className="w-32 text-right pr-1"><div className="font-semibold text-red-600 dark:text-red-400 cursor-help" title={tooltipForCurrencies(service.purchaseCurrencies, service.purchaseTotal)}>{displayCurrencyTotal(service.purchaseCurrencies, service.purchaseTotal)}</div></div>
                  
                  {/* Alış Kur */}
                  <div className="w-24 text-right pr-1"><div className="text-gray-600 dark:text-gray-400 cursor-help" title={tooltipForFx(service.purchaseFxValues)}>{displayFx(service.purchaseFxValues)}</div></div>
                  
                  {/* Alış TL */}
                  <div className="w-32 text-right pr-1"><div className="font-semibold text-red-600 dark:text-red-400">{formatTRY(service.purchaseTotalTRY)}</div></div>
                  
                  {/* Kar/Zarar */}
                  <div className="w-32 text-right pr-1">
                    <div className={`font-bold ${profitLoss >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                  ...
                  <div className="w-24 text-right pr-1">
                    <div className={`${profitMargin >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
      ...
      {Object.keys(groupedServices).length > 0 && (
        <div className="bg-gray-200 dark:bg-gray-600 rounded-md p-2 border-2 border-gray-300 dark:border-gray-500">
          <div className="flex items-center gap-2 text-sm font-bold">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-bold text-gray-900 dark:text-white">GENEL TOPLAM</div>
            </div>
            ...
            <div className="w-32 text-right pr-1">
              <div className="font-bold text-green-600 dark:text-green-400">
                {(() => {
                  const totalSales = aggregatedList.reduce((sum, s) => sum + Number(s.salesTotal || 0), 0);
                  const currency = (() => {
            ...
            <div className="w-32 text-right pr-1">
              <div className="font-bold text-green-600 dark:text-green-400">
                {formatTRY(aggregatedList.reduce((sum, s) => sum + Number(s.salesTotalTRY || 0), 0))}
              </div>
            </div>
            ...
            <div className="w-32 text-right pr-1">
              <div className="font-bold text-red-600 dark:text-red-400">
                {(() => {
                  const totalPurchase = aggregatedList.reduce((sum, s) => sum + Number(s.purchaseTotal || 0), 0);
                  const currency = (() => {
                    for (const s of aggregatedList) {
                      if (s.purchaseCurrencies && s.purchaseCurrencies.size > 0) {
            ...
            <div className="w-32 text-right pr-1">
              <div className="font-bold text-red-600 dark:text-red-400">
                {formatTRY(aggregatedList.reduce((sum, s) => sum + Number(s.purchaseTotalTRY || 0), 0))}
              </div>
            </div>
            ...
            <div className="w-32 text-right pr-1">
              <div className={`font-bold ${(() => {
                const totalSales = aggregatedList.reduce((sum, s) => sum + Number(s.salesTotalTRY || 0), 0);
                const totalPurchase = aggregatedList.reduce((sum, s) => sum + Number(s.purchaseTotalTRY || 0), 0);
                return (totalSales - totalPurchase) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400';
              })()}`}>
                {formatTRY(aggregatedList.reduce((sum, s) => sum + Number(s.salesTotalTRY || 0) - Number(s.purchaseTotalTRY || 0), 0))}
              </div>
            </div>
            ...
            <div className="w-24 text-right pr-1">
              <div className={`font-bold ${(() => {
                ...
                return margin >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400';
              })()}`}>
                %{(() => {
                  const totalSales = aggregatedList.reduce((sum, s) => sum + Number(s.salesTotalTRY || 0), 0);
                  const totalPurchase = aggregatedList.reduce((sum, s) => sum + Number(s.purchaseTotalTRY || 0), 0);
...
function SearchableVendorSelect({ value, onChange, vendors, className }: { value: string, onChange: (value: string) => void, vendors: any[], className?: string }) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case 'ArrowUp':
        e.preventDefault();
        setSelectedIndex(prev => prev <= 0 ? filteredVendors.length - 1 : prev - 1);
        break;
      case 'Enter':
        e.preventDefault();
          ...
          {filteredVendors.length > 0 ? (
            filteredVendors.map((v: any, index: number) => (
              <div
                ...
                } ${value === v.id ? 'bg-blue-50 dark:bg-blue-800' : ''}`}
                onClick={() => handleSelect(v.id)}
              >
                <div className="font-medium">{v.name} {v.type === 'hotel' && <span className="inline-block ml-1 text-[10px] px-1 py-0.5 rounded bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300">OTEL</span>}</div>
                {v.subtitle && <div className="text-gray-500 dark:text-gray-400 text-[10px]">{v.subtitle}</div>}
              </div>
            ))
          ) : (
            <div className="px-2 py-1 text-xs text-gray-500 dark:text-gray-400">Tedarikçi bulunamadı</div>
            )}
        </div>
      )}
    </div>
