...
const toggleColumnVisibility = useCallback((column: string) => {
  ...
  return <div className="space-y-4">
      ...
      sortedRoomTypes.forEach(rt => {
        roomStatusCounts[rt] = { total: 0, taken: 0, remaining: 0 };
      });
      let grandTotalRooms = 0;
      let grandTakenRooms = 0;
      let grandRemainingRooms = 0;
      
      Object.entries(groupedByRoomNumber).forEach(([roomNo, roomGroup]) => {
         let roomType = "";
         for(const item of roomGroup) {
             if (item.oda_tipi || item.room_type) {
                 roomType = item.oda_tipi || item.room_type;
                 break;
             }
         }
         if (roomType) {
             if (!roomStatusCounts[roomType]) {
                 roomStatusCounts[roomType] = { total: 0, taken: 0, remaining: 0 };
             }
             const isTaken = roomGroup.some(item => !!item.oda_no_2);
             roomStatusCounts[roomType].total++;
             grandTotalRooms++;
             if (isTaken) {
                 roomStatusCounts[roomType].taken++;
                 grandTakenRooms++;
             } else {
                 roomStatusCounts[roomType].remaining++;
                 grandRemainingRooms++;
             }
         }
      });
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
                <div className="px-4 py-3 border-b border-gray-200 dark:border-v3-border">
                  <h3 className="text-sm font-semibold text-v3-text flex items-center gap-2">
                    ...
                    Oda Durumu (Oda No - Otel Kaydı Analizi)
                  </h3>
                </div>
                <div className="p-4">
                  <div className="overflow-x-auto">
                    <table className="min-w-full text-xs">
                      <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                          <th className="px-2 py-2 text-left font-semibold text-v3-text">Oda Tipi</th>
                          <th className="px-2 py-2 text-center font-semibold text-v3-text">Toplam Oda</th>
                          <th className="px-2 py-2 text-center font-semibold text-v3-text">Alınan Oda (Oda No Girilmiş)</th>
                          <th className="px-2 py-2 text-center font-semibold text-v3-text">Kalan Oda (Bekleyen)</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200 dark:divide-v3-border">
                        {sortedRoomTypes.filter(rt => roomStatusCounts[rt] && roomStatusCounts[rt].total > 0).map(roomType => (
                          <tr key={roomType} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td className="px-2 py-2 text-left font-medium text-v3-text">{roomType}</td>
                            <td className="px-2 py-2 text-center font-semibold text-blue-600 dark:text-blue-400">{roomStatusCounts[roomType].total}</td>
                            <td className="px-2 py-2 text-center font-semibold text-green-600 dark:text-green-400">{roomStatusCounts[roomType].taken}</td>
                            <td className="px-2 py-2 text-center font-semibold text-orange-600 dark:text-orange-400">{roomStatusCounts[roomType].remaining}</td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot className="bg-gray-100 dark:bg-gray-700 font-semibold">
                        <tr>
                          <td className="px-2 py-2 text-left text-v3-text">GENEL TOPLAM</td>
                          <td className="px-2 py-2 text-center text-blue-700 dark:text-blue-300">{grandTotalRooms}</td>
                          <td className="px-2 py-2 text-center text-green-700 dark:text-green-300">{grandTakenRooms}</td>
                          <td className="px-2 py-2 text-center text-orange-700 dark:text-orange-300">{grandRemainingRooms}</td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
