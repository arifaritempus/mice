...
import { projectsService, agenciesService, hotelsService, categoriesService, publicLinksService, projectSalesItemsService, SettingsService } from '@/lib/supabaseService';
...
export default function ProjectViewPublicPage() {
  ...
  const loadProjectData = async () => {
    ...
    try {
      ...
      try {
        ...
        const salesItems = await projectSalesItemsService.getByProjectId(projectId);
        console.log('2. projectSalesItemsService bitti. Kayıt sayısı:', salesItems?.length || 0);
      ...
      try {
        console.log('Hata objesi tüm özellikleri:', Object.getOwnPropertyNames(error || {}));
      } catch (e) {}
      
      setError(`Veri yüklenirken bir hata oluştu: ${error?.message || 'Bilinmeyen hata (Detay konsolda)'}`);
    } finally {
  ...
  const getHotelConcept = (hotelId: string) => {
    ...
    if (!hotel) return '';
    return hotel.concept || hotel.board || hotel.board_name || hotel.meal_plan || hotel.pension || '';
  };
  ...
  const getClientGeoData = async () => {
    try {
      ...
      try {
        ...
        return { ip: ipData.ip || 'N/A', city: 'N/A', region: 'N/A', country: 'N/A', postal: 'N/A' };
      } catch {
        return { ip: 'N/A', city: 'N/A', region: 'N/A', country: 'N/A', postal: 'N/A' };
      }
    }
  };
  ...
  const formatSalesData = () => {
    if (!itemsSales || itemsSales.length === 0) {
      return {
        ...
        tarih: project?.start_date && project?.end_date 
    ...
    try {
      ...
      const addSheet = (sheetName: string, h: any, items: any[]) => {
        ...
        if (iconLogoBase64 && iconLogoBase64.startsWith('data:')) {
          try {
            sheet.addImage(workbook.addImage({ base64: iconLogoBase64, extension: guessExt(iconLogoBase64) as any }), { tl: { col: 0.15, row: 0.15 }, ext: { width: inchToPx(1.25), height: inchToPx(0.70) } });
          } catch (e) { console.error('Icon error:', e); }
        }
        if (wordmarkLogoBase64 && wordmarkLogoBase64.startsWith('data:')) {
          try {
            sheet.addImage(workbook.addImage({ base64: wordmarkLogoBase64, extension: guessExt(wordmarkLogoBase64) as any }), { tl: { col: 5.90, row: 0.23 }, ext: { width: inchToPx(2.4), height: inchToPx(0.55) } });
          } catch (e) { console.error('Wordmark error:', e); }
        }
        ...
        const headerInfo = [
          ...
          ['C/IN - C/OUT', !isGeneral && h.check_in_date ? `${new Date(h.check_in_date).toLocaleDateString('tr-TR')} - ${new Date(h.check_out_date).toLocaleDateString('tr-TR')}` : '-', 'OPSİYON', isGeneral ? '-' : 'SOR-SAT'],
          ['OTEL', isGeneral ? 'GENEL HİZMETLER' : getHotelName(h.hotel_id), 'DURUM', getStatusText(project.status)],
        ...
        headerInfo.forEach(([lLabel, lVal, rLabel, rVal]) => {
          ...
          rowValues[0] = lLabel; rowValues[1] = lVal; rowValues[4] = rLabel; rowValues[5] = rVal;
          ...
          row.getCell(5).font = { bold: true, size: 12 }; row.getCell(5).alignment = { horizontal: 'left', vertical: 'middle' };
          row.getCell(6).font = { size: 12 }; row.getCell(6).alignment = { horizontal: 'left', vertical: 'middle' };
          sheet.mergeCells(`B${rowIndex}:D${rowIndex}`);
          sheet.mergeCells(`F${rowIndex}:F${rowIndex}`);
          for (let c = 1; c <= 6; c++) row.getCell(c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD3CBBE' } };
          rowIndex++;
        });
        ...
        sortedCatIds.forEach((catId, i) => {
          ...
          const hRow = sheet.addRow(['DETAY/AÇIKLAMA', 'BİRİM/ADET', 'SEFER/TEKRAR', 'BİRİM/FİYAT', 'TOPLAM EUR', 'AÇIKLAMA']);
          hRow.font = { bold: true, size: 11 };
          hRow.height = 22;
          for (let c = 1; c <= 6; c++) hRow.getCell(c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF0F0F0' } };
          rowIndex++;

          let firstItemRow: number | null = null;
          catItems.forEach(item => {
            const sRow = sheet.addRow([getCategoryName(item.sub_category || ''), item.qty || 0, item.repeat || 0, item.unit_price || 0, 0, item.description || '']);
          ...
          for (let c = 1; c <= 6; c++) araRow.getCell(c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD3CBBE' } }; // Aligned with header color for subtotal
          if (firstItemRow) {
        ...
        const totalRow = sheet.addRow([reportType === 'alis' ? 'ALIŞ GENEL TOPLAMLAR' : 'SATIŞ GENEL TOPLAMLAR', '', '', '', 0, '']);
        totalRow.font = { bold: true, size: 16, color: { argb: 'FFFFFFFF' } };
        for (let c = 1; c <= 6; c++) totalRow.getCell(c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF333333' } };
      ...
      hotelsData.forEach((h: any, idx: number) => {
        const hItems = itemsSales.filter(item => item.hotel_id === h.hotel_id || item.hotel_id === h.id);
      ...
      const generalItems = itemsSales.filter(item => !item.hotel_id || item.hotel_id === 'general');
  ...
  const filteredItems = itemsSales.filter(item => 
    ...
    (activeViewHotelId === 'general' && (!item.hotel_id || item.hotel_id === 'general'))
  );
  ...
  if (!project) return <div className="p-8 text-center text-gray-500">Proje bulunamadı.</div>;
        ...
        >
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-white text-right border-l border-white/10 pl-4">
              <p className="text-[10px] text-gray-100 font-bold uppercase tracking-wider">PROJE REFERANS</p>
          ...
          >
            <div>
              <label 
                ...
                style={{ color: appSettings?.secondary_color || '#64748b' }}
              >
                ACENTE | FİRMA
              </label>
              <p className="text-sm font-bold" style={{ color: appSettings?.light_text_color || '#1e293b' }}>
                {getAgencyName(project.agency_id)} | {project.company_name}
              </p>
            </div>
            <div>
              <label className="block text-[10px] text-gray-600 font-black uppercase tracking-tighter mb-2 underline decoration-blue-600/30">PROJE DURUMU</label>
              <span className={`px-2 py-1 text-[10px] font-bold rounded-full ${project.status === 'approved' || project.status === 'completed' ? 'bg-green-500/10 text-green-500' : 'bg-blue-500/10 text-blue-500'}`}>
                {getStatusText(project.status).toUpperCase()}
              </span>
            </div>
            <div>
              <label className="block text-[10px] text-gray-600 font-black uppercase tracking-tighter mb-2 underline decoration-blue-600/30">C-IN TARİHİ</label>
              <p className="text-sm font-bold text-slate-900">
                {project.start_date ? new Date(project.start_date).toLocaleDateString('tr-TR') : '-'}
              </p>
            </div>
            <div>
              <label className="block text-[10px] text-gray-600 font-black uppercase tracking-tighter mb-2 underline decoration-blue-600/30">C-OUT TARİHİ</label>
              <p className="text-sm font-bold text-slate-900">
          ...
          >
            <div className="flex-1 flex flex-nowrap overflow-x-auto scrollbar-hide py-1 px-1">
              <button 
                ...
                style={{ 
                  backgroundColor: activeViewHotelId === 'all' ? (appSettings?.light_bg_secondary || '#ffffff') : 'transparent',
              ...
              {hotelsData.map((h: any, idx: number) => (
                <button 
                  ...
                  style={{ 
                    backgroundColor: activeViewHotelId === h.id ? (appSettings?.light_bg_secondary || '#ffffff') : 'transparent',
                    minWidth: 'fit-content'
                  }}
                >
                  {getHotelName(h.hotel_id) !== '' ? getHotelName(h.hotel_id) : `OTEL ${idx + 1}`}
                </button>
              ))}
              <button 
                ...
                style={{ 
                  backgroundColor: activeViewHotelId === 'general' ? (appSettings?.light_bg_secondary || '#ffffff') : 'transparent',
                  minWidth: 'fit-content'
                }}
              >
                GENEL HİZMETLER
        ...
        {/* Active Hotel Details Card - Exact Style from Quote Link */}
                ...
                >
                  <div>
                    <span 
                      ...
                      style={{ color: appSettings?.secondary_color || '#475569' }}
                    >
                      OTEL KONSEPTİ
                    </span>
                    <span className="text-sm font-bold" style={{ color: appSettings?.light_text_color || '#1e293b' }}>{h.hotel_concept || '-'}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] text-gray-600 font-bold uppercase mb-1 tracking-tighter">GİRİŞ / ÇIKIŞ</span>
                  ...
                  <div>
                    <span className="block text-[10px] text-gray-600 font-bold uppercase mb-1 tracking-tighter">ODA / PAX</span>
                    <span className="text-sm font-bold text-slate-900">{h.room_count || 0} Oda / {h.pax_count || 0} Pax</span>
                  </div>
                  <div>
                    <span className="block text-[10px] text-gray-600 font-bold uppercase mb-1 tracking-tighter">OPSİYON</span>
                    <span className="text-sm font-bold text-slate-900">{h.option || 'SOR - SAT'} {h.option_date ? `(${new Date(h.option_date).toLocaleDateString('tr-TR')})` : ''}</span>
                  </div>
                </div>
               );
            })()}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead style={{ backgroundColor: appSettings?.light_bg_primary || '#f8fafc' }}>
                  <tr className="border-b-2" style={{ borderColor: appSettings?.light_sidebar_border || '#e2e8f0' }}>
                    <th 
                    ...
                    <th className="py-4 text-[10px] font-black text-gray-700 uppercase tracking-tighter text-right">MİKTAR</th>
                    <th className="py-4 text-[10px] font-black text-gray-700 uppercase tracking-tighter text-right">BİRİM FİYAT</th>
                    <th className="py-4 px-4 text-[10px] font-black text-gray-700 uppercase tracking-tighter text-right">TOPLAM EUR</th>
                  </tr>
                </thead>
                {filteredItems.length === 0 ? (
                  <tbody className="divide-y divide-slate-200">
                    <tr>
                      <td colSpan={6} className="py-10 text-center text-gray-400 text-xs italic">Seçilen kategori/tab için kayıtlı hizmet bulunmamaktadır.</td>
                    </tr>
                  </tbody>
                ) : (() => {
                  const grouped = filteredItems.reduce((acc: Record<string, any[]>, item: any) => {
                  ...
                  return sortedCatIds.map(catId => {
                    ...
                    const catTotalEur = items.reduce((s, i) => s + (i.total || 0), 0);
                    const catTotalTry = items.reduce((s, i) => s + (i.total_try || 0), 0);
                    
                    return (
                      <tbody key={catId} className="border-b border-gray-100 ">
                        {/* Category Header Row */}
                          ...
                          <tr key={item.id} className="hover:bg-blue-50/20  transition-colors">
                            <td className="py-4 px-4">
                               <p className="text-xs font-bold text-slate-800">
                                {scrubText(getCategoryName(item.sub_category || '') || getCategoryName(item.main_category || ''))}
                              </p>
                              {item.description && (
                            ...
                            <td className="py-4 text-xs font-medium text-right text-slate-600 whitespace-nowrap">{item.qty} x {item.repeat}</td>
                            <td className="py-4 text-xs font-medium text-right text-slate-600 whitespace-nowrap">{formatTr(item.unit_price)} {item.currency === 'TRY' ? '₺' : (item.currency === 'EUR' ? '€' : item.currency)}</td>
                            <td className="py-4 px-4 text-sm font-black text-right text-slate-900 whitespace-nowrap">{formatEUR(item.total)}</td>
                          </tr>
                        ))}
                        ...
                        <tr className="bg-slate-50/50">
                          <td colSpan={3} className="py-3 text-right">
                             <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest italic pr-4">ARA TOPLAM ({getCategoryName(catId)})</span>
                          </td>
            ...
            <div className="mt-10 flex flex-wrap justify-between items-end gap-6 bg-[#232f38] p-8 rounded-2xl shadow-xl">
              <div className="flex-1 min-w-[300px]">
                ...
                <ul className="text-[11px] text-gray-300 space-y-1.5 leading-relaxed">
                  ...
                  <li>• ALINMASI HENÜZ KESİNLEŞMEYEN SERVİSLER İÇİN BİRİM/ ADET veya SEFER/TEKRAR ÇARPANI "0" (SIFIR) OLARAK GÜNCELLENMİŞTİR.</li>
                  <li>• OTELE GİRİŞ GÜNÜ KONAKLAMA ÖĞLE YEMEĞİ İLE BAŞLAR, SABAH KAHVALTISI EKSTRA ÜCRETLENDİRİLİR.</li>
                </ul>
              </div>
              <div className="text-right">
                <h3 className="text-[10px] text-white/80 font-bold mb-1 uppercase tracking-widest">TOPLAM GENEL TUTAR</h3>
              ...
              {isApproved ? (
                <div className="bg-green-500/10 border border-green-500/20 rounded-2xl p-8 flex flex-wrap items-center gap-6">
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-bold text-green-500 mb-2">Mutabakat Onaylandı</h3>
                    {linkData?.approval && (
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 text-[10px] font-bold text-gray-500  uppercase tracking-wider">
                        <div>
                          <p className="text-gray-400 font-medium mb-1">Onaylayan</p>
                          <p className="text-gray-900 ">{linkData.approval.name} {linkData.approval.surname}</p>
                        </div>
                        <div>
                          <p className="text-gray-400 font-medium mb-1">E-posta</p>
                          <p className="text-gray-900  lowercase font-normal">{linkData.approval.email}</p>
                        </div>
                        <div>
                          <p className="text-gray-400 font-medium mb-1">Onay Tarihi</p>
                          <p className="text-gray-900 ">{new Date(linkData.approval.approved_at).toLocaleString('tr-TR')}</p>
                        </div>
                        <div>
                          <p className="text-gray-400 font-medium mb-1">IP Adresi</p>
                          <p className="text-gray-900 ">{linkData.approval.ip_address || 'N/A'}</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="bg-white  rounded-2xl shadow-xl overflow-hidden border border-gray-100  group">
                  <div className="bg-blue-600 p-8 flex flex-wrap justify-between items-center gap-6 group-hover:bg-blue-700 transition-colors">
                    <div className="max-w-xl text-white">
                      <h3 className="text-2xl font-black mb-2 uppercase tracking-tight">Mutabakat Onayı</h3>
                      <p className="text-sm opacity-80 leading-relaxed font-medium">Bu projeyi onaylayarak tüm şartları ve rakamları kabul etmiş sayılırsınız. Onayınız dijital olarak mühürlenecektir.</p>
                    </div>
                    {!showApprovalForm && (
                      <button 
                    ...
                    <div className="p-8 bg-gray-50 ">
                      <form onSubmit={handleApprovalSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          <div>
                            <label className="block text-[10px] font-black text-gray-600 uppercase mb-2 tracking-widest">İSİM</label>
                            <input 
                              type="text" 
                          ...
                          <div>
                            <label className="block text-[10px] font-black text-gray-600 uppercase mb-2 tracking-widest">SOYİSİM</label>
                            <input 
                              type="text" 
                              value={approvalForm.surname} 
                              onChange={e => setApprovalForm({...approvalForm, surname: e.target.value})} 
                              className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-sm text-slate-900 outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                              required 
                            />
                          </div>
                          <div>
                            <label className="block text-[10px] font-black text-gray-600 uppercase mb-2 tracking-widest">E-POSTA</label>
                            <input 
                              type="email" 
