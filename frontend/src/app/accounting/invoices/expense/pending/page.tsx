...
export default function ExpensePendingPage() {
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const response = await invoicesService.getPendingPurchaseItemsPage({
        page,
        pageSize,
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-2rem)] p-4 space-y-4 bg-gray-50 text-slate-900 dark:bg-gray-900 dark:text-slate-100 w-full min-w-0 transition-colors duration-200">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            📤 Bekleyen Gider Faturaları
          </h1>
          <p className="text-sm text-slate-600 dark:text-slate-300 mt-1">
            Gelen faturası işlenmemiş veya eksik gelen alış kalemlerini listeleyin
          </p>
        </div>
        <div className="flex items-center gap-4">
          <button
            ...
            className="px-6 py-2.5 bg-gradient-to-r from-orange-600 to-orange-700 hover:from-orange-700 hover:to-orange-800 text-white rounded-xl font-bold shadow-lg shadow-orange-500/25 disabled:opacity-50 disabled:shadow-none transition-all flex items-center gap-2 h-fit"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg>
            Fatura Girişi ({selectedItems.length})
          </button>
        </div>
      </div>
          ...
          <div className="min-w-0">
            ...
            <div className="w-full h-8 px-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-800 flex items-center gap-1 overflow-x-auto">
              {voucherTokens.length > 0 && (
                <button type="button" className="shrink-0 inline-flex items-center gap-1 px-1 py-0.5 rounded bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-200 text-[10px]" onClick={() => removeLastToken(setVoucherTokens)} title={voucherTokens.join(', ')}>
          ...
          <div className="min-w-0">
            ...
            <div className="w-full h-8 px-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-800 flex items-center gap-1 overflow-x-auto">
              {companyTokens.length > 0 && (
                <button type="button" className="shrink-0 inline-flex items-center gap-1 px-1 py-0.5 rounded bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-200 text-[10px]" onClick={() => removeLastToken(setCompanyTokens)} title={companyTokens.join(', ')}>
          ...
          <div className="min-w-0">
            ...
            <div className="w-full h-8 px-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-800 flex items-center gap-1 overflow-x-auto">
              {agencyTokens.length > 0 && (
                <button type="button" className="shrink-0 inline-flex items-center gap-1 px-1 py-0.5 rounded bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-200 text-[10px]" onClick={() => removeLastToken(setAgencyTokens)} title={agencyTokens.join(', ')}>
          ...
          <div className="min-w-0">
            ...
            <div className="w-full h-8 px-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-800 flex items-center gap-1 overflow-x-auto">
              {hotelTokens.length > 0 && (
                <button type="button" className="shrink-0 inline-flex items-center gap-1 px-1 py-0.5 rounded bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-200 text-[10px]" onClick={() => removeLastToken(setHotelTokens)} title={hotelTokens.join(', ')}>
          ...
          <div className="min-w-0">
            ...
            <div className="w-full h-8 px-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-800 flex items-center gap-1 overflow-x-auto">
              {categoryTokens.length > 0 && (
                <button type="button" className="shrink-0 inline-flex items-center gap-1 px-1 py-0.5 rounded bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-200 text-[10px]" onClick={() => removeLastToken(setCategoryTokens)} title={categoryTokens.join(', ')}>
      ...
      {selectedItems.length > 0 && (
        <div className="flex items-center gap-3 p-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-800 rounded-2xl animate-in slide-in-from-top-2 duration-300">
          ...
          <div>
            <h4 className="text-[10px] font-black uppercase tracking-widest text-orange-600 dark:text-orange-400">Seçilen Kalemlerin Toplamı</h4>
            <div className="flex flex-wrap gap-x-4 gap-y-1 mt-0.5">
              {Object.entries(
                ...
                <p key={curr} className="text-xl font-black text-slate-900 dark:text-slate-100 leading-none">
                  {new Intl.NumberFormat('tr-TR', { style: 'currency', currency: curr }).format(Number(total))}
                </p>
              ))}
            </div>
          </div>
