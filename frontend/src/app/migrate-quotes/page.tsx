    ...
    try {
      ...
      if (savedQuotes) {
        ...
        const migratedQuotes = quotes.map((quote: any) => {
          const migratedItems = (quote.items || quote.service_items || []).map((item: any) => {
            let migrated = false;

            // Eski kategori ID'lerini dönüştür
            if (item.category_id && categoryMapping[item.category_id as keyof typeof categoryMapping]) {
              item.category_id = categoryMapping[item.category_id as keyof typeof categoryMapping];
              migrated = true;
            }

            if (item.main_category && categoryMapping[item.main_category as keyof typeof categoryMapping]) {
              item.main_category = categoryMapping[item.main_category as keyof typeof categoryMapping];
              migrated = true;
            }

            // Eski format kodlarını dönüştür
            if (item.sub_category_id && oldFormatMapping[item.sub_category_id as keyof typeof oldFormatMapping]) {
              item.sub_category_id = oldFormatMapping[item.sub_category_id as keyof typeof oldFormatMapping];
              migrated = true;
            }

            if (item.sub_category && oldFormatMapping[item.sub_category as keyof typeof oldFormatMapping]) {
              item.sub_category = oldFormatMapping[item.sub_category as keyof typeof oldFormatMapping];
              migrated = true;
            }
        ...
        setMigrationResult(`Migration tamamlandı! ${migratedCount} kategori kodu güncellendi.`);
      } else {
      ...
      setMigrationResult(`Migration hatası: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setMigrating(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="text-center">
        ...
        <h2 className="text-lg font-semibold text-gray-900 mb-1">Teklif Kategorileri Migrate Ediliyor...</h2>
        <p className="text-sm text-gray-600 mb-3">Eski kategori kodları yeni sabit ID'lere dönüştürülüyor...</p>
        {migrationResult && (
