      ...
      <div className="flex flex-col gap-4 mb-4 shrink-0 w-full">
        ...
        <div className="flex items-start justify-between gap-4 w-full">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div>
              <h1 className="text-xl font-light tracking-wide text-v3-text glow-text">
                {t('reports.title') || "Rapor Merkezi"}
              </h1>
              <p className="text-[10px] text-v3-muted mt-0.5 hidden sm:block">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={() => fetchReport()}
              className="h-10 bg-blue-500/20 text-blue-600 dark:text-blue-400 border border-blue-500/30 hover:bg-blue-500/30 py-2 px-3 sm:px-6 rounded-xl shadow-md text-[10px] font-black uppercase tracking-widest transition-all"
            >
              <span className="hidden sm:inline">{t('reports.query') || "SORGULA"}</span>
              <span className="sm:hidden">ARA</span>
            </button>
            <button
              onClick={handleExportExcel}
              className="h-10 bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30 py-2 px-3 sm:px-4 rounded-xl shadow-md text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1 sm:gap-2"
              title="Excel'e Aktar"
            >
        ...
        {/* Filters Area */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-start lg:justify-between gap-3 w-full">
          
          <div className="flex items-center gap-1 p-1 bg-v3-surface backdrop-blur-md border border-v3-border rounded-xl w-full flex-nowrap h-[40px] overflow-x-auto custom-scrollbar">
            {(["bu_hafta", "bu_ay", "bu_yil", "ozel"] as DatePreset[]).map(
              (preset) => (
                <button
                  key={preset}
                  onClick={() => applyPreset(preset)}
                  className={`flex-1 min-w-max px-2 py-1.5 sm:px-4 sm:py-2 text-[9px] sm:text-[11px] font-bold uppercase tracking-wider rounded-lg transition-all duration-300 flex items-center justify-center ${
                    datePreset === preset
          ...
          <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
            ...
            {datePreset === "ozel" && (
              <div className="shrink-0 w-full sm:w-[240px] h-10 animate-in fade-in zoom-in-95 duration-200">
            ...
            {/* Search (MultiToken) */}
            <div className="flex-1 min-w-[150px] max-w-full sm:max-w-sm h-10">
            ...
            {activeReport.id.includes("otel") && (
              <div className="shrink-0 w-[120px] sm:w-[150px] h-10">
                <input
