...
export default function CategoriesPage() {
  ...
  const loadCategories = () => {
    ...
    if (savedCategories) {
      ...
      const mainCategories = dedupedCategories.filter(
        (cat: Category) => cat.type === "main",
      );
      const subCategories = dedupedCategories.filter(
        (cat: Category) => cat.type === "sub",
      );

      const categoriesWithSubs = mainCategories.map((mainCat: Category) => ({
        ...mainCat,
        sub_categories: subCategories.filter(
          (subCat: Category) => subCat.parent_id === mainCat.id,
        ),
      }));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newCategory: Category = {
      ...
      created_at: editingCategory
        ? editingCategory.created_at
    ...
    if (editingCategory) {
      ...
      const withoutDup = allCategories.filter(
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Teklif kategorilerini yönetin ve düzenleyin
            </p>
          </div>
          <button
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-500/90"
                  >
