...
export default function GeneralSettingsPage() {
  ...
  const applyThemeColor = (field: string, color: string) => {
    ...
    if (field === "dark_bg_primary" || field === "light_bg_primary") {
      ...
      field === "dark_bg_secondary" ||
      field === "light_bg_secondary"
    ) {
      ...
      field === "dark_sidebar_header_bg" ||
      field === "light_sidebar_header_bg"
    ) {
      ...
      field === "dark_sidebar_border" ||
      field === "light_sidebar_border"
    ) {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ message: "Logo yüklenemedi" }));
        throw new Error(
          errorData.message || `HTTP ${response.status}: Logo yüklenemedi`,
        );
      }
      ...
      const updatedSettingsForDisplay = {
        ...settings,
        [field]: logoUrlWithCache,
      };
      ...
      if (
        oldLogoUrl &&
        oldLogoUrl.startsWith("http") &&
        oldLogoUrl.includes("/storage/v1/object/public/logos/")
      ) {
        try {
      ...
      try {
        ...
        await SettingsService.updateSetting(
          "general_settings",
          updatedSettingsForSave,
        );

        // Custom event dispatch et (Sidebar/Theme canlı güncellensin)
        window.dispatchEvent(
          new CustomEvent("settingsUpdated", {
            detail: { settings: updatedSettingsForSave },
          }),
        );
      ...
      if (field === "dark_icon_logo" || field === "light_icon_logo") {
        ...
        const currentIconLogo = isDark
          ? settings.dark_icon_logo
          : settings.light_icon_logo;
        const newIconLogo = field.includes("dark")
          ? isDark
            ? logoUrl
            : currentIconLogo
          : isDark
      ...
      setError(
        e?.message || "Logo yüklenemedi. Lütfen farklı bir dosya deneyin.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const clearLogo = async (
    ...
    if (
      ...
      logoUrl.includes("/storage/v1/object/public/logos/")
    ) {
      try {
  ...
  const handleMailTest = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccess(
          "Test maili başarıyla gönderildi! Mail ayarlarınız doğru çalışıyor.",
        );
      } else {
        const errorData = await response.json();
        setError(
          `Mail testi başarısız: ${errorData.message || "Bilinmeyen hata"}`,
        );
      }
    } catch (error) {
      console.error("Mail test hatası:", error);
      setError(
        "Mail testi sırasında bir hata oluştu. Lütfen ayarlarınızı kontrol edin.",
      );
    } finally {
      setLoading(false);
    }
      ...
      try {
        ...
        if (Object.keys(generalSettings).length > 0) {
          const mergedSettings: GeneralSettings = {
            ...
            ...generalSettings,
            smtp_port: String(
              generalSettings.smtp_port ?? DEFAULT_GENERAL_SETTINGS.smtp_port,
            ),
            smtp_host: String(
              generalSettings.smtp_host ?? DEFAULT_GENERAL_SETTINGS.smtp_host,
            ),
            smtp_username: String(
              generalSettings.smtp_username ??
                DEFAULT_GENERAL_SETTINGS.smtp_username,
            ),
            smtp_password: String(
              generalSettings.smtp_password ??
                DEFAULT_GENERAL_SETTINGS.smtp_password,
            ),
            mail_from_name: String(
              generalSettings.mail_from_name ??
                DEFAULT_GENERAL_SETTINGS.mail_from_name,
            ),
            mail_from_email: String(
              generalSettings.mail_from_email ??
                DEFAULT_GENERAL_SETTINGS.mail_from_email,
            ),
            mail_reply_to: String(
              generalSettings.mail_reply_to ??
                DEFAULT_GENERAL_SETTINGS.mail_reply_to,
            ),
          };
          ...
          const currentIconLogo = isDark
            ? mergedSettings.dark_icon_logo
            : mergedSettings.light_icon_logo;
          if (currentIconLogo) {
  ...
  const applyAllColors = (settingsData: GeneralSettings) => {
    ...
    if (settingsData.secondary_color)
      root.style.setProperty("--color-secondary", settingsData.secondary_color);
    if (settingsData.success_color)
      root.style.setProperty("--color-success", settingsData.success_color);
    if (settingsData.warning_color)
      root.style.setProperty("--color-warning", settingsData.warning_color);
    if (settingsData.error_color)
      root.style.setProperty("--color-error", settingsData.error_color);
    if (settingsData.info_color)
      root.style.setProperty("--color-info", settingsData.info_color);
    ...
    if (isDark) {
      if (settingsData.dark_bg_primary) {
        root.style.setProperty(
          "--theme-bg-primary",
          settingsData.dark_bg_primary,
        );
      }
      if (settingsData.dark_bg_secondary) {
        root.style.setProperty(
          "--theme-bg-secondary",
          settingsData.dark_bg_secondary,
        );
      }
      ...
      if (settingsData.light_bg_primary) {
        root.style.setProperty(
          "--theme-bg-primary",
          settingsData.light_bg_primary,
        );
      }
      if (settingsData.light_bg_secondary) {
        root.style.setProperty(
          "--theme-bg-secondary",
          settingsData.light_bg_secondary,
        );
      }
      ...
      if (settingsData.light_sidebar_bg) {
        root.style.setProperty(
          "--theme-sidebar-bg",
          settingsData.light_sidebar_bg,
        );
      }
      if (settingsData.light_sidebar_header_bg) {
        root.style.setProperty(
          "--theme-sidebar-header-bg",
          settingsData.light_sidebar_header_bg,
        );
      }
      if (settingsData.light_text_color) {
        root.style.setProperty(
          "--theme-text-color",
          settingsData.light_text_color,
        );
      }
      if (settingsData.light_sidebar_border) {
        root.style.setProperty(
          "--theme-sidebar-border",
          settingsData.light_sidebar_border,
        );
      }
    }
  };
  ...
  useEffect(() => {
    if (settings.primary_color)
      applyColorToCSS("primary_color", settings.primary_color);
    if (settings.secondary_color)
      applyColorToCSS("secondary_color", settings.secondary_color);
    if (settings.success_color)
      applyColorToCSS("success_color", settings.success_color);
    if (settings.warning_color)
      applyColorToCSS("warning_color", settings.warning_color);
    if (settings.error_color)
      applyColorToCSS("error_color", settings.error_color);
    if (settings.info_color) applyColorToCSS("info_color", settings.info_color);

    // Koyu tema renkleri
    if (settings.dark_bg_primary)
      applyColorToCSS("dark_bg_primary", settings.dark_bg_primary);
    if (settings.dark_bg_secondary)
      applyColorToCSS("dark_bg_secondary", settings.dark_bg_secondary);
    if (settings.dark_card_bg)
      applyColorToCSS("dark_card_bg", settings.dark_card_bg);
    if (settings.dark_sidebar_bg)
      applyColorToCSS("dark_sidebar_bg", settings.dark_sidebar_bg);
    if (settings.dark_sidebar_header_bg)
      applyColorToCSS(
        "dark_sidebar_header_bg",
        settings.dark_sidebar_header_bg,
      );
    if (settings.dark_text_color)
      applyColorToCSS("dark_text_color", settings.dark_text_color);
    if (settings.dark_sidebar_border)
      applyColorToCSS("dark_sidebar_border", settings.dark_sidebar_border);

    // Açık tema renkleri
    if (settings.light_bg_primary)
      applyColorToCSS("light_bg_primary", settings.light_bg_primary);
    if (settings.light_bg_secondary)
      applyColorToCSS("light_bg_secondary", settings.light_bg_secondary);
    if (settings.light_card_bg)
      applyColorToCSS("light_card_bg", settings.light_card_bg);
    if (settings.light_sidebar_bg)
      applyColorToCSS("light_sidebar_bg", settings.light_sidebar_bg);
    if (settings.light_sidebar_header_bg)
      applyColorToCSS(
        "light_sidebar_header_bg",
        settings.light_sidebar_header_bg,
      );
    if (settings.light_text_color)
      applyColorToCSS("light_text_color", settings.light_text_color);
    if (settings.light_sidebar_border)
      applyColorToCSS("light_sidebar_border", settings.light_sidebar_border);
  }, [
  ...
  useEffect(() => {
    if (isDark) {
      if (settings.dark_bg_primary)
        applyThemeColor("dark_bg_primary", settings.dark_bg_primary);
      if (settings.dark_bg_secondary)
        applyThemeColor("dark_bg_secondary", settings.dark_bg_secondary);
      if (settings.dark_card_bg)
        applyThemeColor("dark_card_bg", settings.dark_card_bg);
      if (settings.dark_sidebar_bg)
        applyThemeColor("dark_sidebar_bg", settings.dark_sidebar_bg);
      if (settings.dark_sidebar_header_bg)
        applyThemeColor(
          "dark_sidebar_header_bg",
          settings.dark_sidebar_header_bg,
      ...
      if (settings.dark_text_color)
        applyThemeColor("dark_text_color", settings.dark_text_color);
      if (settings.dark_sidebar_border)
        applyThemeColor("dark_sidebar_border", settings.dark_sidebar_border);
    } else {
      if (settings.light_bg_primary)
        applyThemeColor("light_bg_primary", settings.light_bg_primary);
      if (settings.light_bg_secondary)
        applyThemeColor("light_bg_secondary", settings.light_bg_secondary);
      if (settings.light_card_bg)
        applyThemeColor("light_card_bg", settings.light_card_bg);
      if (settings.light_sidebar_bg)
        applyThemeColor("light_sidebar_bg", settings.light_sidebar_bg);
      if (settings.light_sidebar_header_bg)
        applyThemeColor(
          "light_sidebar_header_bg",
          settings.light_sidebar_header_bg,
      ...
      if (settings.light_text_color)
        applyThemeColor("light_text_color", settings.light_text_color);
      if (settings.light_sidebar_border)
        applyThemeColor("light_sidebar_border", settings.light_sidebar_border);
    }
  }, [
    isDark,
    settings.dark_bg_primary,
          ...
          <form onSubmit={handleSubmit} className="p-2 space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      {settings.dark_icon_logo ? (
                        ...
                        <div className="h-12 w-12 rounded border border-dashed border-gray-300 dark:border-gray-600 flex items-center justify-center text-[10px] text-gray-400">
                          Önizleme
                        </div>
                      )}
                      <div className="flex gap-2">
                        ...
                        {settings.dark_icon_logo && (
                          <button
                            type="button"
                            onClick={() => clearLogo("dark_icon_logo")}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      {settings.dark_wordmark_logo ? (
                        ...
                        <div className="h-12 w-40 rounded border border-dashed border-gray-300 dark:border-gray-600 flex items-center justify-center text-[10px] text-gray-400">
                          Önizleme
                        </div>
                      )}
                      <div className="flex gap-2">
                        ...
                        {settings.dark_wordmark_logo && (
                          <button
                            type="button"
                            onClick={() => clearLogo("dark_wordmark_logo")}
                            className="px-2 py-1 text-xs rounded bg-red-100 text-red-700 hover:bg-red-200 dark:bg-red-900/30 dark:text-red-300"
                          >
                            Kaldır
                          </button>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        {settings.dark_menu_logo && (
                          <button
                            type="button"
                            onClick={() => clearLogo("dark_menu_logo")}
                            className="px-2 py-1 text-xs rounded bg-red-100 text-red-700 hover:bg-red-200 dark:bg-red-900/30 dark:text-red-300"
                          >
                            Kaldır
                          </button>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        {settings.light_icon_logo && (
                          <button
                            type="button"
                            onClick={() => clearLogo("light_icon_logo")}
                            className="px-2 py-1 text-xs rounded bg-red-100 text-red-700 hover:bg-red-200 dark:bg-red-900/30 dark:text-red-300"
                          >
                            Kaldır
                          </button>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        {settings.light_wordmark_logo && (
                          <button
                            type="button"
                            onClick={() => clearLogo("light_wordmark_logo")}
                            className="px-2 py-1 text-xs rounded bg-red-100 text-red-700 hover:bg-red-200 dark:bg-red-900/30 dark:text-red-300"
                          >
                            Kaldır
                          </button>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        {settings.light_menu_logo && (
                          <button
                            type="button"
                            onClick={() => clearLogo("light_menu_logo")}
                            className="px-2 py-1 text-xs rounded bg-red-100 text-red-700 hover:bg-red-200 dark:bg-red-900/30 dark:text-red-300"
                          >
                            Kaldır
                          </button>
              ...
              <div className="flex items-center space-x-3 mt-4">
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      handleChange("ai_assistant_enabled", e.target.checked)
                    }
                  />
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      ...
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "primary" ? null : "primary",
                        )
                      }
                    >
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      ...
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "secondary" ? null : "secondary",
                        )
                      }
                    >
                      <span className="text-v3-text text-xs font-medium drop-shadow-lg">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      ...
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "success" ? null : "success",
                        )
                      }
                    >
                      <span className="text-v3-text text-xs font-medium drop-shadow-lg">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      ...
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "warning" ? null : "warning",
                        )
                      }
                    >
                      <span className="text-v3-text text-xs font-medium drop-shadow-lg">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      ...
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "error" ? null : "error",
                        )
                      }
                    >
                      <span className="text-v3-text text-xs font-medium drop-shadow-lg">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      ...
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "info" ? null : "info",
                        )
                      }
                    >
                      <span className="text-v3-text text-xs font-medium drop-shadow-lg">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      className="w-full h-10 rounded-lg border-2 border-gray-300 dark:border-gray-600 cursor-pointer flex items-center justify-center hover:border-blue-500 transition-colors"
                      style={{
                        backgroundColor: settings.dark_bg_primary || "#030712",
                      }}
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "dark_bg_primary"
                            ? null
                            : "dark_bg_primary",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      ...
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "dark_bg_secondary"
                            ? null
                            : "dark_bg_secondary",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      className="w-full h-10 rounded-lg border-2 border-gray-300 dark:border-gray-600 cursor-pointer flex items-center justify-center hover:border-blue-500 transition-colors"
                      style={{
                        backgroundColor: settings.dark_card_bg || "#1f2937",
                      }}
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "dark_card_bg"
                            ? null
                            : "dark_card_bg",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      className="w-full h-10 rounded-lg border-2 border-gray-300 dark:border-gray-600 cursor-pointer flex items-center justify-center hover:border-blue-500 transition-colors"
                      style={{
                        backgroundColor: settings.dark_sidebar_bg || "#030712",
                      }}
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "dark_sidebar_bg"
                            ? null
                            : "dark_sidebar_bg",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      ...
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "dark_sidebar_header_bg"
                            ? null
                            : "dark_sidebar_header_bg",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      className="w-full h-10 rounded-lg border-2 border-gray-300 dark:border-gray-600 cursor-pointer flex items-center justify-center hover:border-blue-500 transition-colors"
                      style={{
                        backgroundColor: settings.dark_text_color || "#f9fafb",
                      }}
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "dark_text_color"
                            ? null
                            : "dark_text_color",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      ...
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "dark_sidebar_border"
                            ? null
                            : "dark_sidebar_border",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      className="w-full h-10 rounded-lg border-2 border-gray-300 dark:border-gray-600 cursor-pointer flex items-center justify-center hover:border-blue-500 transition-colors"
                      style={{
                        backgroundColor: settings.light_bg_primary || "#f9fafb",
                      }}
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "light_bg_primary"
                            ? null
                            : "light_bg_primary",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      ...
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "light_bg_secondary"
                            ? null
                            : "light_bg_secondary",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      className="w-full h-10 rounded-lg border-2 border-gray-300 dark:border-gray-600 cursor-pointer flex items-center justify-center hover:border-blue-500 transition-colors"
                      style={{
                        backgroundColor: settings.light_card_bg || "#ffffff",
                      }}
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "light_card_bg"
                            ? null
                            : "light_card_bg",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      className="w-full h-10 rounded-lg border-2 border-gray-300 dark:border-gray-600 cursor-pointer flex items-center justify-center hover:border-blue-500 transition-colors"
                      style={{
                        backgroundColor: settings.light_sidebar_bg || "#f9fafb",
                      }}
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "light_sidebar_bg"
                            ? null
                            : "light_sidebar_bg",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      ...
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "light_sidebar_header_bg"
                            ? null
                            : "light_sidebar_header_bg",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      className="w-full h-10 rounded-lg border-2 border-gray-300 dark:border-gray-600 cursor-pointer flex items-center justify-center hover:border-blue-500 transition-colors"
                      style={{
                        backgroundColor: settings.light_text_color || "#111827",
                      }}
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "light_text_color"
                            ? null
                            : "light_text_color",
                        )
                      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div
                      ...
                      onClick={() =>
                        setColorPickerOpen(
                          colorPickerOpen === "light_sidebar_border"
                            ? null
                            : "light_sidebar_border",
                        )
                      }
            ...
            <div className="flex justify-end pt-3 border-t border-gray-200 dark:border-v3-border">
              <button
                type="submit"
                disabled={loading}
                className="bg-blue-500 dark:bg-blue-500 text-white px-3 py-1 rounded-md hover:bg-blue-500/90 dark:hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200 text-xs"
              >
