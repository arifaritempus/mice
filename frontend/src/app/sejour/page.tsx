...
export default function SejourPage() {
  ...
  const exportToExcel = async () => {
    ...
    const sheet = workbook.addWorksheet(`${typeof document !== "undefined" ? document.title.split("-")[0].trim() : "MICE"} - Sejour Listesi`);
      ...
      <style dangerouslySetInnerHTML={{__html: `
        @media (min-width: 768px) {
          .sejour-filters-grid {
            ...
            grid-template-columns: minmax(0, 1.5fr) minmax(0, 1fr) minmax(0, 1fr) minmax(0, 1fr) minmax(0, 1fr) minmax(0, 1fr) auto !important;
          }
        }
      `}} />
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow mb-3 p-3 transition-colors duration-200 w-full min-w-0">
        <div className="flex flex-col sejour-filters-grid items-end gap-2 w-full min-w-0">
