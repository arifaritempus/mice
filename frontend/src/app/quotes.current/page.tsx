...
import { quotesService, quoteItemsService, agenciesService, hotelsService } from '@/lib/supabaseService';
...
export default function QuotesPage() {
  ...
  const searchQuotes = (quotes: Quote[], searchTerm: string) => {
    ...
    return quotes.filter(quote => {
      ...
      const checkInDate = quote.check_in_date ? new Date(quote.check_in_date).toLocaleDateString('tr-TR') : '';
      const checkOutDate = quote.check_out_date ? new Date(quote.check_out_date).toLocaleDateString('tr-TR') : '';
      if (checkInDate.includes(searchLower) || checkOutDate.includes(searchLower)) return true;
      
      // Oluşturulma tarihi
      const createdDate = quote.created_at ? new Date(quote.created_at).toLocaleDateString('tr-TR') : '';
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      const exportData = filteredQuotes.map(quote => ({
        ...
        'Oluşturulma Tarihi': new Date(quote.created_at).toLocaleDateString('tr-TR')
      }));
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      pdf.text(`Oluşturulma Tarihi: ${new Date().toLocaleDateString('tr-TR')}`, pageWidth / 2, 30, { align: 'center' });
      
      // Tablo başlıkları
      const headers = [
        'Referans', 'Acente', 'Firma', 'Otel', 'Tarih', 'Tür', 'Durum', 'Tutar'
      ];
  ...
  const filteredQuotesForCounts = filter === 'all' 
    ? quotes 
    : quotes.filter(q => q.status === filter);
  ...
  return (
    <div className="p-6 bg-gray-50 dark:bg-gray-950 min-h-screen transition-colors duration-200">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white transition-colors duration-200">Teklifler</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2 transition-colors duration-200">Müşteri tekliflerini yönetin</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 transition-colors duration-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400 transition-colors duration-200">Toplam Teklif</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white transition-colors duration-200">{quotes.length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 transition-colors duration-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400 transition-colors duration-200">Konfirme</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white transition-colors duration-200">{konfirmeCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 transition-colors duration-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400 transition-colors duration-200">Bekleyen</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white transition-colors duration-200">{teklifCount}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        {searchTerm && (
          <div className="mt-2 text-sm text-gray-600 dark:text-gray-400 transition-colors duration-200">
            <span className="font-medium">{filteredQuotes.length}</span> sonuç bulundu
                ...
                >
                  <div className="flex items-center">
                    ...
                    {sortField === 'created_at' && (
                      <svg className={`ml-1 h-4 w-4 ${sortDirection === 'asc' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
