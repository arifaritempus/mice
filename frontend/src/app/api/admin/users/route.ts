...
export async function GET() {
  try {
    ...
    const profileById = new Map<string, any>(
      (profileRows || []).map((row: any) => [row.id, row]),
    );
  ...
  try {
    ...
    if (authError) {
      if (authError.message.includes("already been registered")) {
        console.log(
          "ℹ️ Kullanıcı zaten Auth tarafında kayıtlı, profil kontrol ediliyor...",
        );
        ...
        if (existingUser) {
          ...
          return NextResponse.json(
            { error: "Kullanıcı kayıtlı görünüyor ama ID bulunamadı" },
            { status: 400 },
          );
        }
      } else {
    ...
    if (insertErr) {
      ...
      return NextResponse.json(
        { error: insertErr.message || "Profil oluşturulamadı" },
        { status: 400 },
      );
    }
