...
function getAdminClient() {
  ...
  if (!supabaseUrl) {
    throw new Error(
      "Supabase URL eksik. NEXT_PUBLIC_SUPABASE_URL veya SUPABASE_URL ayarlanmalı.",
    );
  }
  if (!serviceRoleKey) {
    throw new Error(
      "Service Role Key eksik. SUPABASE_SERVICE_ROLE_KEY ayarlanmalı.",
    );
  }
  ...
  try {
    ...
    if (!email || !role) {
      return NextResponse.json(
        { error: "email ve role zorunludur" },
        { status: 400 },
      );
    }
    ...
    if (userErr || !userRow) {
      return NextResponse.json(
        { error: "Kullanıcı bulunamadı (public.users)" },
        { status: 404 },
      );
    }
    ...
    if (metaErr) {
      ...
      if (!findErr && authUser) {
        ...
        console.warn(
          "Auth kullanıcısı bulunamadı, sadece public.users rolü güncellenecek",
        );
      }
    }
    ...
    if (updErr) {
      return NextResponse.json(
        { error: `Profil rolü güncellenemedi: ${updErr.message}` },
        { status: 400 },
      );
    }

    return NextResponse.json(
      { ok: true, id: authUserId, email, role },
      { status: 200 },
    );
  } catch (e: any) {
    return NextResponse.json(
      { error: e?.message || "Beklenmeyen bir hata" },
      { status: 500 },
    );
  }
}
