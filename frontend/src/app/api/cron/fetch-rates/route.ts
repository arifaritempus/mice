...
async function fetchTcmbDate(dateStr: string) {
  ...
  return currencies
    ...
    .map((c: any) => ({
      ...
      banknote_buying: c.BanknoteBuying ? parseFloat(c.BanknoteBuying.replace(',', '.')) : null,
      banknote_selling: c.BanknoteSelling ? parseFloat(c.BanknoteSelling.replace(',', '.')) : null,
      cross_rate_usd: c.CrossRateUSD ? parseFloat(c.CrossRateUSD.replace(',', '.')) : null,
    }));
}
  ...
  try {
    ...
    for (const d of datesToFill) {
      try {
        ...
        console.warn(`[TCMB] ${d} kur çekilemedi. Önceki bilinen kur kopyalanıyor.`);
    ...
    if (allRatesToUpsert.length === 0) {
      return NextResponse.json({ message: 'Kur bulunamadı veya çekilemedi.' }, { status: 200 });
    }
    ...
    console.log(`[TCMB] Toplam ${allRatesToUpsert.length} kur başarıyla kaydedildi/güncellendi.`);
