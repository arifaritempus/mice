...
function getAdminClient() {
  ...
  if (!supabaseUrl || !serviceRoleKey)
    throw new Error("Supabase service env eksik");
...
export async function GET(req: NextRequest) {
  try {
    ...
    const [{ data: roles }, { data: permissions }, { data: rolePermissions }] =
      await Promise.all([
        ...
        adminClient
          ...
          .select(
            "role_id,permission_id,permissions(id,module,action,is_active)",
          ),
      ]);
    ...
    const permissionById = new Map<string, any>(
      permissionRows.map((p: any) => [p.id, p]),
    );
