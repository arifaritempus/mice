...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      const agencyName =
        project.agencies?.name || project.company_name || "Bilinmiyor";
      const hotelName = project.hotels?.name || "Çoklu Otel/Belirtilmemiş";
      const checkIn = project.start_date
        ? new Date(project.start_date).toLocaleDateString("tr-TR")
        : "-";
      const checkOut = project.end_date
      ...
      const totalAmount =
        salesItems?.reduce((sum, item) => sum + (item.total_price || 0), 0) ||
        0;
    ...
    console.log(
      `✅ Success: Project ${projectId} reconciliation approved via token ${token.substring(0, 8)}...`,
    );
