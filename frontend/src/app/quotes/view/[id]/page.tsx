...
export default function QuoteViewPublicPage() {
    ...
    try {
      ...
      const addSheetForHotel = (sheetName: string, h: any, items: ServiceItem[]) => {
        ...
        sortedCatIds.forEach((catId, i) => {
          ...
          for (let c = 1; c <= 6; c++) araRow.getCell(c).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD0D0D0' } };
          if (firstItemRow) {
        ...
        const totalRow = sheet.addRow(['SATIŞ GENEL TOPLAMLAR', '', '', '', 0, '']);
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      setQuote(prev => prev ? { ...prev, status: tempStatus, hotels_data: tempHotelsData } : null);
  ...
  const handleHotelStatusChange = (hotelId: string, newStatus: string) => {
    setTempHotelsData(prev => prev.map(h => 
      (h.id === hotelId || h.hotel_id === hotelId) ? { ...h, hotel_status: newStatus, is_confirmed: newStatus === 'KONFİRME' } : h
    ));
  ...
  const handleApprovalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Teklif başarıyla onaylandı. İlginiz için teşekkür ederiz.');
    } catch (err) {
  ...
  const filteredItems = serviceItems.filter(item => 
    item.hotel_id === activeViewHotelId || (activeViewHotelId === 'general' && !item.hotel_id)
  );
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <svg className="w-3 h-3 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2/5} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
              {exporting ? 'İşleniyor...' : 'EXCEL İNDİR'}
            </button>
            <div className="text-white text-right border-l border-white/10 pl-4">
              <p className="text-[10px] text-gray-100 font-bold uppercase tracking-wider">TEKLİF REFERANS</p>
        ...
        <div className="p-6 md:p-10">
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm font-bold text-slate-900">{quote?.check_in_date ? new Date(quote.check_in_date).toLocaleDateString('tr-TR') : '-'}</p>
            </div>
            <div>
              ...
              <p className="text-sm font-bold text-slate-900">{quote?.check_out_date ? new Date(quote.check_out_date).toLocaleDateString('tr-TR') : '-'}</p>
            </div>
            <div className="md:col-span-2 lg:col-span-1">
              <label className="block text-[10px] text-gray-600 font-black uppercase tracking-tighter mb-2 italic underline decoration-blue-600/30">TEKLİF DURUMU (GÜNCELLE)</label>
          ...
          <div className="space-y-6">
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredItems.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="py-10 text-center text-gray-400 text-xs italic">Kayıtlı hizmet bulunmamaktadır.</td>
                    </tr>
                  ) : (() => {
                    const grouped = filteredItems.reduce((acc: Record<string, ServiceItem[]>, item: any) => {
                      const catId = item.main_category || 'other';
                        ...
                        <Fragment key={catId}>
                          ...
                          {sortedCatItems.map((item) => (
                            <tr key={item.id} className="hover:bg-blue-50/20 transition-colors">
                              ...
                              <td className="py-4 text-xs font-medium text-right text-slate-600 whitespace-nowrap">
                                {formatTr(item.unit_price)} {item.currency === 'TRY' ? '₺' : (item.currency === 'EUR' ? '€' : item.currency)}
                              </td>
            ...
            <div className="mt-10 flex flex-wrap justify-between items-end gap-6 bg-[#232f38] p-8 rounded-2xl shadow-xl">
              ...
              <div className="text-right">
                ...
                <p className="text-3xl font-black text-white">{formatEUR(filteredItems.reduce((s, i) => s + i.total, 0))}</p>
              </div>
            </div>
              ...
              {linkData?.approval?.is_approved ? (
                <div className="bg-green-50 border-2 border-green-100 rounded-3xl p-8 transition-all">
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <h3 className="text-xl font-black text-green-900 uppercase tracking-tight text-left">Teklif Onaylandı</h3>
                      <p className="text-sm text-green-700 font-medium text-left">Bu teklif resmi olarak onaylanmış ve kayda alınmıştır.</p>
                    </div>
                  </div>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 pt-6 border-t border-green-100">
                    <div className="text-left">
                      <p className="text-[10px] font-black text-green-800/40 uppercase mb-1">ONAYLAYAN</p>
                      <p className="text-sm font-bold text-green-900">{linkData.approval.name} {linkData.approval.surname}</p>
                    </div>
                    <div className="text-left">
                      <p className="text-[10px] font-black text-green-800/40 uppercase mb-1">E-POSTA</p>
                      <p className="text-sm font-bold text-green-900 lowercase">{linkData.approval.email}</p>
                    </div>
                    <div className="text-left">
                      <p className="text-[10px] font-black text-green-800/40 uppercase mb-1">ONAY TARİHİ</p>
                      <p className="text-sm font-bold text-green-900">{new Date(linkData.approval.approved_at).toLocaleString('tr-TR')}</p>
                    </div>
                    <div className="text-left">
                      <p className="text-[10px] font-black text-green-800/40 uppercase mb-1">ONAY MÜHÜRÜ (IP)</p>
                      <p className="text-sm font-bold text-green-900">{linkData.approval.ip_address || 'N/A'}</p>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border-2 border-blue-50 group hover:border-blue-100 transition-all duration-500">
                  <div className="bg-blue-600 p-8 flex flex-wrap justify-between items-center gap-6 group-hover:bg-blue-700 transition-colors">
                    <div className="max-w-xl text-white text-left">
                      <h3 className="text-2xl font-black mb-2 uppercase tracking-tight text-white">Teklif Onayı</h3>
                      <p className="text-sm font-medium opacity-80 leading-relaxed text-white/90">Bu teklifi onaylayarak seçimlerinizi kaydedip, yukarıda belirtilen tüm hizmetleri ve şartları kabul etmiş sayılırsınız. Onayınız dijital olarak mühürlenecek ve sistemimize kaydedilecektir.</p>
                    </div>
                    {!showApprovalForm && (
                      <button 
                        onClick={() => {
                  ...
                  {showApprovalForm && (
                    <div className="p-10 bg-gray-50/50">
                      <form onSubmit={handleApprovalSubmit} className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-500">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                          <div className="text-left">
                            <label className="block text-[10px] font-black text-gray-600 uppercase mb-3 tracking-widest">İSİM</label>
                            <input 
                              type="text" 
                          ...
                          <div className="text-left">
                            <label className="block text-[10px] font-black text-gray-600 uppercase mb-3 tracking-widest">SOYİSİM</label>
                            <input 
                              type="text" 
                          ...
                          <div className="text-left">
                            <label className="block text-[10px] font-black text-gray-600 uppercase mb-3 tracking-widest">E-POSTA ADRESİ</label>
                            <input 
                              type="email" 
