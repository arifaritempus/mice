...
OTELE GİRİŞ GÜNÜ SABAH KAHVALTISI, OTELDEN ÇIKIŞ GÜNÜ ÖĞLE YEMEĞİ EKSTRA OLARAK ÜCRETLENDİRİLİR.`
  ...
  const handleHotelListChange = (id: string, field: keyof SelectedHotel, value: string | number) => {
    setSelectedHotels(prev => prev.map(h => {
      if (h.id === id) {
        if (field === 'hotel_id') {
          const hotel = hotels.find(x => x.id === value);
          return { ...h, hotel_id: value as string, hotel_concept: hotel ? hotel.concept : '' };
        }
        if (field === 'room_count' || field === 'pax_count') {
          return { ...h, [field]: Number(value) };
        }
        if (field === 'option' && (value !== '1. OPSİYON' && value !== '2. OPSİYON')) {
  ...
  const copyHotel = (hotel: SelectedHotel) => {
    ...
    const itemsToCopy = serviceItems.filter(item => item.hotel_id === hotel.id);
  ...
  const loadData = async () => {
    try {
      ...
      try {
        ...
        const active = (u as any[]).filter((user: any) => user.is_active !== false);
  ...
  const handleAgencySelect = (id: string) => {
    console.log('Acente seçildi:', id, 'UUID formatı:', /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id));
    setFormData(prev => ({
      ...prev,
      agency_id: id
    }));
  ...
  const handleServiceItemChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    setNewServiceItem(prev => ({
      ...prev,
      [name]: name === 'unit_quantity' || name === 'unit_price' || name === 'sefer' ? Number(value) : value
    }));
  };
  ...
  const handleAddItem = (hotelId?: string) => {
    ...
    console.log('handleAddItem çağrıldı - Ana kategori seçildi, satır oluşturuluyor');
  ...
  const handleAddBelow = (itemId: string) => {
    ...
    const sameCategoryItems = serviceItems.filter(item => item.main_category === clickedItem.main_category);
    const lastItemInCategory = sameCategoryItems[sameCategoryItems.length - 1];
    
    if (lastItemInCategory) {
      const lastItemIndex = serviceItems.findIndex(item => item.id === lastItemInCategory.id);
  ...
  const handleAddBelowWithHotel = (itemId: string, hotelId: string) => {
    console.log('handleAddBelowWithHotel çağrıldı - itemId:', itemId, 'hotelId:', hotelId);
    // Tıklanan item'ı bul
    const clickedItem = serviceItems.find(item => item.id === itemId);
    ...
    const sameCategoryItems = serviceItems.filter(item => 
      item.main_category === clickedItem.main_category && 
      (hotelId === 'general' ? (!item.hotel_id || item.hotel_id === 'general') : item.hotel_id === hotelId)
    );
    const lastItemInCategory = sameCategoryItems[sameCategoryItems.length - 1];
  ...
  const createProjectFromQuote = async (quoteId: string, confirmedHotels: SelectedHotel[]) => {
    ...
    for (const item of relevantItems) {
      ...
      const dbHotelId = originalIndex !== -1 ? normalizedHotels[originalIndex].id : (!item.hotel_id || item.hotel_id === 'general' ? null : null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const validOperationManagers = formData.operation_managers.filter(id => uuidRegex.test(id));
      const currency = serviceItems.length > 0 ? serviceItems[0].currency : 'EUR';
      ...
      for (const item of serviceItems) {
        const matchedHotel = selectedHotels.find(sh => sh.id === item.hotel_id);
  ...
  const SearchableSelect = ({
    ...
    onChange,
    placeholder,
  }: { options: { id: string; name: string }[]; value: string; onChange: (id: string) => void; placeholder: string; }) => {
      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute left-0 right-0 mt-1 max-h-56 overflow-auto bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-20">
            ...
            {filtered.length === 0 && (
              <div className="px-3 py-2 text-sm text-gray-500 dark:text-gray-400">Sonuç yok</div>
            )}
          </div>
        )}
      </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center transition-colors duration-200">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-6">Yeni teklif oluşturmak için yetkiniz bulunmuyor.</p>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200 compact">
      <div className="p-4">
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-white transition-colors duration-200">Yeni Teklif Oluştur</h1>
          </div>
          <Link
            href="/quotes"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() => {
                              setFormData(prev => ({
                                ...prev,
                                operation_managers: prev.operation_managers.filter(id => id !== managerId)
                              }));
                            }}
              ...
              <div className="md:col-span-2 space-y-3 bg-gray-50 dark:bg-gray-900/50 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
                ...
                {activeHotelId !== 'general' && selectedHotels.map((h) => activeHotelId === h.id && (
                  <div key={h.id} className="p-4 bg-white dark:bg-gray-800/80 rounded-lg border border-blue-500 ring-2 ring-blue-500/10 shadow-sm relative group space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                      <div className="md:col-span-4">
                        ...
                        <SearchableSelect
                          options={hotels}
                          value={h.hotel_id}
                          onChange={(val) => handleHotelListChange(h.id, 'hotel_id', val)}
                      ...
                      <div className="md:col-span-2">
                        ...
                        <input
                          ...
                          onChange={(e) => handleHotelListChange(h.id, 'hotel_concept', e.target.value)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end border-t border-gray-50 dark:border-gray-700/50 pt-3">
                      <div className="md:col-span-2">
                        ...
                        <input
                          type="number"
                          value={h.room_count}
                          onChange={(e) => handleHotelListChange(h.id, 'room_count', parseInt(e.target.value) || 0)}
                          min="1"
                          className="w-full px-2 py-1 h-10 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        />
                      </div>
                      <div className="md:col-span-2">
                        ...
                        <input
                          type="number"
                          value={h.pax_count}
                          onChange={(e) => handleHotelListChange(h.id, 'pax_count', parseInt(e.target.value) || 0)}
                          min="1"
                          className="w-full px-2 py-1 h-10 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        />
                      </div>
                      <div className="md:col-span-3">
                        ...
                        <select
                          value={h.option}
                          onChange={(e) => handleHotelListChange(h.id, 'option', e.target.value)}
                          className="w-full px-2 py-1 h-10 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        >
                      ...
                      <div className="md:col-span-2">
                        ...
                        <select
                          ...
                          onChange={(e) => handleHotelListChange(h.id, 'hotel_status', e.target.value)}
                          className="w-full px-2 py-1 h-10 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        >
                      ...
                      <div className="md:col-span-3">
                        ...
                        <input
                          type="date"
                          value={h.option_date}
                          onChange={(e) => handleHotelListChange(h.id, 'option_date', e.target.value)}
