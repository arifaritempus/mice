...
const getItemContext = (log: any, uuidMap: Record<string, string>) => {
  ...
  if (data.flight_code || data.gidis_ucus_kodu)
    details.push(
      `Uçuş: ${resolveUuidsInString(data.flight_code || data.gidis_ucus_kodu, uuidMap)}`,
    );
  if (data.hotel_id && uuidMap[data.hotel_id])
    details.push(`Otel: ${uuidMap[data.hotel_id]}`);
  if (data.supplier_id && uuidMap[data.supplier_id])
    details.push(`Tedarikçi: ${uuidMap[data.supplier_id]}`);
  if (data.category && uuidMap[data.category])
    details.push(`Kategori: ${uuidMap[data.category]}`);
    ...
    try {
      ...
      await publicLinksService.create({
        ...
        expiry_date: linkExpiryDate
          ? new Date(linkExpiryDate).toISOString()
          : undefined,
      ...
      console.error(err);
      alert("Link oluşturulurken hata!");
    }
  };
            ...
            <div className="flex justify-between items-start mb-8">
              <div>
                ...
                <p className="text-sm text-gray-500 font-medium">
                  Müşterileriniz için şifre korumalı ve güvenli teklif linkleri
                  oluşturun.
                </p>
              </div>
              <button
                          ...
                          <div className="flex flex-wrap justify-between items-start gap-4 mb-6">
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleToggleLinkStatus(link.id || link.token);
                                }}
                              ...
                              >
                                {link.is_active ? <>PASİF ET</> : <>AKTİF ET</>}
                              </button>
                            </div>
                          </div>
                              ...
                              <div className="space-y-1">
                                ...
                                >
                                  <span className="text-xs font-black dark:text-gray-300 font-mono tracking-wider cursor-pointer">
                                    {visiblePasswords[link.id || link.token]
                                      ? link.password
                                      : "••••••••"}
                                  </span>
                              ...
                              <div className="bg-green-100/50 dark:bg-green-900/10 p-4 rounded-2xl border border-green-200 dark:border-green-900/30">
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="flex-1">
                                    <p className="text-[10px] font-black text-green-800 dark:text-green-400 uppercase tracking-widest mb-1.5">
                                      MUTABAKAT ONAY BİLGİSİ
                                    </p>
                                    <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                                      <div>
                                        <p className="text-[9px] text-green-700/60 dark:text-green-500/60 font-bold uppercase">
                                          ONAYLAYAN
                                        </p>
                                        <p className="text-[11px] font-black text-green-900 dark:text-green-300">
                                          {approval.name} {approval.surname}
                                        </p>
                                      </div>
                                      <div>
                                        <p className="text-[9px] text-green-700/60 dark:text-green-500/60 font-bold uppercase">
                                      ...
                                      <div>
                                        ...
                                        <p className="text-[11px] font-mono font-bold text-green-800 dark:text-green-400">
                                          {approval.ip_address}
                                        </p>
                                      </div>
                                      <div>
                                        ...
                                        <p className="text-[11px] font-bold text-green-800 dark:text-green-400">
                                          {approval.geo_location
                                            ? `${approval.geo_location.city}, ${approval.geo_location.country}`
                                            : "N/A"}
                                        </p>
                                      </div>
                                    </div>
            ...
            <div className="space-y-4 border-t border-gray-100 dark:border-v3-border pt-5">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-blue-500 text-white text-[11px] font-bold uppercase tracking-widest rounded-lg hover:bg-blue-500/90 transition-all shadow-lg shadow-blue-900/20 active:scale-[0.98]"
                >
                  Link Oluştur
                </button>
              </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {loadingLogs ? (
              ...
              <div className="text-center text-gray-500 py-8">
                Bu projeye ait log kaydı bulunamadı.
              </div>
            ) : (
              <div className="space-y-4">
                {logsData
                          ...
                          if (changes.length === 0) {
                            return (
                              <div className="text-gray-500 italic text-[11px] py-1">
                                Görsel bir değişiklik tespit edilmedi (sadece
                                arka plan verileri güncellenmiş olabilir).
                              </div>
                            );
                          }
