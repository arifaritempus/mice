        ...
        <div className="absolute z-[100] w-full mt-1 bg-white dark:bg-gray-800 border border-v3-border rounded-xl shadow-lg max-h-60 overflow-y-auto">
          ...
          <div className="p-1">
            {filteredOptions.length === 0 ? (
              <div className="h-10 px-3 text-sm text-v3-muted text-center">Sonuç bulunamadı</div>
            ) : (
              filteredOptions.map((opt: any) => (
                <div
                  key={opt.id}
                  className={`h-10 px-3 text-sm rounded-lg cursor-pointer transition-colors ${value === opt.id ? "bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-medium" : "hover:bg-v3-surface text-v3-text"}`}
                  onClick={() => {
                    onChange(opt.id);
                    setIsOpen(false);
                    setSearch("");
        ...
        <div className="absolute z-[100] w-full mt-1 bg-white dark:bg-gray-800 border border-v3-border rounded-xl shadow-lg max-h-72 overflow-y-auto flex flex-col">
          <div className="sticky top-0 bg-white dark:bg-gray-800 p-2 border-b border-v3-border z-10">
            <input
              type="text"
              autoFocus
              className="w-full bg-v3-bg border border-v3-border rounded-lg h-10 px-3 text-sm focus:outline-none focus:border-blue-500"
            ...
            {filteredOptions.length === 0 ? (
              ...
              filteredOptions.map((opt: any) => {
                ...
                return (
                  <div
                    key={opt.id}
                    className={`h-10 px-3 text-sm rounded-lg cursor-pointer transition-colors flex items-center justify-between ${isSelected ? "bg-blue-50/50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400" : "hover:bg-v3-surface text-v3-text"}`}
      ...
      if (existingQuotes && existingQuotes.length > 0) {
          ...
          await supabase.from("quotes").update({
            ...
            hotels_data: selectedHr.map((hr: any) => {
              ...
              const hotelConcept = dbHotels?.find(h => h.id === hr.hotel_id)?.concept || (hotels as any[]).find(h => h.id === hr.hotel_id)?.concept || "";
              return {
                id: hr.hotel_id,
    ...
    <div className="flex flex-col h-full bg-v3-bg">
      ...
      {responseModalOpen && selectedHotelForResponse && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-v3-surface rounded-2xl border border-v3-border p-6 shadow-2xl w-full max-w-[95vw] lg:max-w-6xl m-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-4 gap-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-v3-border">
                <div>
                  ...
                  <select value={responseStatus} onChange={(e) => setResponseStatus(e.target.value)} className="w-full bg-white dark:bg-gray-800 border border-v3-border rounded-lg h-10 px-3 text-sm focus:outline-none">
                ...
                <div>
                  ...
                  <select value={respOptionType} onChange={(e) => setRespOptionType(e.target.value)} className="w-full bg-white dark:bg-gray-800 border border-v3-border rounded-lg h-10 px-3 text-sm focus:outline-none">
                ...
                <div>
                  ...
                  <input type="date" value={respOptionDate} onChange={(e) => setRespOptionDate(e.target.value)} className="w-full bg-white dark:bg-gray-800 border border-v3-border rounded-lg h-10 px-3 text-sm focus:outline-none" />
                </div>
                <div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-v3-surface rounded-2xl border border-v3-border p-6 shadow-2xl w-full max-w-2xl m-4">
            ...
            <div className="space-y-3 max-h-[60vh] overflow-y-auto">
              {hotelResponses.filter((hr: any) => hr.status === "FİYAT GİRDİ").length === 0 ? (
                ...
                hotelResponses.filter((hr: any) => hr.status === "FİYAT GİRDİ").map((hr: any) => (
