...
export default function UsersPage() {
  ...
  const getRoleBadgeClass = (role: string) => {
    ...
    switch (roleKey) {
      ...
      default:
        return "bg-slate-100 text-slate-800 dark:bg-v3-surface dark:text-v3-text";
    }
  };
  ...
  if (!canView(Module.USERS)) {
    return (
      <div className="min-h-screen bg-transparent flex items-center justify-center transition-all duration-500">
        <div className="text-center p-8 rounded-3xl bg-v3-border0 dark:bg-gray-900/50 backdrop-blur-xl border border-gray-200 dark:border-gray-800 shadow-2xl">
          ...
          <h1 className="text-2xl font-bold text-v3-text mb-2">
            Erişim Engellendi
          </h1>
          <p className="text-v3-muted mb-8 max-w-xs mx-auto">
      ...
      <div className="w-full min-w-0 flex flex-col flex-1 min-h-0">
        ...
        <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-4 mb-4 shrink-0">
          ...
          <div className="flex flex-row items-end justify-start xl:justify-end gap-3 flex-1 flex-wrap">
            ...
            <label className="h-10 bg-v3-surface text-orange-600 dark:text-orange-400 border border-orange-500/20 hover:bg-orange-500/10 hover:border-orange-500/30 py-2 px-4 rounded-xl shadow-sm text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 shrink-0 cursor-pointer">
              {importing ? (
                <>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4 bg-v3-surface backdrop-blur-md border border-v3-border rounded-xl p-2 shadow-sm shrink-0">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all duration-200 flex items-center gap-2 ${statsFilter === "passive" ? "bg-red-500/20 border border-red-500/30 text-red-300" : "text-v3-muted hover:text-v3-text hover:bg-v3-border border border-transparent"}`}
          >
            PASİF
            <span
                ...
                {paginatedUsers.items.map((user) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div>
                        <div className="text-sm font-medium text-v3-text">
                          {user.full_name ||
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-xs text-v3-text">
      ...
      >
        <form onSubmit={handleCreateUser} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-v3-text mb-1.5 flex items-center gap-2">
          ...
          <div>
            ...
            <select
              ...
              required
              className="w-full px-4 py-3 bg-v3-surface border border-v3-border text-v3-text rounded-xl focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all text-sm appearance-none cursor-pointer"
            >
              {roles && roles.length > 0 ? (
                roles
