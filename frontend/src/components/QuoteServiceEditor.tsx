...
interface QuoteServiceEditorProps {
  ...
  hotels?: { id: string; hotel_id: string; hotel_concept: string; name?: string }[];
  hotelId?: string;
  disabled?: boolean;
}
...
export default function QuoteServiceEditor({
  ...
  const handleItemChange = (itemId: string, field: keyof ServiceItem, value: any) => {
    const updatedItems = items.map(item => {
      if (item.id === itemId) {
        ...
        if (field === 'unit_quantity' || field === 'sefer' || field === 'unit_price' || field === 'fx') {
          const qty = field === 'unit_quantity' ? Number(value) : item.unit_quantity;
  ...
  const handleTotalTRYChange = (itemId: string, newTotalTRY: number) => {
    ...
    const updatedItems = items.map(i =>
      i.id === itemId 
        ? { ...i, unit_price: newUnitPrice, total: newTotal, total_try: newTotalTRY }
        : i
    );
    onSave(updatedItems);
  };
  ...
  const sortItemsByCategoryOrder = (itemsToSort: ServiceItem[]): ServiceItem[] => {
    ...
    return [...itemsToSort].sort((a, b) => {
      ...
      const aMainOrder = a.main_category ? (mainCategoryOrder[a.main_category] ?? 999) : 999;
      const bMainOrder = b.main_category ? (mainCategoryOrder[b.main_category] ?? 999) : 999;
      ...
      const aSubOrder = a.sub_category ? (subCategoryOrder[a.sub_category] ?? 999) : 999;
      const bSubOrder = b.sub_category ? (subCategoryOrder[b.sub_category] ?? 999) : 999;
  ...
  const isSomeSubCategoriesSelected = (mainCategoryId: string) => {
    const subCats = subCategoriesByMain[mainCategoryId] || [];
    if (subCats.length === 0) return false;
    const selectedCount = subCats.filter(sub => selectedCategories.has(sub.id)).length;
  ...
  const handleDeleteMainCategory = (mainCategoryName: string) => {
    ...
    const mainCategory = categories.find(c => !c.parent_id && c.name === mainCategoryName);
    if (!mainCategory) return;

    // Bu ana kategoriye ait tüm item'ları bul
    const itemsToDelete = items.filter(item => item.main_category === mainCategory.id);
  ...
  const confirmDeleteMainCategory = () => {
    ...
    // Ana kategori adına göre ana kategori ID'sini bul
    const mainCategory = categories.find(c => !c.parent_id && c.name === categoryToDelete);
    if (!mainCategory) return;

    // Bu ana kategoriye ait tüm item'ları sil
    const updatedItems = items.filter(item => item.main_category !== mainCategory.id);
    onSave(updatedItems);
  ...
  return (
    <div className="space-y-2">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-6">
            ...
            <div className="mb-6">
              <p className="text-sm text-gray-700 dark:text-gray-300">
                <span className="font-semibold">{categoryToDelete}</span> ana kategorisini ve bu ana kategoriye ait tüm alt kategorileri silmek istediğinizden emin misiniz?
              </p>
              {(() => {
                const mainCategory = categories.find(c => !c.parent_id && c.name === categoryToDelete);
      ...
      {showCategoryModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div ref={modalRef} className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
            {/* Modal Header */}
                        ...
                        <div className="flex items-center gap-2 flex-1">
                          ...
                          <label className="flex items-center gap-2 cursor-pointer flex-1" onClick={(e) => e.stopPropagation()}>
                            <CheckboxWithIndeterminate
                              ...
                              onChange={(e) => {
                                e.stopPropagation();
                                toggleAllSubCategories(mainCat.id, e.target.checked);
                              }}
                            />
      ...
      {showAddRow && (
        <div className="rounded-md p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700">
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                onClick={() => {
                  ...
                  console.log('Ana kategori seçim satırındaki Ekle butonuna tıklandı');
      ...
      {items.length > 0 && (
        <div className="space-y-1">
          {(() => {
            ...
            const grouped = sortedItems.reduce((acc: any, item: ServiceItem) => {
            ...
            const sortedMainCategories = Object.keys(grouped).sort((a, b) => {
              // Ana kategori adına göre ana kategori ID'sini bul
              const aMainCat = categories.find(c => !c.parent_id && c.name === a);
              const bMainCat = categories.find(c => !c.parent_id && c.name === b);
              
              if (!aMainCat || !bMainCat) return 0;
              
              // Ana kategori sıralamasına göre karşılaştır
              const aIndex = mainCategories.findIndex(c => c.id === aMainCat.id);
              const bIndex = mainCategories.findIndex(c => c.id === bMainCat.id);
            ...
            sortedMainCategories.forEach((mainCategory) => {
              ...
              const sortedCategoryItems = categoryItems.sort((a: ServiceItem, b: ServiceItem) => {
                const aSubOrder = a.sub_category ? (subCategoriesByMain[a.main_category || '']?.findIndex(c => c.id === a.sub_category) ?? 999) : 999;
                const bSubOrder = b.sub_category ? (subCategoriesByMain[b.main_category || '']?.findIndex(c => c.id === b.sub_category) ?? 999) : 999;
              ...
              // Alt kategoriler
              sortedCategoryItems.forEach((item: ServiceItem, idx: number) => {
                result.push({
                  type: 'item',
                  item,
                  idx
              ...
              // Ara toplam
              const categoryTotal = sortedCategoryItems.reduce((sum: number, item: ServiceItem) => sum + (item.total || 0), 0);
              const categoryTotalTRY = sortedCategoryItems.reduce((sum: number, item: ServiceItem) => sum + (item.total_try || 0), 0);
              result.push({
                type: 'subtotal',
                category: mainCategory,
                total: categoryTotal,
            ...
            if (row.type === 'header') {
              ...
              const mainCategory = categories.find(c => !c.parent_id && c.name === row.category);
              
              return (
                <div key={`header-${row.category}`} className={`bg-gray-200 dark:bg-gray-600 rounded-md p-2 flex items-center justify-between ${!isFirstHeader ? 'mt-4' : ''}`}>
            ...
            if (row.type === 'subtotal') {
              return (
                <div key={`subtotal-${row.category}`} className="bg-gray-100 dark:bg-gray-700 rounded-md p-2 flex flex-wrap md:flex-nowrap items-center gap-2">
                  <div className="w-44 text-xs font-semibold text-gray-800 dark:text-gray-200">ARA TOPLAM</div>
                ...
                {it.isEditing ? (
                  ...
                  <>
                    <select 
                      ...
                      onChange={(e) => handleItemChange(it.id, 'sub_category', e.target.value)} 
                    ...
                    <input 
                      ...
                      onChange={(e) => handleItemChange(it.id, 'unit_quantity', Number(e.target.value))} 
                      type="number" 
                      step="1" 
                    ...
                    <input 
                      value={it.sefer} 
                      onChange={(e) => handleItemChange(it.id, 'sefer', Number(e.target.value))} 
                      type="number" 
                      step="1" 
                    ...
                    <select 
                      value={it.currency} 
                      onChange={(e) => handleItemChange(it.id, 'currency', e.target.value)} 
                      className="w-16 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                    >
                    ...
                    <input 
                      value={it.vat || 0} 
                      onChange={(e) => handleItemChange(it.id, 'vat', Number(e.target.value))} 
                      type="number" 
                      step="1" 
                      min="0" 
                      max="100" 
                    ...
                    <input 
                      value={it.fx || 1} 
                      onChange={(e) => handleItemChange(it.id, 'fx', Number(e.target.value))} 
                      type="number" 
                      step="0.0001" 
                      className="w-16 px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white" 
                    />
                    <input
                      ...
                      onChange={(e) => handleTotalTRYChange(it.id, Number(e.target.value))}
                      type="number"
                      step="0.01"
                    ...
                    {(it.main_category === 'CAT_001' || it.main_category === 'CAT_002') && hotels.length > 0 && (
                      <select 
                        value={it.hotel_id || ''} 
                        onChange={(e) => handleItemChange(it.id, 'hotel_id', e.target.value)} 
                      ...
                      >
                        ...
                        {hotels.map(h => (
                          <option key={h.id} value={h.id}>
                            {h.name || (h.hotel_id ? `Otel #${h.id.slice(-4)}` : 'Seçilmedi')}
                          </option>
                        ))}
                      </select>
                    )}
                    {(it.main_category !== 'CAT_001' && it.main_category !== 'CAT_002') && (
                    ...
                    <div className="w-24 flex items-center gap-1 justify-end pr-1">
                      ...
                      <button 
                        ...
                        onClick={() => {
                          const updatedItems = items.map(item =>
                            item.id === it.id ? { ...item, isEditing: false } : item
                          );
                          onSave(updatedItems);
                        }} 
                        className="p-1 rounded text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/30" 
                  ...
                  <>
                    ...
                    <div className="w-16 text-right pr-1 text-xs text-gray-900 dark:text-white">{Math.round(it.unit_quantity)}</div>
                    <div className="w-20 text-right pr-1 text-xs text-gray-900 dark:text-white">{Math.round(it.sefer)}</div>
                    ...
                    <div className="w-16 text-xs text-gray-900 dark:text-white">%{it.vat || 0}</div>
                    <div className="w-16 text-right pr-1 text-xs text-gray-900 dark:text-white">{formatTRY(it.fx || 1)}</div>
                    <div className="w-32 text-right pr-1 text-xs font-semibold text-gray-900 dark:text-white">{formatTRY(it.total_try || 0)}</div>
                    
                    <div className="w-32 text-xs text-gray-900 dark:text-white ml-2">
                       {it.hotel_id && hotels.find(h => h.id === it.hotel_id) ? (
                         <span className="px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 rounded text-[10px] font-medium border border-blue-200 dark:border-blue-800">
                           {hotels.find(h => h.id === it.hotel_id)?.name || 'Seçili Otel'}
                         </span>
                       ) : '-'}
                    </div>
          ...
          <div className="mt-4 bg-blue-600 dark:bg-blue-700 rounded-md p-3">
            <div className="flex flex-wrap md:flex-nowrap items-center gap-2">
              ...
              <div className="w-16"></div>
              <div className="w-32 text-right pr-1 text-sm font-bold text-white">
                {formatTRY(items.reduce((sum: number, it: ServiceItem) => sum + Number(it.total_try || 0), 0))} TL
              </div>
              <div className="flex-1 min-w-0"></div>
              <div className="w-24"></div>
            </div>
