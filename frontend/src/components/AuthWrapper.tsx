...
export default function AuthWrapper({
  ...
  const resolveModule = (path: string): Module | undefined => {
    ...
    if (path.startsWith("/accounting/invoices/expense")) return Module.INVOICES_EXPENSE;
    ...
    if (path.startsWith("/permissions/roles")) return Module.ROLES;
    ...
    if (path.startsWith("/sejour/services")) return Module.SEJOUR_SERVICES;
    ...
    if (path.startsWith("/suppliers/service-types")) return Module.SUPPLIER_CATEGORIES;
    if (path.startsWith("/suppliers")) return Module.SUPPLIERS;
    if (path.startsWith("/tickets/options")) return Module.TICKETS_OPTIONS;
    if (path.startsWith("/tickets/payments")) return Module.TICKETS_PAYMENTS;
    if (path.startsWith("/tickets/calendar")) return Module.TICKETS_CALENDAR;
