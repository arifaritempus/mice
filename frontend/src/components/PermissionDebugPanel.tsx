...
export default function PermissionDebugPanel() {
  const { canView, canCreate, canEdit, canDelete, userRole, loading, getAllPermissions } = usePermissions();
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-4 z-50 bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-4 w-96 max-h-[80vh] overflow-y-auto border-2 border-purple-500">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">Yetki Debug Paneli</h3>
            <button
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
              <h4 className="font-semibold text-gray-900 dark:text-white mb-2">Kullanıcı Bilgileri</h4>
              <div className="text-sm">
                <p className="text-gray-700 dark:text-gray-300">
                  <span className="font-medium">Rol:</span> {userRole || 'Yükleniyor...'}
                </p>
                <p className="text-gray-700 dark:text-gray-300">
            ...
            {/* Modül Yetkileri */}
            <div>
              <h4 className="font-semibold text-gray-900 dark:text-white mb-2">Modül Yetkileri</h4>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className={`text-xs ${hasView ? 'text-green-600' : 'text-red-600'}`}>
                          {hasView ? '✅' : '❌'}
                        </span>
                      </div>
                      <div className="flex gap-2 text-xs">
                        <span className={hasView ? 'text-green-600' : 'text-gray-400'}>👁️ View</span>
                        <span className={hasCreate ? 'text-blue-600' : 'text-gray-400'}>➕ Create</span>
                        <span className={hasEdit ? 'text-yellow-600' : 'text-gray-400'}>✏️ Edit</span>
                        <span className={hasDelete ? 'text-red-600' : 'text-gray-400'}>🗑️ Delete</span>
                      </div>
                      {perms.length > 0 && (
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3">
              <h4 className="font-semibold text-gray-900 dark:text-white mb-2">Hızlı Test</h4>
              <div className="space-y-2 text-sm">
                <button
                  ...
                  className="w-full bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700 text-xs"
                >
                  Console'a Yazdır
                </button>
              </div>
