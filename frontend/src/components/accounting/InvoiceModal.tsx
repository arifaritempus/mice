...
function sortInvoiceModalItems(items: any[], categories: any[]): any[] {
  ...
  const idToRank = (value: any) => {
    ...
    return { w: Number.isFinite(weight) ? weight : Number.MAX_SAFE_INTEGER, key: key.toUpperCase() };
  };
  ...
  const rank = (item: any) => {
    ...
    if (item.category) {
      ...
      if (item.sub_category) {
        ...
        subManualId = String(subCat?.code || subCat?.id || item.sub_category || '');
      }
      ...
      mainManualId = String(mainCat?.code || mainCat?.id || item.category_name || '');
      subManualId = String(subCat?.code || subCat?.id || item.sub_category_name || '');
    }
  ...
  return [...items].sort((a, b) => {
    ...
    const c0 = ra.mainRank.key.localeCompare(rb.mainRank.key, 'en', { numeric: true });
    if (c0 !== 0) return c0;
    if (ra.subRank.w !== rb.subRank.w) return ra.subRank.w - rb.subRank.w;
    return ra.subRank.key.localeCompare(rb.subRank.key, 'en', { numeric: true });
  });
}
...
export default function InvoiceModal({ isOpen, onClose, selectedItems, type, onSuccess, editInvoice }: InvoiceModalProps) {
  ...
  const [itemExchangeRates, setItemExchangeRates] = useState<Record<string, number>>({});
      ...
      if (editInvoice) {
        ...
        if (selectedItems.length > 0) {
          if (type === 'income') {
            ...
            if (firstAgency && selectedItems.every(item => item.project?.agency_id === firstAgency)) {
            ...
            if (firstSupplier && selectedItems.every(item => item.supplier_id === firstSupplier)) {
              setSelectedAccountId(firstSupplier);
              setSelectedAccountType('vendor');
            } else if (firstHotel && selectedItems.every(item => item.hotel_id === firstHotel)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedAccountId || !invoiceNo || !invoiceDate) {
      alert('Lütfen faturanın asıl zorunlu alanlarını (No, Tarih, Cari Hesap) doldurun.');
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="flex-1 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
            <div className="md:col-span-8 grid grid-cols-2 md:grid-cols-4 gap-4 p-5 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700">
              <div className="col-span-2">
                <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Fatura No *</label>
                <input
                  required
                  type="text"
            ...
            <div className="md:col-span-4 flex flex-col h-full space-y-2">
              <label className="block text-xs font-semibold text-gray-500 uppercase">Cari Hesap Seçimi *</label>
              <div className="relative flex flex-col h-full border rounded-xl dark:border-gray-700 overflow-hidden bg-white dark:bg-gray-800/30">
                ...
                <div className="flex-1 overflow-y-auto max-h-[120px] p-2 space-y-1">
                  {filteredAccounts.map(acc => {
                    ...
                    const typeLabels = { agency: 'Acente', hotel: 'Otel', vendor: 'Tedarikçi' };
                    // @ts-ignore
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 overflow-hidden">
                          {isSelected && (
                            <svg className="w-4 h-4 text-blue-600 dark:text-blue-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                          )}
                          <div className="min-w-0">
                            <h4 className="text-sm font-semibold text-gray-900 dark:text-white truncate">
                              {acc.name || acc.company_name}
                            </h4>
                            <span className="text-[10px] uppercase text-gray-500">{typeLabel}</span>
                          </div>
                        </div>
                      </div>
                    );
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden shadow-sm min-w-[900px]">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-800/80">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase w-[25%]">Kategori & Proje</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase w-[25%]">Satır Açıklaması</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase w-[15%]">Döviz/Kur</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase w-[10%]">KDV (%)</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase w-[15%]">KDV Dahil Tutar</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase w-[10%]">İşlem</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-gray-900">
                  {sortedLineItems.map(item => {
                    ...
                    const projectTypeBadge = isSejour ? 'SEJOUR' : (isMice ? 'MICE' : 'PROJE');
                      ...
                      <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex gap-2 items-center mb-1">
                            ...
                            <span className="text-sm font-bold text-gray-900 dark:text-white truncate">
                              {item.category_name}
                              {item.sub_category_name && <span className="text-gray-400 font-normal ml-1 text-xs">({item.sub_category_name})</span>}
                            </span>
                          </div>
                          <div className="text-xs text-blue-500 dark:text-blue-400 font-semibold truncate">
                        ...
                        <td className="px-4 py-3">
                          <input
                            ...
                            onChange={(e) => setItemDescriptions(prev => ({ ...prev, [item.id]: e.target.value }))}
                          />
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex flex-col gap-1">
                            <select
                              ...
                              onChange={(e) => setItemCurrencies(prev => ({ ...prev, [item.id]: e.target.value }))}
                            >
                            ...
                            <input
                              ...
                              onChange={(e) => setItemExchangeRates(prev => ({ ...prev, [item.id]: Number(e.target.value) }))}
                            />
                          </div>
                        </td>
                        <td className="px-4 py-3 text-right">
                          <input
                            ...
                            onChange={(e) => setItemVats(prev => ({ ...prev, [item.id]: Number(e.target.value) }))}
                          />
                        </td>
                        <td className="px-4 py-3 text-right">
                          <div className="flex flex-col items-end">
                            <input
                              ...
                              onChange={(e) => setItemAmounts(prev => ({ ...prev, [item.id]: Number(e.target.value) }))}
                            />
                            {!editInvoice && (
                              <span className="text-[10px] text-gray-500 mt-0.5 whitespace-nowrap">
                                Max: {new Intl.NumberFormat('tr-TR').format(item.balance)}
                              </span>
                            )}
                          </div>
                        </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            <div className="space-y-2">
              <label className="block text-xs font-semibold text-gray-500 uppercase">Fatura Genel Açıklaması / Notlar</label>
              <textarea
            ...
            <div className="bg-gray-50 dark:bg-gray-800/80 rounded-xl border border-gray-200 dark:border-gray-700 p-6 flex flex-col justify-center space-y-4 shadow-sm">
              <div className="flex justify-between items-center text-sm font-medium text-gray-600 dark:text-gray-400">
                ...
                <span className="text-gray-900 dark:text-white">{new Intl.NumberFormat('tr-TR', { minimumFractionDigits: 2 }).format(totals.matrahTRY)} <span className="text-xs">TRY</span></span>
              </div>
              <div className="flex justify-between items-center text-sm font-medium text-gray-600 dark:text-gray-400 pb-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <span className="text-gray-900 dark:text-white">{new Intl.NumberFormat('tr-TR', { minimumFractionDigits: 2 }).format(totals.kdvTRY)} <span className="text-xs">TRY</span></span>
              </div>
              <div className="flex justify-between items-end pt-2">
                ...
                <div className="text-right">
                  <div className="text-2xl font-black text-blue-600 dark:text-blue-400 leading-none mb-1">
                    {new Intl.NumberFormat('tr-TR', { minimumFractionDigits: 2 }).format(totals.genelToplamTRY)} <span className="text-base font-semibold">TRY</span>
                  </div>
                </div>
              </div>
            </div>
