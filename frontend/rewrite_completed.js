...
import { ResponsiveDateRangeField } from '@/components/accounting/ResponsiveDateRangeField';
import { MultiTokenFilterInput } from '@/components/accounting/MultiTokenFilterInput';
import { DEFAULT_PAGE_SIZE } from '@/types/pagination';
import { usePermissions, Module } from '@/lib/permissions';
import { Download, X } from 'lucide-react';
import * as XLSX from 'xlsx';

export default function IncomeCompletedPage() {
  ...
  const filteredInvoices = useMemo(() => {
    ...
    return allInvoices.filter((inv) => {
      ...
      const combinedHaystack = [
        voucher, reference, firmaBarHaystack, acenteBarHaystack, hotelSearchTarget, category, notes, inv.invoice_no, inv.contact_name
      ].join(' ').toLowerCase();
  ...
  const exportToExcel = () => {
    const exportData = filteredInvoices.map(inv => ({
      ...
      'Cari': inv.contact_name,
      'Firma / Acente': inv.metadata?.company_name || '-',
      'Otel': inv.metadata?.hotel_name || '-',
      'Hizmet Tarihi': inv.metadata?.date_start ? \`\${new Date(inv.metadata.date_start).toLocaleDateString('tr-TR')} - \${new Date(inv.metadata.date_end).toLocaleDateString('tr-TR')}\` : '-',
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Tamamlanan Gelir Faturalari");
    XLSX.writeFile(wb, \`Tamamlanan_Gelir_Faturalari_\${new Date().toISOString().split('T')[0]}.xlsx\`);
  };
    ...
    <div className="flex flex-col h-[calc(100vh-2rem)] p-4 space-y-4 text-slate-900 dark:text-slate-100 w-full min-w-0 transition-colors duration-200 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row items-end gap-6 bg-[#0f172a] p-6 rounded-2xl border border-white/10 shadow-2xl relative overflow-hidden shrink-0">
        ...
        <div className="shrink-0 mr-2 relative z-10">
          <h1 className="text-2xl font-light tracking-wide text-white glow-text">Tamamlanan Gelir Faturaları</h1>
          <p className="text-xs text-slate-400 mt-1">Kaydı tamamlanmış gelir faturalarının listesi</p>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-4 flex-1 relative z-10">
          ...
          <div className="flex-[2] min-w-[300px] max-w-lg">
            <MultiTokenFilterInput
              label="Genel Arama (Fatura No, Voucher, Firma, Acente, Otel vb.)"
              tokens={globalTokens}
      ...
      {loading && allInvoices.length === 0 ? (
        ...
        <div className="bg-[#0f172a]/40 backdrop-blur-md rounded-2xl border border-white/10 flex-1 min-h-0 flex flex-col w-full relative mt-4 overflow-hidden">
          ...
          <PaginationControls
            ...
            preferenceKey="income_completed_page_size"
            compactRight
...
fs.writeFileSync('src/app/accounting/invoices/income/completed/page.tsx', code);
