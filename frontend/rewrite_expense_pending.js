...
export default function ExpensePendingPage() {
  ...
  const exportToExcel = () => {
    const exportData = filteredItems.map(item => ({
      ...
      'Tedarikçi': item.project?.supplier_name || item.supplier_name || item.supplier?.name || '-',
      'Otel': item.project?.hotel_name || item.hotel_name || '-',
      'Proje/Voucher': item.project?.voucher_number || item.project?.title || '-',
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Bekleyen_Giderler");
    XLSX.writeFile(wb, \`Bekleyen_Gider_Faturalari_\${new Date().toISOString().split('T')[0]}.xlsx\`);
  };
    ...
    return (
      <div className="min-h-screen bg-transparent flex items-center justify-center transition-colors duration-200">
        <div className="text-center">
          ...
          <a href="/accounting" className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors">
            Muhasebeye Dön
          </a>
        </div>
      </div>
    ...
    <div className="h-full w-full p-6 sm:p-8 flex flex-col gap-6 overflow-hidden font-sans text-white">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-4 shrink-0">
        ...
        <div className="shrink-0 mr-2">
          <h1 className="text-2xl font-light tracking-wide text-white glow-text">Bekleyen Gider Faturaları</h1>
          <p className="text-xs text-slate-400 mt-1">Sisteme girilmeyi bekleyen alış/gider kalemleri</p>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-4 flex-1">
          ...
          <div className="shrink-0 flex items-center gap-3 ml-auto">
            {(dateRange.start || dateRange.end || globalTokens.length > 0 || globalInput.trim().length > 0) && (
              <button
                onClick={() => {
            ...
            <button
              ...
              className="bg-orange-600 hover:bg-orange-500 text-white shadow-[0_0_15px_rgba(234,88,12,0.2)] hover:shadow-[0_0_25px_rgba(234,88,12,0.4)] px-4 h-10 rounded-xl transition-all duration-300 text-[11px] font-bold tracking-wide flex items-center justify-center gap-2 hover:scale-105 uppercase disabled:opacity-50 disabled:pointer-events-none"
            >
              Fatura Girişi {selectedItems.length > 0 && \`(\${selectedItems.length})\`}
            </button>
          </div>
        </div>
      </div>
      ...
      {selectedItems.length > 0 && (
        <div className="flex items-center gap-3 p-4 bg-[#0f172a]/40 backdrop-blur-md border border-white/10 shadow-sm rounded-2xl animate-in slide-in-from-top-2 duration-300 mt-4 shrink-0">
          ...
          <div>
            <h4 className="text-[10px] font-black uppercase tracking-widest text-orange-500 dark:text-orange-400">Seçilen Kalemlerin Toplamı ({selectedItems.length} Kalem)</h4>
...
fs.writeFileSync('src/app/accounting/invoices/expense/pending/page.tsx', code, 'utf8');
