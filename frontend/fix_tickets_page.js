...
const oldHeaderBlockRegex = /{\/\* Header \*\/}[\s\S]*?(?={\/\* Tickets Table \*\/})/;
...
const newHeaderBlock = `{/* Unified Header */}
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-2">
          ...
          <div className="shrink-0 mr-4">
            ...
            <p className="text-xs text-slate-400 mt-1">Uçak biletlerinin opsiyon tarihlerini ve detaylarını yönetin</p>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 bg-[#0f172a]/40 backdrop-blur-md border border-white/10 rounded-xl p-2 shadow-sm shrink-0 mb-3 text-xs">
          <div className="flex items-center gap-2">
            <span className="text-slate-400 font-medium uppercase tracking-wider ml-2">BİLET KAYNAĞI:</span>
          ...
          <div className="flex items-center gap-2 border-l border-white/10 pl-4">
            <span className="text-slate-400 font-medium uppercase tracking-wider">GÖRÜNÜM:</span>
            <button onClick={() => setActiveTab('detail')} className={\`px-3 py-1.5 rounded-lg transition-colors flex items-center gap-2 \${activeTab === 'detail' ? 'bg-purple-500/20 border border-purple-500/50 text-white' : 'hover:bg-white/5 border border-transparent text-slate-300'}\`}>
              <span>📋 DETAY</span>
            </button>
            <button onClick={() => setActiveTab('summary')} className={\`px-3 py-1.5 rounded-lg transition-colors flex items-center gap-2 \${activeTab === 'summary' ? 'bg-pink-500/20 border border-pink-500/50 text-white' : 'hover:bg-white/5 border border-transparent text-slate-300'}\`}>
              <span>📊 ÖZET</span>
            </button>
          </div>
        </div>
...
content = content.replace(/className="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-200"/g, 
...
content = content.replace(/className="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider"/g, 
...
content = content.replace(
  /className="hover:bg-gray-50 dark:hover:bg-gray-700\/50 transition-colors duration-150 cursor-pointer"/g,
  'className="hover:bg-blue-500/10 transition-colors group cursor-pointer border-b border-white/5 last:border-0"'
);

// 7. `td` styling
content = content.replace(/px-3 py-2 whitespace-nowrap text-xs text-gray-900 dark:text-white/g, 'px-2.5 py-2.5 whitespace-nowrap text-[11px] text-white');
content = content.replace(/px-3 py-2 whitespace-nowrap text-xs text-gray-500 dark:text-gray-400/g, 'px-2.5 py-2.5 whitespace-nowrap text-[11px] text-slate-300');
content = content.replace(/px-3 py-2 text-xs text-gray-900 dark:text-white/g, 'px-2.5 py-2.5 text-[11px] text-white');
...
console.log("Tickets page modernized successfully.");
