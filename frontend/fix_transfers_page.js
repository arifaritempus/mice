...
let content = fs.readFileSync('src/app/operations/transfers/page.tsx', 'utf8');
...
const oldHeaderBlockRegex = /{\/\* Header \*\/}[\s\S]*?(?={\/\* Transferler Tablosu \*\/})/;

const newHeaderBlock = `{/* Unified Header */}
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-2">
          {/* Left: Title */}
          <div className="shrink-0 mr-4">
            <h1 className="text-2xl font-light tracking-wide text-white glow-text">Transfer Yönetimi</h1>
            <p className="text-xs text-slate-400 mt-1">MICE ve Sejour Transfer Operasyonlarını Yönetin</p>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-4 bg-[#0f172a]/40 backdrop-blur-md border border-white/10 rounded-xl p-2 shadow-sm shrink-0 mb-3 text-xs">
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setFilter('all')} className={\`px-3 py-1.5 rounded-lg transition-colors flex items-center gap-2 \${filter === 'all' ? 'bg-blue-500/20 border border-blue-500/50 text-white' : 'hover:bg-white/5 border border-transparent text-slate-300'}\`}>
              <span>TÜMÜ</span>
              <span className="font-bold">{typeCounts.all}</span>
            </button>
            <button onClick={() => setFilter('mice')} className={\`px-3 py-1.5 rounded-lg transition-colors flex items-center gap-2 \${filter === 'mice' ? 'bg-indigo-500/20 border border-indigo-500/50 text-white' : 'hover:bg-white/5 border border-transparent text-slate-300'}\`}>
              <span>MICE</span>
              <span className="font-bold">{typeCounts.mice}</span>
            </button>
            <button onClick={() => setFilter('sejour')} className={\`px-3 py-1.5 rounded-lg transition-colors flex items-center gap-2 \${filter === 'sejour' ? 'bg-emerald-500/20 border border-emerald-500/50 text-white' : 'hover:bg-white/5 border border-transparent text-slate-300'}\`}>
              <span>SEJOUR</span>
...
// 3. Table Wrappers
content = content.replace(
  /className={`bg-white dark:bg-gray-800 rounded-lg shadow transition-colors duration-200 w-full min-w-0 flex-1 flex flex-col min-h-\[300px\] md:min-h-0 relative \${tableBusy \? 'opacity-80' : ''}`}/g,
...
// 8. Fix searchTerm logic to use referenceTokens as global search
content = content.replace(
  "searchTerm: '',",
  "searchTerm: referenceTokens.join(' '),"
);

fs.writeFileSync('src/app/operations/transfers/page.tsx', content, 'utf8');

console.log("Transfers page modernized successfully.");
