...
// Find the section to replace: from {/* Header */} to {/* Data Table */}
const startIdx = content.indexOf('{/* Header */}');
const endIdx = content.indexOf('{/* Data Table */}');
...
if (startIdx !== -1 && endIdx !== -1) {
  const newHeader = `{/* Unified Header */}
      <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-4 mb-2">
        ...
        <div className="flex flex-wrap items-end gap-3 flex-1 xl:justify-end">
          ...
          {(dateStart || dateEnd || voucherTokens.length > 0 || customerTokens.length > 0 || agencyTokens.length > 0 || guestTokens.length > 0) && (
            <button
  ...
  console.log("Successfully replaced header and stats strip.");
} else {
