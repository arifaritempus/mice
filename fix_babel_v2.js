  ...
  traverse(ast, {
    JSXOpeningElement(path) {
      ...
      if (name === 'input' || name === 'select' || name === 'textarea') {
        ...
        if (!hasDisabled) {
          const closestFuncDecl = path.findParent(p => t.isFunctionDeclaration(p));
          let isMain = false;
          if (closestFuncDecl && closestFuncDecl.node.id && closestFuncDecl.node.id.name === mainFuncName) {
             isMain = true;
          }

          if (isMain) {
            const disableExpr = parser.parseExpression(`!permEdit || (isLocked && !isSuperAdmin)`);
